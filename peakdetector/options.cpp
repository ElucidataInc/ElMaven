...
OptStrTokIter::OptStrTokIter(const char * tokens, const char * delimiters) :
		len(unsigned(strlen(tokens))), str(tokens), seps(delimiters), cur(
		NULLSTR), tokstr(NULLSTR) {
	if (seps == NULL)
		seps = default_delims;
...
enum kwdmatch_t {
	NO_MATCH, PARTIAL_MATCH, EXACT_MATCH
};
...
//    Trivial
// ^^-------------------------------------------------------------------------
static kwdmatch_t kwdmatch(const char * src, const char * attempt,
		int len = 0) {
	int i;
	...
	for (i = 0;
			((i < len) || (len == 0)) && (attempt[i]) && (attempt[i] != ' ');
			i++) {
...
public:
	...
	operator const char*() {
		return isHiddenOpt() ? (spec - 1) : spec;
	}

	// Is this option NULL?
	int isNULL(void) const {
		return ((spec == NULL) || (spec == NULL_spec));
	}
	...
	int isHiddenOpt(void) const {
		return hidden;
	}

	// Get the corresponding option-character
	char OptChar(void) const {
		return *spec;
	}
	...
	const char *
	LongOpt(void) const {
		return (spec[1] && spec[2] && (!isspace(spec[2]))) ?
				(spec + 2) : NULLSTR ;
	}
...
Options::Options(const char * name, const char * const optv[]) :
		cmdname(name), optvec(optv), explicit_end(0), optctrls(DEFAULT), nextchar(
		NULLSTR), listopt(NULLSTR) {
...
int Options::parse_opt(OptIter & iter, const char * & optarg) {
	...
	OptionSpec optspec = match_opt(*(nextchar++),
			(optctrls & Options::ANYCASE));
	...
	if (optspec.isNULL()) {
		...
		if (!(optctrls & Options::QUIET)) {
			cerr << cmdname << ": unknown option -" << *(nextchar - 1) << "."
					<< endl;
		}
...
int Options::parse_longopt(OptIter & iter, const char * & optarg) {
	...
	if (optspec.isNoArg()) {
		if ((val) && !(optctrls & Options::QUIET)) {
			cerr << cmdname << ": option "
					<< ((optctrls & Options::LONG_ONLY) ? "-" : "--")
					<< optspec.LongOpt() << " does NOT take an argument."
					<< endl;
		}
