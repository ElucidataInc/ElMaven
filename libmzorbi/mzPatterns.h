...
public:
    ...
    int longestSymmetry(char x, char y) {
        ...
        for(unsigned int i=0; i < pattern.length()-1; i++ ) {
            ...
            int ycount=0;
            if (pattern[i] == x && pattern[i+1] == y) {
                for(int j=i; j>=0; j--) if (pattern[j]==x)  xcount++;
                    else break;
                for(unsigned int j=i+1; j<pattern.length(); j++) if (pattern[j]==y)  ycount++;
                    else break;
    ...
    float noiseness() {
        ...
        for(int i=lb; i<rb; i++ ) {
            ...
            if (i+1 < rb && pattern[i] == '-' && pattern[i+1] == '+') {
                count++;
                continue;
            }
            if (i+1 < rb && pattern[i] == '+' && pattern[i+1] == '0') {
                count++;
                continue;
            }
        }
