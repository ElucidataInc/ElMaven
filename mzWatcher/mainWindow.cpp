...
bool MainWindow::checkParameters() {
    ...
    bool ok=true;
    if (!s.exists()) {
        setStatus("Source Path doesn't exists");
        return 0;
    ...
    QDir d(destFolder);
    if (!d.exists()) {
        setStatus("Destination Path doesn't exists");
        return 0;
        guiForm->sourceFolderEdit->setFrame(true);
        ok=false;
    }
...
QString MainWindow::createTempPath() {
    ...
    if (! tempPath.exists()) {
        tempPath.mkpath(tempPathName);
        tempPath=QDir(tempPathName);
    }
    return tempPathName;
}
...
void MainWindow::getDBFileList() {
    ...
    if(!query.exec()) {
        setStatus("Error: showDataFilesTable:" + query.lastError().text());
        return;
    }
    while (query.next()) {
