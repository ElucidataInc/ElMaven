...
float Edge::computeArrowSize(float flux) {
	...
	GraphWidget* g = NULL;
	if (source)
		g = source->getGraphWidget();
	float size = g->getAvgEdgeLength() * 0.25;
	float scale = 1;
	if (g)
		scale *= g->getEdgeSizeScale();
	float arrowSize = size * scale;
	if (flux > 0) {
		arrowSize += 5 * log2(flux);
	}
	if (arrowSize < 2)
		arrowSize = 2;
...
void Edge::drawArrow(QPainter *painter, int direction) {
	...
	if (edgeLength > sceneLength)
		return;

	_arrowSize = arrowSize;
	if (_arrowSize > edgeLength)
		_arrowSize = edgeLength / 2;
	...
	if (source->isEnzyme())
		a = ((EnzymeNode*) source)->activeSitePosition();
	if (dest->isEnzyme())
		b = ((EnzymeNode*) dest)->activeSitePosition();
	...
	if (direction != 0) { //forward
		...
		QPointF p1 = b
				+ QPointF(cos(angle + Pi - arrowAngle) * arrowAngleSize,
						sin(angle + Pi - arrowAngle) * arrowAngleSize);
		QPointF p1m = b
				+ QPointF(cos(angle + Pi - arrowAngle / 2) * arrowAngleSize,
						sin(angle + Pi - arrowAngle / 2) * arrowAngleSize);
		QPointF p2 = b
				+ QPointF(cos(angle + Pi + arrowAngle) * arrowAngleSize,
						sin(angle + Pi + arrowAngle) * arrowAngleSize);
		QPointF p2m = b
		...
		QPointF a1m = a
				+ QPointF(cos(angle + Pi / 2) * arrowAngleSize / 20,
						sin(angle + Pi / 2) * arrowAngleSize / 20);
		QPointF a2m = a
				+ QPointF(cos(angle - Pi / 2) * arrowAngleSize / 20,
						sin(angle - Pi / 2) * arrowAngleSize / 20);
