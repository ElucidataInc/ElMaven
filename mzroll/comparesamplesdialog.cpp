...
void CompareSamplesDialog::showEvent(QShowEvent *) {
    ...
    foreach (mzSample* sample, samples ) {
        ...
        foreach(QString name, names) {
            ...
            if (!name.isEmpty()) setnames.insert(name);
        }
    }
...
void CompareSamplesDialog::compareSets() {
    ...
    for (int i = 0; i < allgroups.size(); i++ ) {
        ...
        vector<float> yvalues = group->getOrderedIntensityVector(sampleSet, PeakGroup::AreaTop);
