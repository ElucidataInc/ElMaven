...
Note::Note(UserNote* usernote, QGraphicsItem *parent) :
		QGraphicsItem(parent) {
	setupGraphicOptions();

	if (usernote) {

		QString text = tr("<b>%2</b><br>%3<br> rt:%4").arg(usernote->title,
				usernote->description, QString::number(usernote->rt, 'f', 2));
...
void Note::showBoundBox() {
	...
	if (_labelBox) {
		...
		if (_labelBox->scene() != scene())
			scene()->addItem(_labelBox);
...
void Note::mouseDoubleClickEvent(QGraphicsSceneMouseEvent*) {
	...
	if (editable) {
		bool ok;
		QString text = QInputDialog::getText(0, tr("Edit Note"), tr("Note:"),
				QLineEdit::Normal, toPlainText(), &ok);
