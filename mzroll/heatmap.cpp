...
void HeatMap::drawMap() {
	...
	for (int i = 0; i < Nrows; i++) {
		...
		StatisticsVector<float> yvalues = group->getOrderedIntensityVector(
				vsamples, PeakGroup::AreaTop);
		...
		if (center)
			for (int j = 0; j < yvalues.size(); j++) {
				...
				if (ratio != 0)
					ratio = log2(ratio);  //fold change on log2 scale
				heatmap(i, j) = ratio;
				if (ratio > _heatMax)
					_heatMax = ratio;
				if (ratio < _heatMin)
	...
	for (int i = 0; i < heatmap.rows(); i++) {
		...
		for (int j = 0; j < heatmap.cols(); j++) {
			...
			QGraphicsRectItem* item = scene()->addRect(
					QRectF(xpos, ypos, _boxW, _boxH), pen, brush);
	...
	for (int i = 0; i < vsamples.size(); i++) {
		QGraphicsTextItem* item = scene()->addText(
				QString(vsamples[i]->sampleName.c_str()));
		int textWidth = item->boundingRect().width();
	...
	for (int i = 0; i < steps; i++) {
		...
		QGraphicsRectItem* item = scene()->addRect(
				QRectF(-boxSize / 2, -boxSize / 2, boxSize, boxSize), pen,
				brush);
		QGraphicsTextItem* text = scene()->addText(
				QString::number(cellValue, 'f', 0));
