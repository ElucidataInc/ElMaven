...
Rexp::~Rexp() {
	...
	if (msg) {
		if (rcount > 0)
			fprintf(stderr,
					"WARNING! Rexp master %lx delete requested, but %d object(s) are using our memory - refusing to free, leaking...\n",
					(long) this, rcount);
		else
			delete (msg);
	}
...
Rexp *Rexp::attribute(const char *name) {
	return (attr && attr->type == XT_LIST) ?
			((Rlist*) attr)->entryByTagName(name) : 0;
}
...
void Rsymbol::fix_content() {
	if (type == XT_SYM && *data == 3)
		name = data + 4; // normally the symbol should consist of a string SEXP specifying its name - no further content is defined as of now
	if (type == XT_SYMNAME)
		name = data; // symname consists solely of the name
#ifdef DEBUG_CXX
				printf("SYM %p \"%s\"\n", this, name);
#endif
}
...
int Rvector::indexOfString(const char *str) {
	...
	while (i < count) {
		if (cont[i] && cont[i]->type == XT_STR
				&& !strcmp(((Rstring*) cont[i])->string(), str))
			return i;
		i++;
	}
	return -1;
...
Rexp* Rvector::byName(const char *name) {
	if (count < 1 || !attr
			|| (attr->type != XT_LIST && attr->type != XT_LIST_TAG))
		return 0;
	Rexp *e = ((Rlist*) attr)->head;
	if (((Rlist*) attr)->tag)
		e = ((Rlist*) attr)->entryByTagName("names");
	if (!e
			|| (e->type != XT_VECTOR && e->type != XT_ARRAY_STR
					&& e->type != XT_STR))
		return 0;
	if (e->type == XT_VECTOR) {
...
int Rconnection::connect() {
	...
	if (family == AF_INET) {
		...
		int opt = 1;
		setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (const char*) &opt,
				sizeof(opt));
#endif
	...
	if (strncmp(IDstring + 8, myID + 8, 4)
			|| strncmp(IDstring + 4, myID + 4, 4) > 0) {
		closesocket(s);
		s = -1;
	...
	}
	{
		int i = 12;
		while (i < 32) {
			if (!strncmp(IDstring + i, "ARuc", 4))
				auth |= A_required | A_crypt;
			if (!strncmp(IDstring + i, "ARpt", 4))
				auth |= A_required | A_plain;
			if (IDstring[i] == 'K') {
				salt[0] = IDstring[i + 1];
				salt[1] = IDstring[i + 2];
			}
...
int Rconnection::shutdown(const char *key) {
	Rmessage *msg = new Rmessage();
	Rmessage *cm =
			key ? new Rmessage(CMD_shutdown, key) : new Rmessage(CMD_shutdown);
...
int Rconnection::assign(const char *symbol, Rexp *exp) {
	...
	((unsigned int*) (cm->data + 4 + tl))[0] = SET_PAR(
			(Rsize_t) ((xl>0x7fffff)?(DT_SEXP|DT_LARGE):DT_SEXP),
			(Rsize_t ) xl);
	((unsigned int*) (cm->data + 4 + tl))[0] = itop(
			((unsigned int* )(cm->data + 4 + tl))[0]);
	if (xl > 0x7fffff)
...
Rexp *Rconnection::eval(const char *cmd, int *status, int opt) { /* opt = 1 -> void eval */
	Rmessage *msg = new Rmessage();
	Rmessage *cmdMessage = new Rmessage((opt & 1) ? CMD_voidEval : CMD_eval,
			cmd);
	int res = request(msg, cmdMessage);
	delete (cmdMessage);
	if (opt & 1 && !res) {
		if (status)
			*status = 0; // we should put response code here
		delete (msg);
		return 0;
	}
	if ((opt & 1) == 0 && !res
			&& (msg->pars != 1 || (ptoi(msg->par[0][0]) & 0x3f) != DT_SEXP)) {
		delete (msg);
		if (status)
			*status = -12; // returned object is not SEXP
		return 0;
	}
	if (res) {
		delete (msg);
...
Rsession *Rconnection::detachedEval(const char *cmd, int *status) {
	Rmessage *msg = new Rmessage();
	Rmessage *cmdMessage =
			cmd ? new Rmessage(CMD_detachedVoidEval, cmd) : new Rmessage(
			CMD_detachSession);
	int res = request(msg, cmdMessage);
	delete cmdMessage;
	if (res) {
	...
	if (msg->pars != 2 ||
	PAR_TYPE(ptoi(msg->par[0][0])) != DT_INT
			|| PAR_LEN(ptoi(msg->par[0][0])) != sizeof(int) ||
			PAR_TYPE(ptoi(msg->par[1][0])) != DT_BYTESTREAM
			|| PAR_LEN(ptoi(msg->par[1][0])) != 32) { // invalid contents
		if (status)
			*status = -12;
		delete msg;
		return 0;
	}
	Rsession *session = new Rsession(host, ptoi(msg->par[0][1]),
			(const char*) (msg->par[1] + 1));
	delete msg;
	if (status)
		*status = 0;
	return session;
