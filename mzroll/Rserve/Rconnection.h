...
#if defined __GNUC__ && !defined unix && !defined Win32 && !defined WIN32 && !defined Q_WIN32 && !defined Q_WIN64
#define unix
#endif
...
public:
	...
	int command() {
		return complete ? head.cmd : -1;
	}
	Rsize_t length() {
		return complete ? head.len : -1;
	}
	int is_complete() {
		return complete;
	}
...
public:
	...
	virtual Rsize_t storageSize() {
		return len + ((len > 0x7fffff) ? 8 : 4);
	}
...
public:
	Rinteger(Rmessage *msg) :
			Rexp(msg) {
		fix_content();
	}
	Rinteger(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		fix_content();
	}
	Rinteger(int *array, int count) :
			Rexp(XT_ARRAY_INT, (char*) array, count * sizeof(int)) {
		fix_content();
	}
	...
	int intAt(int pos) {
		return (pos >= 0 && (unsigned) pos < len / 4) ? ((int*) data)[pos] : 0;
	}
	virtual Rsize_t length() {
		return len / 4;
	}
...
public:
	Rdouble(Rmessage *msg) :
			Rexp(msg) {
		fix_content();
	}
	Rdouble(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		fix_content();
	}
	Rdouble(double *array, int count) :
			Rexp(XT_ARRAY_DOUBLE, (char*) array, count * sizeof(double)) {
		fix_content();
	}
	...
	double doubleAt(int pos) {
		return (pos >= 0 && (unsigned) pos < len / 8) ?
				((double*) data)[pos] : 0;
	}
	virtual Rsize_t length() {
		return len / 8;
...
public:
	Rsymbol(Rmessage *msg) :
			Rexp(msg) {
		name = "";
		fix_content();
	}

	Rsymbol(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		name = "";
		fix_content();
	}
...
class Rstrings: public Rexp {
	...
	unsigned int nel;
public:
	Rstrings(Rmessage *msg) :
			Rexp(msg) {
		decode();
	}
	Rstrings(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		decode();
	}
...
public:
	Rstring(Rmessage *msg) :
			Rexp(msg) {
	}
	Rstring(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
	}
	Rstring(const char *str) :
			Rexp(XT_STR, str, strlen(str) + 1) {
	}
...
public:
	...
	Rlist(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		head = tag = 0;
		tail = 0;
		fix_content();
	...
	Rlist(int type, Rexp *head, Rexp *tag, char *next, Rmessage *imsg) :
			Rexp(type, 0, 0, 0) {
		this->head = head;
		this->tag = tag;
		tail = 0;
	...
	Rexp *entryByTagName(const char *tagName) {
		if (tag && (tag->type == XT_SYM || tag->type == XT_SYMNAME)
				&& !strcmp(((Rsymbol*) tag)->symbolName(), tagName))
			return head;
		if (tail)
			return tail->entryByTagName(tagName);
		return 0;
	}
...
public:
	...
	Rvector(unsigned int *ipos, Rmessage *imsg) :
			Rexp(ipos, imsg) {
		cont = 0;
		count = 0;
		strs = 0;
	...
	char *stringAt(int i) {
		if (i < 0 || i > count || !cont[i] || cont[i]->type != XT_STR)
			return 0;
