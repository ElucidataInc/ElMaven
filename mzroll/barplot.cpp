...
void BarPlot::switchQValue() {
	...
	PeakGroup* g = NULL;
	if (_mw != NULL && _mw->getEicWidget())
		g = _mw->getEicWidget()->getSelectedGroup();
	if (g != NULL) {
		setPeakGroup(g);
		scene()->update();
	}
...
void BarPlot::setPeakGroup(PeakGroup* group) {
	...
	vector<float> yvalues = group->getOrderedIntensityVector(vsamples,
			BarPlot::qtype);
	...
	for (int i = 0; i < vsamples.size(); i++) {
		...
		if (sample->getNormalizationConstant() != 1.0)
			sampleName = QString::number(sample->getNormalizationConstant(),
					'f', 2) + "x " + sampleName;
...
void BarPlot::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
		QWidget *) {
	...
	for (int i = 0; i < _yvalues.size(); i++) {
		if (_yvalues[i] > maxYvalue)
			maxYvalue = _yvalues[i];
	}
	if (maxYvalue == 0)
		return;
	...
	for (int i = 0; i < _yvalues.size(); i++) {
		...
		if (posY + _barwidth > _height)
			_height = posY + _barwidth + barSpacer;
	}
