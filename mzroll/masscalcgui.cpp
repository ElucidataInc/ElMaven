...
massCalcGui::massCalcGui(MainWindow* mw) {
	setupUi(this);
	_mw = mw;
	_mz = 0;
	setCharge(-1);
	...
	connect(mTable, SIGNAL(currentCellChanged(int,int,int,int)),
			SLOT(showCellInfo(int,int,int,int)));
}

void massCalcGui::setMass(float mz) {
	if (mz == _mz)
		return;
...
void massCalcGui::setCharge(float charge) {

	ionization->setValue(charge);
	_charge = charge;
}
void massCalcGui::setPPM(float diff) {
	maxppmdiff->setValue(diff);
	_ppm = diff;
}

void massCalcGui::compute() {
	bool isDouble = false;
...
void massCalcGui::showTable() {
	...
	p->setHorizontalHeaderLabels(
			QStringList() << "Formula" << "Compound" << "Mass" << "ppmDiff"
					<< "DB");
...
void massCalcGui::setupSortedCompoundsDB() {
...
QSet<Compound*> massCalcGui::findMathchingCompounds(float mz, float ppm,
		float charge) {
	if (sortedcompounds.size() != DB.compoundsDB.size()) {
	...
	vector<Compound*>::iterator itr = lower_bound(sortedcompounds.begin(),
			sortedcompounds.end(), &x, Compound::compMass);
	...
	for (; itr != sortedcompounds.end(); itr++) {
		...
		if (mzUtils::ppmDist((double) cmass, (double) mz) < ppm
				&& !uniqset.contains(c))
			uniqset << c;
		if (cmass > mz + 2)
			break;
...
void massCalcGui::pubChemLink(QString formula) {
	_mw->setStatusText("Searhing pubchem");
	QString requestStr(
			tr(
					"http://pubchem.ncbi.nlm.nih.gov/search/search.cgi?cmd=search&q_type=mf&q_data=%1&simp_schtp=mf").arg(
					formula));

	_mw->setUrl(requestStr);
}

void massCalcGui::keggLink(QString formula) {
	_mw->setStatusText("Searhing Kegg");
	QString requestStr(
			tr(
					"http://www.genome.jp/ligand-bin/txtsearch?column=formula&query=%1&DATABASE=compound").arg(
					formula));
	_mw->setUrl(requestStr);
}

void massCalcGui::showCellInfo(int row, int col, int lrow, int lcol) {
