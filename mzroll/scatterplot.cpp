...
ScatterPlot::ScatterPlot(QWidget* w) :
		PlotDockWidget(w, 0) {
	...
	connect(compareSamplesDialog, SIGNAL(setProgressBar(QString,int,int)),
			(MainWindow*) w, SLOT(setProgressBar(QString,int,int)));
	...
	connect(scene(), SIGNAL(selectArea(QPointF, QPointF)), this,
			SLOT(showSelectedGroups(QPointF,QPointF)));
...
void ScatterPlot::showSelectedGroups(QPointF from, QPointF to) {
	...
	if (similar.size() > 0) {
		...
		while (i.hasNext()) {
			...
			mzSlice* slice = new mzSlice(groupX->minMz, groupX->maxMz,
					groupX->minRt - 2, groupX->maxRt + 2);
...
void ScatterPlot::drawScatter(StatisticsVector<float> vecA,
		StatisticsVector<float> vecB, vector<PeakGroup*> groups) {
	...
	for (int i = 0; i < vecA.size(); i++) {
		...
		R = (1 - 1.0 / R) * 15;
		if (group->compound) {
			qDebug() << QString(group->compound->name.c_str()) << "\t"
					<< pos.x() << " " << pos.y() << r << " " << R;
		}

		if (R < 1)
			R = 10;
		QGraphicsItem* item = scene()->addEllipse(pos.x() - R / 2,
				pos.y() - R / 2, R, R, pen, brush);
		...
		if (group) {
			...
			item->setToolTip(
					tr("m/z:%1 rt:%2 pvalue:%3").arg(group->meanMz).arg(
							group->meanRt).arg(group->changePValue));
		}
	}
}
...
void ScatterPlot::draw() {
	...
	for (int i = 0; i < allgroups.size(); i++) {
		...
		if (group->changePValue > _maxPvalue)
			continue;
		if (group->maxIntensity < _minIntensity)
			continue;
		if (group->changeFoldRatio < _minRatio)
			continue;
		if (group->goodPeakCount < _minGoodSamples)
			continue;
		...
		for (int s = 0; s < set1.size(); s++) {
			for (int j = 0; j < group->peaks.size(); j++) {
				if (group->peaks[j].getSample() == set1[s])
					groupA.push_back(group->peaks[j].peakAreaTop);
			}
		}

		for (int s = 0; s < set2.size(); s++) {
			for (int j = 0; j < group->peaks.size(); j++) {
				if (group->peaks[j].getSample() == set2[s])
					groupB.push_back(group->peaks[j].peakAreaTop);
			}
		}
...
void ScatterPlot::showSimilar(PeakGroup* group) {
	if (!showSimilarOptions->isChecked())
		return;
	if (!_table)
		return;
	...
	for (int i = 0; i < group->peakCount(); i++) {
		...
		if (group->peaks[i].peakIntensity > maxIntensity)
	...
	QSet<PeakGroup*> similar;
	vector<float> peakIntensityA = group->getOrderedIntensityVector(sampleSet,
			PeakGroup::AreaTop);
		...
		if (group->metaGroupId) {
			...
			if (abs(allgroups[i]->meanRt - group->meanRt) < maxRtDiff) {
				vector<float> peakIntensityB =
						allgroups[i]->getOrderedIntensityVector(sampleSet,
								PeakGroup::AreaTop);
				if (correlation(peakIntensityA, peakIntensityB)
						> minSampleCorrelation) {
					if (largestSample->correlation(group->meanMz,
							allgroups[i]->meanMz, ppm, group->minRt,
							group->maxRt) > minRtCorrelation) {
						similar.insert(allgroups[i]);
					}
				}
			}
