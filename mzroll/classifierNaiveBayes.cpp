...
void ClassifierNaiveBayes::saveModel(string filename) {
	...
	for (unsigned int i = 0; i < FEATURES.size(); i++ ) {
		...
		for (unsigned int j = 0; j < FEATURES[i].size(); j++ )   {
			fout << FEATURES[i][j]; if (j + 1 < num_features) fout << ",";
		}
		fout 	<<  endl;
	}
	fout.close();
...
void ClassifierNaiveBayes::loadModel(string filename) {
	...
	while ( getline(myfile, line) ) {
		...
		if (label == 'g' || label == 'b' ) {
			if (num_features >= fields.size() ) continue;
...
void ClassifierNaiveBayes::classify(Peak&p) {
	...
	int Gcount = 0; int Bcount = 0;
	for (unsigned int i = 0; i < labels.size(); i++ ) {
		if (labels[i] == 'g' )  Gcount++;
		else if (labels[i] == 'b')   Bcount++;
	}
	if (Gcount == 0 || Bcount == 0) return;

	cerr << "Gcount=" << Gcount << " Bcount=" << Bcount << " LABEL=" << p.label << endl;
	p.quality = 0.5;

	for (unsigned int jj = 0; jj < num_features; jj++ ) {
		float rssG = 0; float rssB = 0;
		StatisticsVector<float>distG;
		StatisticsVector<float>distB;

		for (unsigned int ii = 0; ii < labels.size(); ii++ )
			if ( labels[ii] == 'g' ) {  float rss = POW2(A[jj] - FEATURES[ii][jj]); distG.push_back(rss); rssG += rss; }

		for (unsigned int ii = 0; ii < labels.size(); ii++ )
			if ( labels[ii] == 'b' ) {  float rss = POW2(A[jj] - FEATURES[ii][jj]); distB.push_back(rss); rssB += rss; }

		rssB = distB.mean();
		rssG = distG.mean();
		if (rssG == 0 && rssB == 0) continue;
		if (rssG < rssB) if (rssG == 0) rssG = 1 / 10 * rssB;
		float w = 0.3 + 1 / (1 + exp(-1 * rssB / rssG));
		p.quality *= w;
		cerr << features_names[jj] << "=" << A[jj] << "\trss(b,g)=" << rssB << " " << rssG << "\tw=" << w << "\tq=" << p.quality << endl;
	}

	if (p.quality > 1.0) p.quality = 1;
	if (p.quality < 0.0) p.quality = 0;
...
void ClassifierNaiveBayes::refineModel(PeakGroup* grp) {
	...
	if (grp->label == 'g' || grp->label == 'b' ) {
		for (unsigned int j = 0; j < grp->peaks.size(); j++ ) {
			Peak& p = grp->peaks[j];
			p.label = grp->label;
			if ( p.width < 2 || p.signalBaselineRatio <= 1 )  p.label = 'b';
...
void ClassifierNaiveBayes::findOptimalSplit(vector<Peak*>&peaks, int fNum) {
	...
	for (float cut = minX; cut < maxX; cut += step ) {
		map<char, float>counts; float pG = 0; float pB = 0;
		for (int i = 0; i < X.size(); i++ )  if (X[i] < cut )  counts[labels[i]]++;
		if (totals.count('g') ) pG = counts['g'] / totals['g'];
		if (totals.count('b') ) pB = 1.0 - (counts['b'] / totals['b']);
		cerr << setprecision(2) << "CUT=" << cut << " pG=" << pG << " pB=" << pB << " GAIN=" << 1 - (pG * pG + pB * pB) << endl;
	}
}
