...
SettingsForm::SettingsForm(QSettings* s, MainWindow *w) :
		QDialog(w) {
	setupUi(this);
	settings = s;
	mainwindow = w;
	...
	connect(eic_smoothingWindow, SIGNAL(valueChanged(int)),
			SLOT(recomputeEIC()));
	connect(eic_smoothingAlgorithm, SIGNAL(currentIndexChanged(int)),
			SLOT(recomputeEIC()));
	connect(grouping_maxRtWindow, SIGNAL(valueChanged(double)),
			SLOT(recomputeEIC()));
	connect(baseline_smoothing, SIGNAL(valueChanged(int)),
			SLOT(recomputeEIC()));
	connect(baseline_quantile, SIGNAL(valueChanged(int)), SLOT(recomputeEIC()));

	connect(ionizationMode, SIGNAL(currentIndexChanged(int)),
			SLOT(getFormValues()));
	connect(minRtCorrelation, SIGNAL(valueChanged(double)),
			SLOT(getFormValues()));
	connect(minSampleCorrelation, SIGNAL(valueChanged(double)),
			SLOT(getFormValues()));
	...
	connect(isotopeC13Correction, SIGNAL(toggled(bool)),
			SLOT(recomputeIsotopes()));

	connect(maxNaturalAbundanceErr, SIGNAL(valueChanged(double)),
			SLOT(recomputeIsotopes()));
	connect(minIsotopicCorrelation, SIGNAL(valueChanged(double)),
			SLOT(recomputeIsotopes()));
	connect(maxIsotopeScanDiff, SIGNAL(valueChanged(int)),
			SLOT(recomputeIsotopes()));

	connect(notes_server_url, SIGNAL(textChanged(QString)),
			SLOT(getFormValues()));
	connect(search_server_url, SIGNAL(textChanged(QString)),
			SLOT(getFormValues()));
	connect(pathway_server_url, SIGNAL(textChanged(QString)),
			SLOT(getFormValues()));

	connect(centroid_scan_flag, SIGNAL(toggled(bool)), SLOT(getFormValues()));
	connect(scan_filter_min_quantile, SIGNAL(valueChanged(int)),
			SLOT(getFormValues()));
	connect(scan_filter_min_intensity, SIGNAL(valueChanged(int)),
			SLOT(getFormValues()));
...
void SettingsForm::setFormValues() {
	...
	grouping_maxRtWindow->setValue(
			settings->value("grouping_maxRtWindow").toDouble());
	maxNaturalAbundanceErr->setValue(
			settings->value("maxNaturalAbundanceErr").toDouble());
	maxIsotopeScanDiff->setValue(
			settings->value("maxIsotopeScanDiff").toDouble());
	minIsotopicCorrelation->setValue(
			settings->value("minIsotopicCorrelation").toDouble());
	minRtCorrelation->setValue(settings->value("minRtCorrelation").toDouble());
	minSampleCorrelation->setValue(
			settings->value("minSampleCorrelation").toDouble());
	maxRtDiff->setValue(settings->value("maxRtDiff").toDouble());
	baseline_smoothing->setValue(
			settings->value("baseline_smoothing").toDouble());
	baseline_quantile->setValue(
			settings->value("baseline_quantile").toDouble());

	C13Labeled->setCheckState(
			(Qt::CheckState) settings->value("C13Labeled").toInt());
	N15Labeled->setCheckState(
			(Qt::CheckState) settings->value("N15Labeled").toInt());
	S34Labeled->setCheckState(
			(Qt::CheckState) settings->value("S34Labeled").toInt());
	D2Labeled->setCheckState(
			(Qt::CheckState) settings->value("D2Labeled").toInt());
	isotopeC13Correction->setCheckState(
			(Qt::CheckState) settings->value("isotopeC13Correction").toInt());

	centroid_scan_flag->setCheckState(
			(Qt::CheckState) settings->value("centroid_scan_flag").toInt());
	scan_filter_min_intensity->setValue(
			settings->value("scan_filter_min_intensity").toInt());
	scan_filter_min_quantile->setValue(
			settings->value("scan_filter_min_quantile").toInt());

	if (settings->contains("notes_server_url"))
		notes_server_url->setText(
				settings->value("notes_server_url").toString());

	if (settings->contains("pathway_server_url"))
		pathway_server_url->setText(
				settings->value("pathway_server_url").toString());

	if (settings->contains("search_server_url"))
		search_server_url->setText(
				settings->value("search_server_url").toString());
...
void SettingsForm::getFormValues() {
	...
	settings->setValue("maxNaturalAbundanceErr",
			maxNaturalAbundanceErr->value());
	settings->setValue("maxIsotopeScanDiff", maxIsotopeScanDiff->value());
	settings->setValue("minIsotopicCorrelation",
			minIsotopicCorrelation->value());
	...
	settings->setValue("isotopeC13Correction",
			isotopeC13Correction->checkState());
	...
	settings->setValue("scan_filter_min_intensity",
			scan_filter_min_intensity->value());
	settings->setValue("scan_filter_min_quantile",
			scan_filter_min_quantile->value());
	...
	if (mainwindow)
		mainwindow->setIonizationMode(mode);

	settings->setValue("ionizationMode", mode);

	mzSample::setFilter_centroidScans(
			centroid_scan_flag->checkState() == Qt::Checked);
