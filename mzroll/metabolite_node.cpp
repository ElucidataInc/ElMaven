...
MetaboliteNode::MetaboliteNode(QGraphicsItem* parent, QGraphicsScene *scene) :
		Node(parent, scene) {
...
void MetaboliteNode::paint(QPainter *painter, const QStyleOptionGraphicsItem*,
		QWidget *) {
...
void MetaboliteNode::addBarPlot() {
	...
	if (!_barplot) {
		_barplot = new BarPlot(this, scene());
	}

	if (_barplot) {
		PathwayWidget* parentView =
				(PathwayWidget*) (this->scene()->views().first());
...
void MetaboliteNode::paintCarbonBalls(QPainter *painter) {
	...
	if (atomCount > 4)
		scale += atomCount / 4 * atomSize;

	QColor carbon = QColor(30, 30, 30);
	carbon.setAlphaF(0.9);
	for (int i = 0; i < atomCount; i++) {
		...
		if (atom.startsWith("C")) {
			painter->setBrush(carbon);
		} else if (atom.startsWith("O")) {
			painter->setBrush(Qt::darkRed);
		} else if (atom.startsWith("N")) {
			painter->setBrush(Qt::darkBlue);
		} else if (atom.startsWith("P")) {
			painter->setBrush(Qt::darkCyan);
		} else if (atom.startsWith("S")) {
...
void MetaboliteNode::paintMetabolite(QPainter *painter) {
	...
	if (c->hasGroup() && getConcentration() > 0)
		color2 = Qt::blue;
	...
	QRadialGradient gradient(QPointF(-circleSize / 3, -circleSize / 3),
			circleSize);
	...
	painter->setPen(pen);
	painter->drawEllipse(-circleSize / 2, -circleSize / 2, circleSize,
			circleSize);
	...
	if (fractionLabeld > 0) {
		...
		painter->drawPie(-circleSize / 2, -circleSize / 2, circleSize,
				circleSize, 0, pieSize * 360 * 16);
	}
...
void MetaboliteNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event) {
	...
	for (int i = 0; i < atomMap.size(); i++) {
		...
		if (abs(xdist) < _defaultAtomSize / 2
				&& abs(ydist) < _defaultAtomSize / 2
				&& sqrt(xdist * xdist + ydist * ydist) < dist) {
			_selectedAtom = i;
		}
	}

	if (_selectedAtom >= 0) {
		qDebug() << "atom=" << coordinates->atoms[_selectedAtom] << " atom# "
				<< _selectedAtom;
	}

	if (_selectedAtom >= 0)
		emit(atomSelected(c, _selectedAtom));
	//expandOnCompound();
}
...
void MetaboliteNode::contextMenuEvent(QGraphicsSceneContextMenuEvent * event) {
	...
	connect(a1, SIGNAL(triggered()), getGraphWidget(),
			SLOT(removeSelectedNodes()));
