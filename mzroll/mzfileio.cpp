...
void mzFileIO::setMainWindow(MainWindow* mw) {
	_mainwindow = mw;
	connect(this, SIGNAL(updateProgressBar(QString,int,int)), _mainwindow,
			SLOT(setProgressBar(QString, int,int)));
	connect(this, SIGNAL(finished()), _mainwindow, SLOT(setupSampleColors()));
	connect(this, SIGNAL(finished()), _mainwindow->projectDockWidget,
	...
	do {
		...
		if (line.startsWith("Name:", Qt::CaseInsensitive) && !name.isEmpty()) {
			if (!name.isEmpty()) { //insert new compound
				...
				Compound* cpd = new Compound(name.toStdString(),
						name.toStdString(), formula.toStdString(), charge);
	...
	while (!xml.atEnd()) {
		...
		if (xml.isStartElement()) {
			...
			if (xml.name() == "spectrum") {
				...
				if (_name.contains("TimeInMinutes", Qt::CaseInsensitive))
					currentScan->rt = _value.toFloat();
				else if (_name.contains("time in seconds", Qt::CaseInsensitive))
					currentScan->rt = _value.toFloat();
				...
				currentScan->mslevel =
						xml.attributes().value("msLevel").toString().toInt();
				if (currentScan->mslevel <= 0)
					currentScan->mslevel = 1;
			} else if (xml.name() == "data" && taglist.size() >= 2
					&& currentScan) {
				int precision =
						xml.attributes().value("precision").toString().toInt();
...
void mzFileIO::fileImport(void) {
	if (filelist.size() == 0)
		return;
	emit (updateProgressBar("Importing files", filelist.size() + 0.01,
			filelist.size()));

	for (int i = 0; i < filelist.size(); i++) {
		QString filename = filelist.at(i);
		emit (updateProgressBar(tr("Importing file %1").arg(filename), i + 1,
				filelist.size()));
