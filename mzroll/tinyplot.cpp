...
TinyPlot::TinyPlot(QGraphicsItem* parent, QGraphicsScene *scene) :
...
QPointF TinyPlot::mapToPlot(float x, float y) {
	...
	if (_maxYValue == 0 && _minYValue == 0)
		return QPointF(xorigin, yorigin);
	float px = xorigin
			+ ((x - _minXValue) / (_maxXValue - _minXValue)) * _width;
	float py = yorigin
			- ((y - _minYValue) / (_maxYValue - _minYValue)) * _height;
...
void TinyPlot::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
		QWidget *) {
	...
	for (int i = 0; i < nSeries; i++) {
		for (int j = 0; j < data[i].size(); j++) {
			if (data[i][j].y() > _maxYValue) {
				_maxYValue = data[i][j].y() * 1.2;
			}
			if (data[i][j].y() < _minYValue) {
				_minYValue = data[i][j].y() * 0.8;
			}
			if (data[i][j].x() > _maxXValue) {
				_maxXValue = data[i][j].x() * 1.2;
			}
			if (data[i][j].x() < _minXValue) {
				_minXValue = data[i][j].x() * 0.8;
			}
		}
	}
	...
	if (!_title.isEmpty()) {
		...
		float _fontH = ((float) _width) / 30;

		if (_fontH > 3) {
			QFont fontSmall("Helvetica", _fontH);
			painter->setFont(fontSmall);
			painter->drawText(0, _fontH + 1, _title);
		}
	}

	int nColors = colors.size();
	if (nColors == 0)
		colors << Qt::blue << Qt::red << Qt::yellow << Qt::green;
	...
	for (int i = 0; i < points.size(); i++) {
		...
		painter->drawEllipse(mapToPlot(points[i].x(), points[i].y()),
