...
void AdductWidget::addToolBar() {
	...
	connect(btnTextZoomIn, SIGNAL(clicked()), _graph,
	...
	connect(btnTextZoomOut, SIGNAL(clicked()), _graph,
	...
	connect(btnNodeZoomOut, SIGNAL(clicked()), _graph,
	...
	connect(btnEdgeZoomOut, SIGNAL(clicked()), _graph,
...
void AdductWidget::addLinks(float centerMz, int recursionLevel) {
	...
	if (_mw->getIonizationMode())
		ionizationMode = _mw->getIonizationMode(); //user specified ionization mode
	...
	for (int c = 2; c < 4; c++) {
		if (chargeCounts[c] > chargeCounts[maxLikeCharge]) {
			maxLikeCharge = c;
		}
	}
	cerr << "addLinks: charge Count" << chargeCounts[0] << " "
			<< chargeCounts[1] << " " << chargeCounts[2] << endl;
	...
	for (int i = 0; i < DB.fragmentsDB.size(); i++) {
		...
		float mzPlus = centerMz + frag->mass;
		{
			...
			mzLink* l = checkConnection(centerMz, mzPlus,
...

		{
			QString noteText = tr("%1 Fragment").arg(
					QString(DB.fragmentsDB[i]->name.c_str()));
			mzLink* l = checkConnection(mzMinus, centerMz,
					noteText.toStdString());
			if (l)
				newMzs << mzMinus;
		}
	...
	for (int i = 0; i < DB.adductsDB.size(); i++) {
		...
		if (abs(parentMass - centerMz) > 0.1 && scan->hasMz(parentMass, ppm)) {
			...
			float correlation = sample->correlation(centerMz, parentMass, 5,
					scan->rt - 1, scan->rt + 1);
			float parentIntensity = getIntensity(parentMass, ppm);

			if (correlation > 0.3 && !linkExists(centerMz, parentMass, 5)
					&& parentIntensity > intensity1) {
				mzLink* l = new mzLink(parentMass, centerMz,
						noteText.toStdString());
	...
	for (int i = 0; i < DB.adductsDB.size(); i++) {
		...
		if (abs(adductMass - centerMz) > 0.1 && scan->hasMz(adductMass, ppm)) {
			QString noteText = tr("Adduct %1").arg(
					QString(DB.adductsDB[i]->name.c_str()));
			float correlation = sample->correlation(centerMz, adductMass, 5,
					scan->rt - 1, scan->rt + 1);
			float childIntensity = getIntensity(adductMass, ppm);

			if (correlation > 0.5 && !linkExists(adductMass, centerMz, 5)
					&& childIntensity < intensity1) {
				mzLink* l = new mzLink(centerMz, adductMass,
						noteText.toStdString());
				l->correlation = correlation;
				links.push_back(l);
				addLink(l);
...
mzLink* AdductWidget::checkConnection(float mz1, float mz2, string note) {
	...
	if (links.size() > 1) {
		for (int i = 0; i < links.size(); i++) {
			if (sample->correlation(links[i]->mz1, mz2, 5, _scan->rt - 1,
					_scan->rt + 1) < 0.0)
				return NULL;
			if (sample->correlation(links[i]->mz1, mz1, 5, _scan->rt - 1,
					_scan->rt + 1) < 0.0)
				return NULL;
		}
	}
	...
	if (_scan->hasMz(mz2, ppm)) {
		float correlation = sample->correlation(mz1, mz2, 5, _scan->rt - 1,
				_scan->rt + 1);
...
void AdductWidget::addLink(mzLink* l) {
	if (!l)
		return;
	if (l->correlation < 0.5)
		return;

	QString name1 = QString::number(l->mz1, 'f', 3);
	QString name2 = tr("%1 %2").arg(QString::number(l->mz2, 'f', 3),
			l->note.c_str());
	...
	if (n1 && n2 && !_graph->findEdge(n1, n2)) {
		QString note = tr("%1 %2").arg(QString(l->note.c_str()),
				QString::number(l->mz1 - l->mz2, 'f', 3));
