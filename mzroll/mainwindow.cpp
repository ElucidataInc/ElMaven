...
//qRegisterMetaTypeStreamOperators<UserNote*>("UserNote*");
	...
	if (settings->contains("dataDir"))
		dirs << settings->value("dataDir").value<QString>();
	...
	QString storageLocation = QDesktopServices::storageLocation(
			QDesktopServices::DataLocation);
	QString ligandDbFilename = dataDir + "/"
			+ settings->value("ligandDbFilename").value<QString>();
	QString chromoMethod = dataDir + "/"
			+ settings->value("chromoMethod").value<QString>();
	QString clsfModelFilename = dataDir + "/"
			+ settings->value("clsfModelFilename").value<QString>();
	QString notesDbFilename = storageLocation + "/"
			+ settings->value("notesDbFilename").value<QString>();

	QFile DBFILE(ligandDbFilename);
	if (DBFILE.exists()) {
		DB.connect(ligandDbFilename.toStdString());
		DB.loadFragments(
				QString(dataDir + "/" + "FRAGMENTS.csv").toStdString());
...
void MainWindow::open() {
	...
	QStringList filelist =
			QFileDialog::getOpenFileNames(this,
					...
					tr(
							"All Known Formats(*.mzroll *.mzPeaks *.mzXML *.mzxml *.mzdata *.mzData *.mzData.xml *.cdf *.nc);;")
	...
	setWindowTitle(
			programName + " " + programVersion + " " + fileInfo.fileName());
...
void MainWindow::readSettings() {
	...
	if (!settings->contains("chromoMethod"))
		settings->setValue("chromoMethod",
				QString("neg-Exactive-10cmC18-25min"));
...
void MainWindow::Align() {
	...
	pd.alignSamplesFlag = true;
	pd.keepFoundGroups = false;
	workerThread->start();
}
	...
	for (unsigned int i = 0; i < samples.size(); i++) {
		for (unsigned int j = 0; j < samples[i]->scans.size(); j++) {
			if (samples[i]->scans[j]->mslevel > 1
					&& ppmDist(samples[i]->scans[j]->precursorMz, pmz) < 10) {
