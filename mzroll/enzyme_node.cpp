...
EnzymeNode::EnzymeNode(QGraphicsItem* parent, QGraphicsScene *scene) :
		Node(parent, scene) {
...
void EnzymeNode::paint(QPainter *painter, const QStyleOptionGraphicsItem*,
		QWidget *) {
	paintEnzyme(painter);
}
...
float EnzymeNode::getFreeConcentration() {
	return _freeConcentration;
}
...
void EnzymeNode::paintEnzyme(QPainter *painter) {
	...
	if (isSelected() || isHighlighted() || labelIsVisible()) {
		...
		painter->drawEllipse(-circleSize / 2, -circleSize / 2, circleSize,
				circleSize);
		_shape.addEllipse(-circleSize / 2, -circleSize / 2, circleSize,
				circleSize);
	}
}
...
void EnzymeNode::hoverEnterEvent(QGraphicsSceneHoverEvent* event) {
	...
	if (e->sourceNode()) {
		e->sourceNode()->setHighlighted(true);
		e->sourceNode()->update();
	}
	if (e->destNode()) {
		e->destNode()->setHighlighted(true);
		e->destNode()->update();
	}
	e->update();
}
	...
	if (r) {
		...
		if (r->reversable)
			direction = " &lt;=&gt; ";
		QString reaction = reactants.join(" + ") + direction
				+ products.join(" + ");
		setToolTip(reaction);
	}
...
void EnzymeNode::hoverLeaveEvent(QGraphicsSceneHoverEvent* event) {
	...
	if (e->sourceNode()) {
		e->sourceNode()->setHighlighted(false);
		e->sourceNode()->update();
	}
	if (e->destNode()) {
		e->destNode()->setHighlighted(false);
		e->destNode()->update();
	}
	e->update();
}
...
void EnzymeNode::layoutCofactors() {
	...
	foreach (Edge* e, edges()){
	if ( e->sourceNode()->isCofactor() || e->destNode()->isCofactor()) {
		hasCofactors = true;
		continue;
	}
	if ( e->sourceNode() == this ) {
		angleFrom = e->angle();    //angle from Enzyme
	}
	if ( e->destNode() == this ) {
		angleTo = e->angle();    //angle to Enzyme
	}
}

//cerr << "angle --> E --> =" << angleTo << " " << angleFrom << endl;

	if (hasCofactors == false)
		return;
	foreach (Edge* e, edges()){
