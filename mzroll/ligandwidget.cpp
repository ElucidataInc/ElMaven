...
LigandWidget::LigandWidget(MainWindow* mw) {
	...
	connect(this, SIGNAL(compoundFocused(Compound*)), mw,
...
void LigandWidget::setDatabaseNames() {
	...
	connect(databaseSelect, SIGNAL(currentIndexChanged(QString)), this,
			SLOT(setDatabase(QString)));
}

QTreeWidgetItem* LigandWidget::addItem(QTreeWidgetItem* parentItem, string key,
		float value) {
...
void LigandWidget::showTable() {
	...
	if (!filterString.isEmpty() && !regexp.isValid())
		return;
	...
	for (unsigned int i = 0; i < DB.compoundsDB.size(); i++) {
		...
		if (!filterString.isEmpty()) {
			if (!(name.contains(regexp) || id.contains(regexp)
					|| formula.contains(regexp))) {
				if (items.count(compound) != 0)
					items[compound]->setHidden(true);
				continue;
			}
		...
		if (compound->category.size() > 0) {
			QString categoryName =
					QString(compound->category[0].c_str()).toLower();
		...
		if (items.count(compound) == 0) {
			...
			if (categoryParent) {
				parent = new NumericTreeWidgetItem(categoryParent,
						CompoundType);
			} else {
			...
			parent->setData(0, Qt::UserRole,
					QVariant(QString(compound->db.c_str())));
			parent->setData(1, Qt::UserRole,
					QVariant(QString(compound->id.c_str())));
			parent->setFlags(
...
void LigandWidget::saveCompoundList() {
	...
	QString fileName = QFileDialog::getSaveFileName(this,
			"Export Compounds to Filename", dbfilename, "CSV (*.csv)");

	if (fileName.isEmpty())
		return;
	if (!fileName.endsWith(".csv", Qt::CaseInsensitive))
		fileName = fileName + tr(".csv");
	...
	if (data.open(QFile::WriteOnly | QFile::Truncate)) {
		...
		out
				<< "polarity,compound,precursorMz,collisionEnergy,productMz,expectedRt,id,formula,srmId,category\n";
	...
	while (!xml.atEnd()) {
		...
		if (xml.isStartElement()) {
			...
			if (xml.name() == "item") {
				if (remoteCompound != NULL) {
					...
					if (!remoteCompound->id.empty()) {
						...
						qDebug() << "new remote compound..: "
								<< remoteCompound->id.c_str();
					}
				}
			}
			...
			if (remoteCompound == NULL) {
				...
				remoteCompound->pubchem_id =
						xml.text().toString().toStdString();
				...
				remoteCompound->collisionEnergy =
						xml.text().toString().toDouble();
