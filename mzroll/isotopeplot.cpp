...
void IsotopePlot::setPeakGroup(PeakGroup* group) {
	...
	if (isVisible() == true && group == _group)
		return;
	_group = group;
...
void IsotopePlot::showBars() {
	...
	for (int i = 0; i < MM.rows(); i++) {		//samples
		...
		for (int j = 0; j < MM.cols(); j++) {	//isotopes
			...
			QString name = tr("%1 <br> %2 <b>%3\%</b>").arg(
					_samples[i]->sampleName.c_str(),
			...
			if (_mw) {
				connect(rect, SIGNAL(groupSelected(PeakGroup*)), _mw,
						SLOT(setPeakGroup(PeakGroup*)));
				connect(rect, SIGNAL(groupUpdated(PeakGroup*)),
						_mw->pathwayWidget,
...
void IsotopePlot::contextMenuEvent(QContextMenuEvent * event) {
	...
	connect(d, SIGNAL(triggered()), settingsForm,
			SLOT(showIsotopeDetectionTab()));
...
void IsotopePlot::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
		QWidget *) {
	return;
}
