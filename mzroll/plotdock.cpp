...
PlotScene::PlotScene(QObject * parent) :
	...
	selectionRect->hide();
	selectionRect->setPen(QPen(Qt::DotLine));
	vline = new QGraphicsLineItem();
	vline->hide();
	vline->setPen(QPen(Qt::DotLine));
	hline = new QGraphicsLineItem();
	hline->hide();
	hline->setPen(QPen(Qt::DotLine));
	...
	xValueLabel = new Note("", hline, this);
	xValueLabel->setExpanded(true);
	xValueLabel->hide();
	yValueLabel = new Note("", vline, this);
	yValueLabel->setExpanded(true);
	yValueLabel->hide();
...
void PlotScene::clear() {
	if (vline && vline->scene() == this)
		removeItem(vline);
	if (hline && hline->scene() == this)
		removeItem(hline);
	if (ylabel && ylabel->scene() == this)
		removeItem(ylabel);
	if (xlabel && xlabel->scene() == this)
		removeItem(xlabel);
	if (xValueLabel && xValueLabel->scene() == this)
		removeItem(xValueLabel);
	if (yValueLabel && yValueLabel->scene() == this)
		removeItem(yValueLabel);
	if (selectionRect && selectionRect->scene() == this)
...
void PlotScene::showXLabel(QString text) {
	if (xlabel) {
		...
		xlabel->setPos(
				plotRect.width() / 2 - xlabel->boundingRect().width() / 2,
				height() - 10);
	}
}
...
void PlotScene::mouseMoveEvent(QGraphicsSceneMouseEvent * mouseEvent) {
	...
	if (vline)
		vline->setLine(p.x(), p.y(), p.x(), plotRect.height() * 1.05);
	if (hline)
		hline->setLine(0, p.y(), p.x(), p.y());
	QGraphicsScene::mouseMoveEvent(mouseEvent);

	if (_mousePressed && !_mouseReleased) {  //drag
		if (selectionRect->scene() != this)
		...
		if (w > 0 && h > 0) {
			...
			x.setTopRight(p);
			x.setBottomLeft(down);
		} else if (w < 0 && h > 0) {
			x.setTopRight(down);
			x.setBottomLeft(p);
		} else {
			x.setTopLeft(p);
			x.setBottomRight(down);
		}
...
QPointF PlotScene::mapToPlot(float px, float py) {
	...
	if (xDim.x() == 0 && xDim.y() == 0)
		return origin;
	if (yDim.x() == 0 && yDim.y() == 0)
		return origin;

	float X0 = zoomXDim.x();
	float X1 = zoomXDim.y();
	if (logX) {
		X0 > 0 ? X0 = log10(X0) : X0 = 1;
		X1 > 0 ? X1 = log10(X1) : X1 = 1;
	}

	float Y0 = zoomYDim.x();
	float Y1 = zoomYDim.y();
	if (logY) {
		Y0 > 0 ? Y0 = log10(Y0) : Y0 = 1;
		Y1 > 0 ? Y1 = log10(Y1) : Y1 = 1;
	}
...
PlotDockWidget::PlotDockWidget(QWidget * parent = 0, Qt::WindowFlags flags = 0) :
	...
	connect(myScene, SIGNAL(selectionChanged()), this,
			SLOT(selectionChanged()));
	connect(myScene, SIGNAL(zoomArea(QPointF, QPointF)), this,
			SLOT(zoomArea(QPointF,QPointF)));
...
void PlotDockWidget::zoomArea(QPointF from, QPointF to) {
	...
	qDebug() << "PlotDockWidget: zoomArea" << scene()->getZoomXDim() << " "
			<< scene()->getZoomYDim();
}
...
void PlotDockWidget::drawAxes() {
	...
	PlotAxes* x = new PlotAxes(0, 5, scene());
	x->setZValue(0);
	x->setOffset(0);

	PlotAxes* y = new PlotAxes(1, 5, scene());
	y->setZValue(0);
	y->setOffset(0);
	y->showTicLines(false);
	y->setLogTransformed(scene()->logTrasformedY());
}
...
QRectF PlotAxes::boundingRect() const {
	...
	if (type == 0) {
		return (QRectF(0, scene()->height() - textmargin, scene()->width(),
				scene()->height() - textmargin));
	} else {
		return (QRectF(0, 0, +textmargin, scene()->height() + textmargin));
	}
}

void PlotAxes::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
		QWidget *) {
	...
	if (type == 0) {
		min = myscene->getZoomXDim().x();
		max = myscene->getZoomXDim().y();
	}
	if (type == 1) {
		min = myscene->getZoomYDim().x();
		max = myscene->getZoomYDim().y();
	}

	if (logTrasformed) {
		min > 0 ? min = log10(min) : min = 0;
		max > 0 ? max = log10(max) : max = 0;
		min = floor(min);
		if (min < 0)
			min = 1;
		max = ceil(max);
	...
	if (type == 0) { 	//X axes
		...
		for (int i = 0; i <= ticks; i++)
			painter->drawLine(x0 + ix * i, y0 - 5, x0 + ix * i, y0 + 5);

		for (int i = 0; i <= ticks; i++) {
			QString label = QString::number(min + b * i, 'f', 2);
			if (logTrasformed) {
				label = QString::number(pow(10, (min + b * i)), 'f', 0);
			}
		...
		for (int i = 0; i <= ticks; i++) {
			QString label = QString::number(min + b * i, 'g', 2);
			if (max < 10000) {
				label = QString::number(min + b * i, 'f', 1);
			}
			if (logTrasformed) {
				label = QString::number(pow(10, (min + b * i)), 'f', 0);
			}
