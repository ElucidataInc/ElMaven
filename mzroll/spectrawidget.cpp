...
void SpectraWidget::overlaySpectra(QList<double>mzs, QList<double>ints) {
    ...
    for(int i=0; i < mzs.size(); i++) {
        if ( mzs[i] < _minX  || mzs[i] > _maxX ) continue;

        int x = toX(mzs[i]+0.01);
        int y = toY(_maxY*0.75);
        if (i < ints.size()) y = toY(ints[i]);
...
void SpectraWidget::drawGraph() {
    ...
    for(unsigned int i=0; i < _items.size(); i++) {
        if(_items[i] != NULL) delete(_items[i]);
        _items[i]=NULL;
    }
    _items.clear();
...
void SpectraWidget::showScan(int increment ) {
    ...
    if (_lastScan && _currentScan && _lastScan->mslevel != _currentScan->mslevel) {
        resetZoom();
    }
}
