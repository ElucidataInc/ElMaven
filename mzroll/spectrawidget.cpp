...
void SpectraWidget::overlaySpectra(QList<double> mzs, QList<double> ints) {

	QPen bluepen(Qt::blue, 3);
	for (int i = 0; i < mzs.size(); i++) {
		if (mzs[i] < _minX || mzs[i] > _maxX)
			continue;
...
void SpectraWidget::drawGraph() {
	...
	QString title =
			tr("Sample:%1   Scan:%2   rt:%3   msLevel:%4  Ionization Mode:%5 ").arg(
					QString(sampleName.c_str()), QString::number(scan->scannum),
					QString::number(scan->rt, 'f', 2),
					QString::number(scan->mslevel), polarity);

	QColor sampleColor = QColor::fromRgbF(sample->color[0], sample->color[1],
			sample->color[2], 1);

	if (scan->precursorMz) {
		QString precursorMzLink = "PrecursorMz: "
				+ QString::number(scan->precursorMz, 'f', 3);
		title += precursorMzLink;
	}

	if (scan->productMz) {
		QString precursorMzLink = " ProductMz: "
				+ QString::number(scan->productMz, 'f', 3);
		title += precursorMzLink;
	}
	...
	for (int j = 0; j < scan->nobs(); j++) {
		...
		if (scan->intensity[j] / _maxY > 0.005) {
			...
			if (abs(scan->mz[j] - _focusedMz) < 0.002) {
				...
				line->setPen(sampleColor);
			}
		}
	}

	//show labels
	QMap<float, int>::iterator itr;
	int labelCount = 0;
	for (itr = shownPositions.end(); itr != shownPositions.begin(); itr--) {
		int pos = itr.value();
		if (pos < 0 || pos >= scan->mz.size())
			continue;

		//create label
		QGraphicsTextItem* text = new QGraphicsTextItem(
				QString::number(scan->mz[pos], 'f', 6), 0, 0);
		text->setFont(font);
		scene()->addItem(text);
	...
	for (int i = 0; i < links.size(); i++) {
		...
		//create label
		Note* text = new Note(
				QString(links[i].note.c_str()) + " "
						+ QString::number(links[i].mz2, 'f', 3));
		scene()->addItem(text);
		_items.push_back(text);
...
void SpectraWidget::findBounds(bool checkX, bool checkY) {
	//bounds
	if (_currentScan == NULL || _currentScan->mz.size() == 0)
		return;
...
void SpectraWidget::showScan(int increment) {
	...
	if (newScan->mslevel == 1) {
		setScan(newScan, _minX, _maxX);
	} else {
		setScan(newScan);
	}

	if (_lastScan && _currentScan
			&& _lastScan->mslevel != _currentScan->mslevel) {
		resetZoom();
	}
}
...
void SpectraWidget::mouseMoveEvent(QMouseEvent* event) {
	...
	if (nearestPos >= 0) {
		...
		drawArrow(_currentScan->mz[nearestPos],
				_currentScan->intensity[nearestPos], invX(pos.x()),
				invY(pos.y()));
...
int SpectraWidget::findNearestMz(QPointF pos) {
	...
	if (matches.size() > 0) {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			float d = sqrt(
			POW2(
					_currentScan->intensity[p]
					- ycoord) + POW2(_currentScan->mz[p] - mz));
			if (d < dist) {
				best = p;
				dist = d;
			}
...
void SpectraWidget::drawArrow(float mz1, float intensity1, float mz2,
		float intensity2) {
	int x1 = toX(mz1);
	...
	if (distance < 0)
		sign = "-";
	QString note = tr("%1 [%2%3]").arg(QString::number(mz1, 'f', 4), sign,
...
void SpectraWidget::spectraToClipboard() {
	...
	for (int i = 0; i < _currentScan->nobs(); i++) {
		clipboardText
				<< tr("%1\t%2").arg(
						QString::number(_currentScan->mz[i], 'f', 6)).arg(
						QString::number(_currentScan->intensity[i], 'f', 6));
	}
...
void SpectraWidget::gotoScan() {
	...
	int scanNumber = QInputDialog::getInt(this, "Go To Scan Number",
			"Enter Scan Number", curScanNum, 0, maxScanNum, 1, &ok, 0);
...
vector<mzLink> SpectraWidget::findLinks(float centerMz, Scan* scan, float ppm,
		int ionizationMode) {
	...
	for (int i = 0; i < DB.fragmentsDB.size(); i++) {
		...
		if (scan->hasMz(mzPlus, ppm)) {
			QString noteText = tr("%1 Fragment").arg(
					QString(DB.fragmentsDB[i]->name.c_str()));
	...
	for (int i = 0; i < DB.adductsDB.size(); i++) {
		if ( SIGN(DB.adductsDB[i]->charge) != SIGN(ionizationMode))
			continue;
		...
		if (abs(parentMass - centerMz) > 0.1 && scan->hasMz(parentMass, ppm)) {
			cerr << DB.adductsDB[i]->name << " " << DB.adductsDB[i]->charge
					<< " " << parentMass << endl;
			QString noteText = tr("Possible Parent %1").arg(
					QString(DB.adductsDB[i]->name.c_str()));
			links.push_back(
					mzLink(centerMz, parentMass, noteText.toStdString()));
		}
	}
	...
	for (int i = 0; i < DB.adductsDB.size(); i++) {
		...
		if (abs(adductMass - centerMz) > 0.1 && scan->hasMz(adductMass, ppm)) {
			QString noteText = tr("Adduct %1").arg(
					QString(DB.adductsDB[i]->name.c_str()));
			links.push_back(
					mzLink(centerMz, adductMass, noteText.toStdString()));
		}
	}
