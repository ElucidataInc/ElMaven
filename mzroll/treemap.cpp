...
//
void TreeMap::drawChildren(Item *parent, const double *surface, double h) {
	//qDebug() << "drawChildren() ";
	...
	// Scale factor
	const double sizePerSquarePixel = (double) parent->TmiGetSize()
	...
	while (head < parent->TmiGetChildrenCount()) {
		...
		const float height =
				horizontal ? remaining.height() : remaining.width();
		...
		if (sum < remainingSize)
			width = (float) ((double) sum / remainingSize * width);
		...
		for (int i = rowBegin; i < rowEnd; i++) {
			double begin = fBegin;
			double fraction = (double) (parent->TmiGetChild(i)->TmiGetSize())
					/ sum;
			double fEnd = fBegin + fraction * height;
			double end = fEnd;

			bool lastChild = (i == rowEnd - 1
					|| parent->TmiGetChild(i + 1)->TmiGetSize() == 0);

			if (lastChild) {
				// Use up the whole height
				end = (horizontal ?
						remaining.top() + height : remaining.left() + height);
			}
		...
		if (remaining.width() <= 0 || remaining.height() <= 0) {
			if (head < parent->TmiGetChildrenCount()) {
				parent->TmiGetChild(head)->TmiSetRectangle(
						QRectF(-1, -1, -1, -1));
			}
...
void TreeMap::recurseDrawGraph(Item *item, const QRectF& rc, bool asroot,
		const double *psurface, double h) {
	assert(rc.width() >= 0);
	assert(rc.height() >= 0);
...
void TreeMap::renderLeaf(Item *item, const double *surface) {
	...
	if (group)
		rect->setData(0, QVariant::fromValue(group));
