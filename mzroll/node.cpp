...
double Node::computeNodeSize(float concentration) {
	...
	} else if (_graph->getNodeSizeNormalization()
			== GraphWidget::AbsoluteSize) {

		if (concentration > 1)
			circleSize *= log2(concentration);

	} else if (_graph->getNodeSizeNormalization()
			== GraphWidget::PairwiseSize) {
	...
	if (area > totalArea / 10)
		circleSize = sqrt(totalArea / 10);
	if (circleSize < 1)
		circleSize = 0;
...
float Node::getFontSize() {
	...
	if (_graph) {
		...
		if (height > scene()->height() / 5)
			height = scene()->height() / 5;
	}

	if (_fontSize > 0)
		return _fontSize * scale;

	float fontSize = 0;
	if (isMetabolite())
		fontSize = 0.12 * height * scale;
	else if (isEnzyme())
		fontSize = 0.08 * height * scale;
	else
		fontSize = 0.10 * height * scale;

	if (fontSize > scene()->height() / 20)
		fontSize = scene()->height() / 20;
	if (fontSize < 1)
		fontSize = 0;
	return fontSize;
}
...
void Node::paint(QPainter *painter, const QStyleOptionGraphicsItem*,
	...
	painter->drawEllipse(-nodesize / 2 + 1, -nodesize / 2 + 1, nodesize,
			nodesize);
...
void Node::drawLabel() {
	...
	if (!_labelBox)
		_labelBox = new QGraphicsRectItem(this, scene());
