...
SuggestPopup::SuggestPopup(QLineEdit *parent) :
...
void SuggestPopup::doSearchCompounds(QString needle) {
	...
	if (!needle.isEmpty() && !regexp.isValid())
		return;
...
void SuggestPopup::doSearchPathways(QString needle) {
	...
	for (unsigned int i = 0; i < DB.pathwayDB.size(); i++) {
		...
		if (name.length() == 0)
			continue;
		if (!(regexp.indexIn(name) >= 0 || regexp.indexIn(id) >= 0))
			continue;
		//score
		float c1 = 0;
		float c2 = 0;
...
if (needle.isEmpty() || needle.length() < 3)
	return;
...
if (startsWithNumber.indexIn(needle) == 0)
	return;
...
foreach( QString name, scores.keys() ) {
	list.append(qMakePair(1/scores[name],name));
}
qSort(list);
...
int h = popup->sizeHintForRow(0)
		* (compoundMatches + pathwayMatches + historyMatches);
