...
QTreeWidgetItem* TreeDockWidget::addItem(QTreeWidgetItem* parentItem,
		string key, float value, int type = 0) {
...
QTreeWidgetItem* TreeDockWidget::addItem(QTreeWidgetItem* parentItem,
		string key, string value, int type = 0) {
	QTreeWidgetItem *item = new QTreeWidgetItem(parentItem, type);
	item->setText(0, QString(key.c_str()));
	if (!value.empty()) {
...
void TreeDockWidget::showInfo() {
	...
	if ( itemType == CompoundType ) {
		...
		if (scan) mainwindow->getSpectraWidget()->setScan(scan);
		...
		for (int i = 0; i < treeWidget->topLevelItemCount(); i++) {
			if (treeWidget->topLevelItem(i) == item) {
				...
				if (group->hasCompoundLink()
						&& group->getCompound()->getPeakGroup() == group) {
...
void TreeDockWidget::setInfo(deque<Pathway*>& pathways) {
	...
	for (int i = 0; i < pathways.size(); i++) {
		...
		parent->setData(0, Qt::UserRole,
				QVariant::fromValue(QString(p->id.c_str())));
	}
}
...
void TreeDockWidget::addScanItem(Scan* scan) {
	...
	item->setText(0, QString::number(scan->precursorMz));
	item->setData(0, Qt::UserRole, QVariant::fromValue(scan));
	item->setText(1, QString::number(scan->rt));
	item->setText(2, QString(scan->filterLine.c_str()));
...
QTreeWidgetItem* TreeDockWidget::addCompound(Compound* c,
	...
	if (c->hasGroup()) {
		...
		if (group != NULL)
			addPeakGroup(group, item);
	}
	return item;
}

QTreeWidgetItem* TreeDockWidget::addPeakGroup(PeakGroup* group,
		QTreeWidgetItem* parent) {
	if (group == NULL)
		return NULL;
	QTreeWidgetItem* item = NULL;
	...
	for (int i = 0; i < group->childCount(); i++)
		addPeakGroup(&(group->children[i]), item);
	return item;
}
...
void TreeDockWidget::itemToClipboard(QTreeWidgetItem* item,
		QString& clipboardtext) {
