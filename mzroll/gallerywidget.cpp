...
void GalleryWidget::addEicPlots(std::vector<Compound*>&compounds) {
	...
	for (unsigned int i = 0; i < compounds.size(); i++) {
		...
		if (plot)
			plot->setTitle(QString(c->name.c_str()));
		if (plot)
			plot->setData(0, QVariant::fromValue(c));
	}
	if (plotitems.size() > 0)
		replot();
}
...
void GalleryWidget::addEicPlots(std::vector<mzLink>&links) {
	...
	for (unsigned int i = 0; i < links.size(); i++) {
		...
		TinyPlot* plot = addEicPlot(slice);
		if (plot)
			plot->setTitle(QString(link.note.c_str()));
		if (plot)
			plot->setData(0, QVariant::fromValue(slice));
...
void GalleryWidget::addEicPlots(std::vector<mzSlice*>&slices) {
	...
	for (unsigned int i = 0; i < slices.size(); i++) {
		...
		TinyPlot* plot = addEicPlot(slice);
		if (plot)
			plot->setTitle(tr("%1-%2").arg(slice.mzmin).arg(slice.mzmax));
		if (plot)
			plot->setData(0, QVariant::fromValue(slice));
	}
	if (plotitems.size() > 0)
...
TinyPlot* GalleryWidget::addEicPlot(mzSlice& slice) {
	...
	vector<EIC*> eics = PeakDetector::pullEICs(&slice, samples,
			EicLoader::NoPeakDetection, smoothingWindow, smoothingAlgorithm,
			amuQ1, amuQ3);
...
TinyPlot* GalleryWidget::addEicPlot(std::vector<EIC*>& eics) {
	...
	for (int i = 0; i < eics.size(); i++) {
		...
		QColor color = QColor::fromRgbF(eic->sample->color[0],
				eic->sample->color[1], eic->sample->color[2], 1.0);
...
void GalleryWidget::wheelEvent(QWheelEvent *event) {
	...
	if (event->delta() > 0) {
		if (_boxW * 0.8 > 50 && _boxH * 0.8 > 50) {
			_boxW *= 0.8;
			_boxH *= 0.8;
			replot();
		}
	} else {
		if (_boxW * 1.2 < width() && _boxH * 1.2 < height()) {
			_boxW *= 1.2;
			_boxH *= 1.2;
			replot();
		}
