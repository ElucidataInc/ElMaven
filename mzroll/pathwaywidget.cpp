...
void PathwayWidget::updateCompoundConcentrations() {
	...
	for(int j=0; j < visibleSamplesCount; j++ ) {
		vl[j] = vl[j]-(vt[j]*cf[j]);
		if (vl[j]<0) vl[j]=0;
	}
...
void PathwayWidget::loadModelFile(QString filename) {
	...
	if ( v.size() > 0 ) {
		Node* n = locateNode(id);
		if (n) n->setLabeledConcentrations(v);
	}
}
	...
	if ( v.size() > 0 ) {
		EnzymeNode* n = (EnzymeNode*) locateNode(id);
		if (n) n->setFluxes(v);
	}
}
...
void PathwayWidget::saveModelFile(QString filename) {
	...
	if (node->isEnzyme() ) enzymes << node;
	else metabolites << node;
}
...
void PathwayWidget::clearSelectedAtoms() {
	foreach( Node* n, nodelist){
	if ((n->isMetabolite() || n->isCofactor()) ) {
		((MetaboliteNode*) n)->setSelectedAtom(-1);
	}
}
getReactionPairs();
}
