...
#include "../libmaven/MersenneTwister.h"
...
QString TableDockWidget::groupTagString(PeakGroup* group) {
	...
	if (!group->tagString.empty())
		tag += " | " + QString(group->tagString.c_str());
...
bool TableDockWidget::hasPeakGroup(PeakGroup* group) {
	for (int i = 0; i < allgroups.size(); i++) {
		...
		if ((double) std::abs(group->meanMz - allgroups[i].meanMz) < 1e-5
				&& (double) std::abs(group->meanRt - allgroups[i].meanRt)
						< 1e-5) {
			return true;
		}
	}
...
void TableDockWidget::showAllGroups() {
	...
	for (int i = 0; i < allgroups.size(); i++) {
		...
		if (metaGroupId && allgroups[i].meanMz > 0
				&& allgroups[i].peakCount() > 0) {
			if (!parents.contains(metaGroupId)) {
				parents[metaGroupId] = new QTreeWidgetItem(treeWidget);
				parents[metaGroupId]->setText(0,
						QString("MetaGroup ") + QString::number(metaGroupId));
				parents[metaGroupId]->setText(3,
						QString::number(allgroups[i].meanRt, 'f', 2));
...
void TableDockWidget::exportGroupsToSpreadsheet() {
	...
	QString fileName = QFileDialog::getSaveFileName(this,
			tr("Export Peaks Table to .csv File"), dir, "CSV File(*.csv)");
	if (fileName.isEmpty())
		return;
...
void TableDockWidget::updateStatus() {
	...
	QString title = tr("Group Validation Status: Good=%2 Bad=%3 Total=%1").arg(
			QString::number(totalCount), QString::number(goodCount),
...
float TableDockWidget::showAccuracy(vector<PeakGroup*>&groups) {
	...
	traindialog->show();
	_mainwindow->setStatusText(
			tr("Good Groups=%1 Bad Groups=%2 Accuracy=%3").arg(
					QString::number(gc), QString::number(bc),
...
void TableDockWidget::printPdfReport() {
	...
	QString fileName = QFileDialog::getSaveFileName(this,
			tr("Save Group Report a PDF File"), dir, tr("*.pdf"));
	if (fileName.isEmpty())
		return;
...
void TableDockWidget::saveModel() {
	QString fileName = QFileDialog::getSaveFileName(this,
			tr("Save Classification Model to a File"));
	if (fileName.isEmpty())
		return;
...
void TableDockWidget::writeGroupXML(QXmlStreamWriter& stream, PeakGroup* g) {
	...
	stream.writeAttribute("expectedRtDiff",
			QString::number(g->expectedRtDiff, 'f', 4));
	...
	stream.writeAttribute("changeFoldRatio",
			QString::number(g->changeFoldRatio, 'f', 4));
	...
	for (int j = 0; j < g->peaks.size(); j++) {
		...
		stream.writeAttribute("gaussFitSigma",
				QString::number(p.gaussFitSigma, 'f', 4));
		stream.writeAttribute("gaussFitR2",
				QString::number(p.gaussFitR2, 'f', 4));
...
PeakGroup* TableDockWidget::readGroupXML(QXmlStreamReader& xml,
	...
	g.expectedRtDiff =
			xml.attributes().value("expectedRtDiff").toString().toDouble();
	...
	g.changeFoldRatio =
			xml.attributes().value("changeFoldRatio").toString().toDouble();
	g.changePValue =
			xml.attributes().value("changePValue").toString().toDouble();
	...
	if (!compoundId.empty()) {
		...
		vector<Compound*> matches = DB.findSpeciesByName(compoundName,
				compoundDB);
		if (matches.size() > 0)
...
void TableDockWidget::savePeakTable() {
	...
	QString fileName = QFileDialog::getSaveFileName(this,
			tr("Save Peaks Table to a .mzPeaks File"), dir,
			"mzPeaks File (*.mzPeaks)");
	if (fileName.isEmpty())
		return;
...
void TableDockWidget::loadPeakTable() {
	...
	QString fileName = QFileDialog::getOpenFileName(this, "Load Saved Peaks",
			dir, "All Files(*.mzPeaks *.mzpeaks)");
	if (fileName.isEmpty())
		return;
...
void TableDockWidget::clusterGroups() {
	...
	double minSampleCorrelation = settings->value("minSampleCorrelation").value<
			double>();
	double minRtCorrelation =
			settings->value("minRtCorrelation").value<double>();
	double ppm = _mainwindow->getUserPPM();

	vector<mzSample*> samples = _mainwindow->getSamples();

	for (unsigned int i = 0; i < allgroups.size(); i++)
		allgroups[i].metaGroupId = 0;
	...
	for (unsigned int i = 0; i < allgroups.size(); i++) {
		...
		for (int i = 0; i < grp1.peakCount(); i++) {
			...
			if (grp1.peaks[i].peakIntensity > maxIntensity)
				largestSample = sample;
		}

		if (largestSample == NULL)
			continue;
		vector<float> peakIntensityA = grp1.getOrderedIntensityVector(samples,
				PeakGroup::AreaTop);

		for (unsigned int j = i + 1; j < allgroups.size(); j++) {
			PeakGroup& grp2 = allgroups[j];
			if (grp2.metaGroupId > 0)
				continue;
			...
			vector<float> peakIntensityB = grp2.getOrderedIntensityVector(
					samples, PeakGroup::AreaTop);
			float cor = correlation(peakIntensityA, peakIntensityB);
			if (cor < minSampleCorrelation)
				continue;

			float cor2 = largestSample->correlation(grp1.meanMz, grp2.meanMz,
					ppm, grp1.minRt, grp1.maxRt);
