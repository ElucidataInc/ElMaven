...
void MyScene::mouseMoveEvent(QGraphicsSceneMouseEvent * mouseEvent) {
	...
	if (_showArrow) {
		if (!_arrow) {
			...
			QPen redpen(Qt::red, 1, Qt::DashDotLine, Qt::RoundCap,
					Qt::RoundJoin);
			_arrow->setPen(redpen);
			addItem(_arrow);
		}
		_arrow->setLine(down.x(), down.y(), lastCursorPos.x(),
				lastCursorPos.y());
		_arrow->show();
...
void GraphWidget::drawBackground(QPainter *painter, const QRectF &rect) {
	...
	if (!_backgroundImage.isNull()) {
		painter->drawPixmap(-_backgroundImage.width() / 2,
				-_backgroundImage.height() / 2, _backgroundImage);
		/*
...
void GraphWidget::scaleView(qreal scaleFactor) {
	qreal factor = matrix().scale(scaleFactor, scaleFactor).mapRect(
...
void GraphWidget::resetZoom() {
	...
	if (_averageEdgeSize > 0) {
		scale = 50 / _averageEdgeSize;
	}
	W *= scale;
	H *= scale;
	if (W < 300)
	...
	if (!a) { //missing parent
		...
		if (layoutMap.contains(n) ) {
			...
			foreach (Edge* e, edges) {
				if (layoutMap.contains(e->sourceNode())) {
					a = e->sourceNode();
					break;
				}
				if (layoutMap.contains(e->destNode())) {
					a = e->destNode();
					break;
				}
			}
...
void GraphWidget::recursiveDepth(Node* n0, int depth) {
	...
	if (layoutMap.contains(n0)) {
		...
		if (layoutMap.contains(other)) {
			parent = layoutMap[other];
			n0 = other;
			break;
		}
	}
	...
	foreach(Node * n, newnodes){
	recursiveDepth(n, depth + 1);
}
...
void GraphWidget::computeAvgEdgeLength() {
	...
	if (lvector.size() > 0)
		_averageEdgeSize = median(lvector);
	return;
}
...
void GraphWidget::deepChildCount(QTreeWidgetItem* x, int* count) {
	...
	for (int i = 0; i < childcount; i++)
		deepChildCount(x->child(i), count);
	...
	for (int i = 0; i < childcount; i++) {
		...
		if (n) {
			...
			if (getLayoutAlgorithm() == FMMM) {
				...
				float angle = angle0
						+ (float) (i - half) / childcount * 0.9 * Pi;
...
void GraphWidget::layoutOGDF() {
	...
	if (metabolites.size() == 0)
		metabolites = getNodes(Node::Unassigned);
	...
	foreach(Node * n, metabolites){
	if (layoutMap.count(n) == 0) recursiveDepth(n, 0);
}

	QList<Node*> enzymes = getNodes(Node::Enzyme);
	foreach(Node * n, enzymes){
	if (layoutMap.count(n) == 0) recursiveDepth(n, 0);
}
...
void GraphWidget::dump() {
	...
	if (n->isVisible()) {
		...
		qDebug() << "\t Edge: ";
		foreach(Edge * e, n->edges()) {
			qDebug() << "\t\t:" << e->destNode()->getId() << " " << e->sourceNode()->getId();
		}
		foreach(Edge * e, n->edgesOut()) {
			qDebug() << "\t\tout:" << e->destNode()->getId();
		}
		foreach(Edge * e, n->edgesIn()) {
			qDebug() << "\t\tin:" << e->sourceNode()->getId();
		}
	}
}
}
...
void GraphWidget::adjustEnzymePositions() {
	...
	if ( enz->isEnzyme() ) {
		...
		foreach (Edge* e, enz->edges() ) {
			Node* met = e->sourceNode();
			if (met == enz) met = e->destNode();
...
void GraphWidget::adjustLayout() {
	...
	int W = scene()->width() * 1.5;
	int H = scene()->height() * 1.5;
	int A = W * H;
