...
EicPoint::EicPoint(float x, float y, Peak* peak, MainWindow* mw) {
	...
	connect(this, SIGNAL(peakSelected(Peak*)), mw->getEicWidget(),
			SLOT(showPeakArea(Peak*)));
	//mouse hover events
	connect(this, SIGNAL(peakGroupFocus(PeakGroup*)), mw->getEicWidget(),
			SLOT(setSelectedGroup(PeakGroup*)));
	connect(this, SIGNAL(peakGroupFocus(PeakGroup*)),
			mw->getEicWidget()->scene(), SLOT(update()));
...
void EicPoint::hoverEnterEvent(QGraphicsSceneHoverEvent*) {
	...
	setToolTip(
			"<b>  Sample: </b>" + QString(sampleName.c_str())
					+ "<br> <b>intensity: </b>"
					+ QString::number(_peak->peakIntensity)
					+ "<br> <b>area: </b>"
					+ QString::number(_peak->peakAreaCorrected)
					+ "<br> <b>rt: </b>" + QString::number(_peak->rt, 'f', 2)
					+ "<br> <b>scan#: </b>" + QString::number(_peak->scan)
					+ "<br> <b>m/z: </b>"
					+ QString::number(_peak->peakMz, 'f', 6)
					/*
...
void EicPoint::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
		QWidget *) {
	QPen pen = _pen;
	QBrush brush = _brush;
...
void EicPoint::setClipboardToGroup() {
	_mw->setClipboardToGroup(_group);
}
...
void EicPoint::contextMenuEvent(QGraphicsSceneMouseEvent* event) {
	...
	if (_group && _group->compound) {
		if (_group->isIsotope() == false
				&& !_group->compound->formula.empty()) {
			QAction* z = menu.addAction(
					"Copy Isotope Information to Clipboard");
	...
	connect(c2, SIGNAL(triggered()), _mw->getEicWidget(),
			SLOT(markGroupGood()));
