...
void ProjectDockWidget::changeSampleOrder() {
    ...
    for(int i=0; i < itemCount; i++ ) {
        ...
        if (item && item->type() == SampleType ) {
            ...
            sampleOrder++;
            qDebug() << "changeSampleOrder: " << sampleOrder << " " << sample->sampleName.c_str();
        }
    }
...
void ProjectDockWidget::setInfo(vector<mzSample*>&samples) {
    ...
    for(int i=0; i < samples.size(); i++ ) {
        ...
        parent->setText(2,QString::number(sample->getNormalizationConstant(),'f',2));
        parent->setFlags(Qt::ItemIsEditable|Qt::ItemIsSelectable|Qt::ItemIsDragEnabled|Qt::ItemIsUserCheckable|Qt::ItemIsEnabled);
        sample->isSelected  ? parent->setCheckState(0,Qt::Checked) : parent->setCheckState(0,Qt::Unchecked);
...
void ProjectDockWidget::loadProject(QString fileName) {
    ...
    pathlist << projectPath << "." << settings->value("lastDir").value<QString>();
