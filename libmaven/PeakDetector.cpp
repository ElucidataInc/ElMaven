...
void PeakDetector::processSlices(vector<mzSlice*>&slices, string setName) {
	...
	for (unsigned int s = 0; s < slices.size(); s++) {
		...
		vector<EIC*> eics = PeakDetector::pullEICs(slice, samples,
				EicLoader::PeakDetection, eic_smoothingWindow,
				eic_smoothingAlgorithm, amuQ1, amuQ3);
		...
		if (allgroups.size() > limitGroupCount)
			break;
//		if (stopped()) @Caution: This should be a part of BackgroundPeakUpdate() which is a child of QThread.
//			break;
		...
		if (showProgressFlag && s % 10 == 0) {
			...
			/* updateProgressBar is a part of GUI and it should not be here
			 emit(
...
//			emit(newPeakGroup(&allgroups[j]));
		...
		if (showProgressFlag && pullIsotopesFlag && j % 10 == 0) {
			/*
			 emit(
			 updateProgressBar("Calculating Isotopes", j,
			 allgroups.size()));
			 */
		}
	}
...
//	emit(updateProgressBar("Done", 1, 1));
	...
	qDebug() << "BackgroundPeakUpdate:processSlices() done. " << timer.elapsed()
			<< " sec.";
	//cleanup();
}
...
void PeakDetector::processCompounds(vector<Compound*> set, string setName) {
	if (set.size() == 0)
		return;
...
void PeakDetector::computePeaks() {
	if (compounds.size() == 0) {
		return;
	}
...
void PeakDetector::computeKnowsPeaks() {
...
void PeakDetector::findPeaksQQQ() {
	if (mainWindow == NULL)
		return;
