...
void MassSlices::algorithmA() {
	...
	for (int i = 0; i < samples.size(); i++) {
		for (int j = 0; j < samples[i]->scans.size(); j++) {
			...
			if (scan->filterLine.empty())
				continue;
			if (seen.count(scan->filterLine))
				continue;
...
void MassSlices::algorithmB(float userPPM, float minIntensity, int rtStep) {
	...
	float rtWindow = 2.0;
	if (samples.size() > 0)
		rtWindow = (samples[0]->getAverageFullScanTime() * rtStep);

	cerr << "#algorithmB:" << " PPM=" << userPPM << " rtWindow=" << rtWindow
			<< " rtStep=" << rtStep << endl;
		...
		for (int j = 0; j < samples[i]->scans.size(); j++) {
			...
			if (lastScan) {
				for (int k = 0; k < scan->nobs(); k++) {
					if (scan->intensity[k] < minIntensity)
						continue;
					float mz = scan->mz[k];
					...
					if (Z) {
						...
						if (Z->mzmin < mz - (mz / 1e6 * userPPM))
							Z->mzmin = mz - (mz / 1e6 * userPPM);
						if (Z->mzmax > mz + (mz / 1e6 * userPPM))
							Z->mzmax = mz + (mz / 1e6 * userPPM);
						Z->mz = (Z->mzmin + Z->mzmax) / 2;
						Z->rt = (Z->rtmin + Z->rtmax) / 2;
					...
					if (lastScan->hasMz(mz, userPPM)) {
						...
						//(lastLastScan && lastLastScan->hasMz(mz,userPPM))) {
						mzSlice* s = new mzSlice(mzmin, mzmax,
								rt - 2 * rtWindow, rt + 2 * rtWindow);
						...
						if (slices.size() % 1000 == 0) {
							cerr << "MassSlices count=" << slices.size()
									<< endl;
						}
					}
				}
