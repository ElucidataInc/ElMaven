...
void Database::loadAll() {
	...
	cerr << "compoundsDB=" << compoundsDB.size() << " " << compoundIdMap.size()
			<< endl;
...
multimap<string, Compound*> Database::keywordSearch(string needle) {
	...
	query.prepare(
			"SELECT compound_id, keyword from sets where keyword like '%?%'");
	...
	while (query.next()) {
		...
		if (cmpd != NULL)
			matches.insert(pair<string, Compound*>(keyword, cmpd));
	}
	return matches;
}
...
void Database::loadCategories() {
	...
	query.prepare(
			"SELECT P.pathway_id, R.species_id from pathways P, reactions R where P.reaction_id = R.reaction_id");
	if (!query.exec())
		qDebug() << query.lastError();
	...
	if (!compoundIdMap.count(c->id)) {
		...
		for (int i = 0; i < compoundsDB.size(); i++) {
			...
			if (currentCompound->db == c->db && currentCompound->id == c->id) { //compound from the same database
				...
				currentCompound->productMz = c->precursorMz;
...
set<Compound*> Database::findSpeciesByMass(float mz, float ppm) {
	...
	;
	deque<Compound*>::iterator itr = lower_bound(compoundsDB.begin(),
			compoundsDB.end(), &x, Compound::compMass);
...
Compound* Database::findSpeciesById(string id) {
	if (compoundIdMap.count(id))
		return compoundIdMap[id];
	return NULL;

	Compound* c = NULL;
	for (int i = 0; i < compoundsDB.size(); i++) {
		if (compoundsDB[i]->id == id) {
			c = compoundsDB[i];
			break;
		}
	}
...
Molecule2D* Database::getMolecularCoordinates(QString id) {
	if (coordinatesMap.count(id.toStdString()))
		return coordinatesMap[id.toStdString()];
...
Compound* Database::findSpeciesByPrecursor(float precursorMz, float productMz,
		int polarity) {
	Compound* x = NULL;
	float dist = FLT_MAX;

	for (unsigned int i = 0; i < compoundsDB.size(); i++) {
		if (compoundsDB[i]->precursorMz == 0)
			continue;
		//cerr << polarity << " " << compoundsDB[i]->charge << endl;
		if ((int) compoundsDB[i]->charge != polarity)
			continue;
		float a = abs(compoundsDB[i]->precursorMz - precursorMz);
		if (a > 0.5)
			continue; // q1 tollorance
		float b = abs(compoundsDB[i]->productMz - productMz);
		if (b > 0.5)
			continue; // q2 tollarance
...
void Database::loadReactions(string db) {
	...
	while (query.next()) {
		...
		if (c != NULL && r != NULL
				&& seenSpeciesReactions.count(c->id + r->id) == 0) {
			seenSpeciesReactions[c->id + r->id] = true;
			c->reactions.push_back(r);
		}

		if (c != NULL && r != NULL && species_type == "R")
			r->addReactant(c, stoch);
		if (c != NULL && r != NULL && species_type == "P")
			r->addProduct(c, stoch);
	}
}
...
void Database::loadPathways() {
	...
	while (query.next()) {
		...
		if (id.empty() || name.empty() || rid.empty())
			continue;
