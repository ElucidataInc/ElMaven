...
int fft_double(unsigned NumSamples, int InverseTransform, double *RealIn,
	...
	if (!IsPowerOfTwo(NumSamples) || (NumSamples < 2)) {
		fprintf(
		stderr, "Error in fft():  NumSamples=%u is not power of two\n",
				NumSamples);
...
int fft_float(unsigned NumSamples, int InverseTransform, float *RealIn,
...
int main() {
	int dim = 8;
	float y[] = { 10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0 };
	float *x = calloc(dim, sizeof(float));
	float *c = calloc(dim, sizeof(float));
	float *r = calloc(dim, sizeof(float));

	int i = 0;
	for (i = 0; i < dim; i++)
		printf("%f\t%f\t%f\t%f .\n", y[i], x[i], r[i], c[i]);

	fft_float(dim, 0, y, x, r, c);
	for (i = 0; i < dim; i++)
		printf("%f\t%f\t%f\t%f .\n", y[i], x[i], r[i], c[i]);
	for (i = 0; i < dim; i++)
		x[i] = y[i] = 0;
	fft_float(dim, 1, r, c, y, x);
	for (i = 0; i < dim; i++)
		printf("%f\t%f\t%f\t%f .\n", y[i], x[i], r[i], c[i]);
