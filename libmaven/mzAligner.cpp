...
void Aligner::saveFit() {
	...
	for (int i = 0; i < samples.size(); i++) {
		...
		for (int ii = 0; ii < samples[i]->scans.size(); ii++) {
...
double Aligner::checkFit() {
	...
	double sumR2 = 0;
	for (int i = 0; i < allgroups.size(); i++) {
		for (int j = 0; j < allgroups[i]->peakCount(); j++) {
...
//	temp.open("/tmp/fit.csv");
	...
	for (int s = 0; s < samples.size(); s++) {
		...
		if (n - removedCount < 10) {
			cerr << "\t Can't align.. too few peaks n=" << n << " removed="
					<< removedCount << endl;
			continue;
		}
		...
		for (int ii = 0; ii < n; ii++) {
			...
			if (newrt != newrt || ISINF(newrt) || newrt < -0.2
					|| newrt > maxRt * 1.2 || abs(newrt - x[ii]) > 2) {
				//cerr << "Polynomical transform failed! (A) " << x[ii] << "-->" << newrt <<  endl;
				bool transformedFailed = true;
			} else {
				R_after += POW2(ref[ii] - newrt);
			}
		}

		cerr << "Alignment Sample: n=" << n << " mean=" << meanDiv << " std="
				<< stdDiv;
		...
		if (transformedFailed == false && R_after < R_before) {
			...
			for (int ii = 0; ii < allgroups.size(); ii++) {
				...
				if (p) {
					...
					if (!isnan(newrt) && !ISINF(newrt)) { //nan check
						...
						cerr << "Polynomial transformed failed! (peak)" << p->rt
								<< endl;
					}
				}
				/*
			...
			cerr << sample->sampleName << "Not Aligning :) R2 increased"
					<< endl;
		}
	}
