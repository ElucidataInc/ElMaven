		...
		while (getline(myfile, line)) {
			...
			if (fields.size() >= 5 && lineNum > 1) {
				...
				if (scannum != lastScanNum) {
					...
					if (!polarity.empty() && polarity[0] == '-')
						scanpolarity = -1;
					scan = new Scan(this, newscannum, mslevel, rt / 60,
							precursorMz, scanpolarity);
					if (mslevel > 1)
						scan->productMz = mz;
...
void mzSample::parseMzData(const char* filename) {
	...
	for (xml_node spectrum = spectrumstore.child("spectrum"); spectrum;
		...
		for (xml_node cvParam = spectrumInstrument.child("cvParam"); cvParam;
			...
			//
			if (strncasecmp(cvParam.attribute("name").value(), "TimeInMinutes",
					10) == 0) {
				rt = cvParam.attribute("value").as_float();
				//cout << "rt=" << rt << endl;
			}

			if (strncasecmp(cvParam.attribute("name").value(),
		...
		if (mslevel <= 0)
			mslevel = 1;
		Scan* scan = new Scan(this, scannum, mslevel, rt, precursorMz,
				scanpolarity);
		addScan(scan);

		int precision1 =
				spectrum.child("intenArrayBinary").child("data").attribute(
						"precision").as_int();
		string b64intensity =
				spectrum.child("intenArrayBinary").child("data").child_value();
		scan->intensity = base64::decode_base64(b64intensity, precision1 / 8,
				false);

		//cout << "mz" << endl;
		int precision2 =
				spectrum.child("mzArrayBinary").child("data").attribute(
						"precision").as_int();
		string b64mz =
...
void mzSample::parseMzXML(const char* filename) {
	...
	try {
		...
		if (!msInstrument.empty()) {
			...
			instrumentInfo["msDetector"] =
					msDetector.attribute("value").value();
		}
...
void mzSample::parseMzXMLScan(const xml_node& scan, int scannum) {
	...
	if (!peaks.empty()) {
		...
		if (b64intensity.empty())
			return;  //no m/z intensity values

		if (msLevel <= 0)
			msLevel = 1;
		Scan* _scan = new Scan(this, scannum, msLevel, rt, precursorMz,
				scanpolarity);
		...
		if (strncasecmp(peaks.attribute("byteOrder").value(), "network", 5)
				== 0) {
			networkorder = true;
		}

		int precision = peaks.attribute("precision").as_int();
		vector<float> mzint = base64::decode_base64(b64intensity, precision / 8,
				networkorder);
...
void mzSample::calculateMzRtRange() {
	...
	minMz = std::numeric_limits<float>::max();
	maxMz = 0;
	...
	if (maxRt >= 1e4)
		maxRt = 1e4;
	cerr << "calculateMzRtRange() rt=" << minRt << "-" << maxRt << " mz="
			<< minMz << "-" << maxMz << endl;
}
...
EIC* mzSample::getEIC(float precursorMz, float collisionEnergy, float productMz,
		float amuQ1 = 0.1, float amuQ2 = 0.5) {
	EIC* e = new EIC();
...
EIC* mzSample::getEIC(float mzmin, float mzmax, float rtmin, float rtmax,
		int mslevel) {
	...
	deque<Scan*>::iterator scanItr = lower_bound(scans.begin(), scans.end(),
			&tmpScan, Scan::compRt);
	...
	if (this->maxRt - this->minRt > 0
		...
		estimatedScans = float(rtmax - rtmin) / (this->maxRt - this->minRt)
				* scans.size() + 10;
	}
	...
	for (; scanItr != scans.end(); scanItr++) {
		...
		if (__maxIntensity > e->maxIntensity)
			e->maxIntensity = __maxIntensity;
...
float mzSample::correlation(float mz1, float mz2, float ppm, float rt1,
		float rt2) {
...
Scan* mzSample::getAverageScan(float rtmin, float rtmax, int mslevel,
		float resolution) {
	...
	for (int s = 0; s < this->scans.size(); s++) {
		...
		if (scan->rt < rtmin || scan->rt > rtmax || scan->mslevel != mslevel)
			continue;
	...
	Scan* avgScan = new Scan(this, 0, 1, rtmin + (rtmax - rtmin) / 2, 0,
			polarity);
