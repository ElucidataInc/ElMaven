...
public:
	...
	Scan(mzSample* sample, int scannum, int mslevel, float rt,
	...
	bool operator<(const Scan& b) {
		return rt < b.rt;    //default comparision operation
	}

	inline int getPolarity() {
		return polarity;
	}
...
public:
	...
	Scan* getAverageScan(float rtmin, float rtmax, int mslevel,
			float resolution);
	...
	EIC* getEIC(float precursorMz, float collisionEnergy, float productMz,
			float amuQ1, float amuQ2);
	...
	inline string getSampleName() {
		return sampleName;
	}
	void setSampleOrder(int x) {
		_sampleOrder = x;
	}
	inline int getSampleOrder() {
		return _sampleOrder;
	}
	inline string getSetName() {
		return _setName;
	}
...
public:
	EIC() {
		...
		spline = NULL;
		baseline = NULL;
		mzmin = mzmax = rtmin = rtmax = maxIntensity = totalIntensity =
				eic_noNoiseObs = 0;
	...
	static vector<PeakGroup> groupPeaks(vector<EIC*>&eics, int smoothingWindow,
			float maxRtDiff);
...
public:
	...
	inline static float overlap(const Peak& a, const Peak& b) {
...
public:
	enum GroupType {
		None = 0, C13 = 1, Adduct = 2, Fragment = 3, Covariant = 4, Isotope = 5
	}; //group types
	enum QType {
		AreaTop = 0,
		Area = 1,
	...
	void setSrmId(string id) {
		srmId = id;
	}
	inline string getSrmId() {
		return srmId;
	}

	bool isPrimaryGroup();
	inline bool hasCompoundLink() {
		if (compound != NULL)
			return true;
		return false;
	}
	...
	inline deque<PeakGroup>& getChildren() {
		return children;
	}
	...
	inline float ppmDist(float cmass) {
		return mzUtils::ppmDist(cmass, meanMz);
	}

	inline void addPeak(const Peak& peak) {
	...
	GroupType _type;
	inline GroupType type() {
		return _type;
	}
	inline void setType(GroupType t) {
		_type = t;
	}
	inline bool isIsotope() {
		return _type == Isotope;
	}
	inline bool isFragment() {
		return _type == Fragment;
	}
	inline bool isAdduct() {
		return _type == Adduct;
	}
	...
	static bool compRankPtr(const PeakGroup* a, const PeakGroup* b) {
		return (a->groupRank > b->groupRank);
	}
	...
	bool operator<(const PeakGroup* b) {
...
public:
	...
	void setPeakGroup(const PeakGroup& group) {
		_group = group;
		_group.compound = this;
	}
	bool hasGroup() {
		if (_group.meanMz != 0)
			return true;
	...
	static bool compMass(const Compound* a, const Compound* b) {
		return (a->mass < b->mass);
	}
	static bool compName(const Compound* a, const Compound* b) {
		return (a->name < b->name);
	}
	static bool compFormula(const Compound* a, const Compound* b) {
		return (a->formula < b->formula);
	}
	static bool compReactionCount(const Compound* a, const Compound* b) {
		return (a->reactions.size() < b->reactions.size());
	}
...
public:
	...
	Isotope(string name, float mass, int c = 0, int n = 0, int s = 0,
			int h = 0) {
		this->mass = mass;
		this->name = name;
		C13 = c;
...
public:
	...
	void saveFit();
	void restoreFit();
	void setMaxItterations(int x) {
		maxItterations = x;
	}
	void setPolymialDegree(int x) {
		polynomialDegree = x;
	}
private:
