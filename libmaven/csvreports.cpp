...
void CSVReports::openPeakReport(string outputfile) {
	...
	if (peakReport.is_open()) {
		peakReport
				<< "groupId,compound,compoundId,sample,peakMz,medianMz,baseMz,rt,rtmin,rtmax,quality,peakIntensity,peakArea,peakAreaTop,peakAreaCorrected,noNoiseObs,signalBaseLineRatio,fromBlankSample"
				<< endl;
	}
}
		...
		for (int i = 0; i < masslist.size(); i++) {
			...
			if ((isotopeName.find("C13-label") != string::npos
				...
				if (counter == 0) { //output zero-intensity peaks
					groupReport << "No peak" << "," << setprecision(7)
							<< group->metaGroupId << "," << "N/A" << ","
							<< "N/A" << "," << "N/A" << "," << "N/A" << ","
							<< "N/A" << "," << isotopeName << ","
							<< group->compound->name << ","
							<< group->compound->id << "," << "N/A" << ","
							<< "N/A";
...
void CSVReports::writeGroupInfo(PeakGroup* group) {
	...
	groupReport << label << "," << setprecision(7) << group->metaGroupId << ","
			<< groupId << "," << group->goodPeakCount << ", " << group->meanMz
			<< "," << group->meanRt << "," << group->maxQuality << ","
			<< tagString;
...
void CSVReports::writePeakInfo(PeakGroup* group) {
	...
	for (unsigned int j = 0; j < group->peaks.size(); j++) {
		...
		peakReport << setprecision(8) << groupId << "," << compoundName << ","
				<< compoundID << "," << sampleName << "," << peak.peakMz << ","
				<< peak.medianMz << "," << peak.baseMz << "," << setprecision(3)
				<< peak.rt << "," << peak.rtmin << "," << peak.rtmax << ","
				<< peak.quality << "," << peak.peakIntensity << ","
				<< peak.peakArea << "," << peak.peakAreaTop << ","
				<< peak.peakAreaCorrected << "," << peak.noNoiseObs << ","
				<< peak.signalBaselineRatio << "," << peak.fromBlankSample
				<< "," << endl;
	}
}
