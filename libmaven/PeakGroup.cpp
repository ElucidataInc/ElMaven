...
float PeakGroup::meanRtW() {
    ...
    if (Wtotal > 0 ) {
        for(int i=0; i < peakCount(); i++ ) mean +=  peaks[i].peakIntensity/Wtotal * peaks[i].rt;
        return mean;
    } else {
        for(int i=0; i < peakCount(); i++ ) mean += peaks[i].rt;
...
void PeakGroup::reduce() { // make sure there is only one peak per sample
    ...
    for( unsigned int i=0; i < peaks.size(); i++) {
        ...
        } else if ( maxPeaks.count(c) > 0  && rtdiff < abs( groupMeanRt- maxPeaks[c].rt) ) {
            maxPeaks[c].copyObj(peaks[i]);
        }
    }
