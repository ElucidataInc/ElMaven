...
void SavGolSmoother::SetOptions(int num_left, int num_right, int order) {
	...
	int res = savgol(golay_coeffs, np, mint_Nleft_golay, mint_Nright_golay, 0,
			mint_golay_order);
	...
	for (int i = 0; i <= mint_Nleft_golay; i++) {
		mvect_coefficients[mint_num_coeffs / 2 - i] =
				(float) golay_coeffs[i + 1];
	}
	for (int i = 1; i <= mint_Nright_golay; i++) {
		mvect_coefficients[mint_num_coeffs / 2 + i] =
				(float) golay_coeffs[mint_num_coeffs - i];
	}
	delete[] golay_coeffs;
}

void SavGolSmoother::Smooth(std::vector<float> *mzs,
	...
	for (int i = 0; i < size; i++) {
		...
		for (int j = start_index; j < stop_index; j++) {
			sum_before = sum;
			float val = intensities->at(j)
					* mvect_coefficients[j - start_index];
			sum = sum_before + val;
		}
	...
	intensities->insert(intensities->begin(), mvect_temp_y.begin(),
			mvect_temp_y.end());
}
