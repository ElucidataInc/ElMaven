...
void conv(int lx, int ifx, float *x, int ly, int ify, float *y, int lz, int ifz,
		 ...
		 *****************************************************************************/
		{
	int ilx = ifx + lx - 1, ily = ify + ly - 1, ilz = ifz + lz - 1, i, j, jlow,
			jhigh;
	float sum;
...
bool withinXppm(float mz1, float mz2, int ppmWindow) {
	if (mz2 > (mz1 - (mz1 / 1e6) * ppmWindow)
			&& mz2 < (mz1 + (mz1 / 1e6) * ppmWindow))
		return (true);
	else
		return (false);
}
...
float ttest(StatisticsVector<float>& groupA, StatisticsVector<float>& groupB) {
	...
	int n2 = groupB.size();
	if (n1 == 0 && n2 == 0)
		return 0;       //both empty.. no different
	if (n1 == 0 || n2 == 0)
		return 1000;    //one is empty inf difference
	...
	float t_test = (meanA - meanB)
			/ sqrt(((stdA * stdA) / n1) + ((stdB * stdB) / n2));
	return t_test;
}
...
double beta(double x, double y) {
	...
	if (x >= 2 && y >= 2) { //approximation
		return sqrt(2 * Pi) * pow(x, x - 0.5) * pow(y, y - 0.5)
				/ pow(x + y, (x + y - 0.5));
	}
