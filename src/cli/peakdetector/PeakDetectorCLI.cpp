...
void PeakDetectorCLI::processOptions(int argc, char* argv[]) {
	...
	while (const char optchar = opts(iter, optarg)) {
		...
		case 'S':
			...
			sample_cohort_file = QString(optarg);
			break;
	...
	if (pollyArgs.isEmpty()) {
		...
		try {
			...
			if (valid_sample_cohort) {
				bool status_sample_copy = QFile::copy(sample_cohort_file, 
			...
			QStringList files_to_be_uploaded = QStringList() << csv_filename + ".csv" << 
													json_filename + ".json" << 
			...
			cerr << "Unable to upload data to Polly...Please check the CLI arguments.." << endl;
		}
...
void PeakDetectorCLI::saveCSV(string setName) {
	...
	// NOTE: The following validation is being done to prevent a workflow breaking
	// issue (SYP-24) caused by missing C12 PARENT labels for labelled data. This
	// is only being done for the CLI.
    ...
    // Iterates over each row, continuously assigning a pointer to an unread CsvRow.
	...
	if (!labeledDataPresent) fileNeedsCorrection = false;
    ...
    if (fileNeedsCorrection) {
        ...
        for (int i=0; i<mavenParameters->allgroups.size(); i++) {
			PeakGroup &group = mavenParameters->allgroups[i];
			// `csvreports->groupId` is incremented with each group added, we can use this to check if the group to be
			// added next is present within groupsWithMissingLabels or not.
			if (binary_search(groupsWithMissingLabels.begin(), groupsWithMissingLabels.end(), csvreports->groupId + 1)) {
				PeakGroup newGroup = group;
				Compound *compound = group.getCompound();
				float compoundMz = MassCalculator::computeMass(compound->formula, mavenParameters->getCharge(compound));
				float cutoffDist = massCutoffDist(group.meanMz, compoundMz, mavenParameters->massCutoffMerge);
		    	if (cutoffDist < mavenParameters->massCutoffMerge->getMassCutoff()) {
