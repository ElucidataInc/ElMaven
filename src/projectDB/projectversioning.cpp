...
namespace ProjectVersioning
{
     ...
     * releases using the same DB version as the last mentioned release need not
     * be added.
     */
    map<Version, int> appDbVersionMap = {{Version("0.6.0"), 0},
                                         {Version("0.7.0"), 1},
                                         {Version("0.8.0"), 2},
                                         {Version("0.9.0"), 3},
                                         {Version("0.10.0"), 4},
    ...
    /**
     * Update this for every release where DB format changes. Please only add
     * the absolute minimum change that would be needed for moving from one
     * database format to the next. Also, please ensure that each SQL statement
     * ends with a semi-colon (;), as it would according to SQLite standard
     * syntax.
     */
    map<int, string> dbVersionUpgradeScripts = {
        {0,
         ...
         "ALTER TABLE peakgroups ADD COLUMN fragmentation_fraction_matched "
         "REAL;"
         ...
         "ALTER TABLE peakgroups ADD COLUMN fragmentation_weighted_dot_product "
         "REAL;"
         "ALTER TABLE peakgroups ADD COLUMN fragmentation_spearman_rank_corr "
         "REAL;"
        ...
        {2,
         ...
         "CREATE TABLE user_settings ( ionization_mode                  "
         "INTEGER "
         "                           , ionization_type                  "
         "INTEGER "
         "                           , instrument_type                  "
        ...
        {3,
         "BEGIN TRANSACTION;"
         "ALTER TABLE user_settings ADD COLUMN identification_match_rt  "
         "INTEGER;"
        ...
        {5,
         ...
         "ALTER TABLE user_settings ADD COLUMN filter_isotopes_against_parent "
         "INTEGER;"
         "ALTER TABLE user_settings ADD COLUMN filter_adducts_against_parent "
         "INTEGER;"
         ...
         "ALTER TABLE user_settings ADD COLUMN peak_width_quantile INTEGER;"
    ...
    Version::Version(string version)
    {
        ...
        auto shuffled =
            std::remove_if(begin(version), end(version), [](unsigned char x) {
                return isspace(x);
            });
                                                   ...
                                                   int* aheadBy)
    {
        ...
        if (regex_search(tag, matches, rgx)) {
            ...
            // captured groups for alpha/beta build stage should be on index 4
            // and 5
            ...
            cerr << "Error: given tag string \"" << tag
                 << "\" does not have expected format" << endl;
        }
    ...
    {
        // the assumption here is that every database version must exist as a
        // key
        if (!dbVersionUpgradeScripts.count(fromVersion)
            || !dbVersionUpgradeScripts.count(toVersion - 1)) {
            cerr << "No conversion script can be generated for: " << fromVersion
                 << " → " << toVersion << endl;
            return "";
        }
        ...
        for (const auto entry : dbVersionUpgradeScripts) {
            ...
            // relying on keys of map being sorted for generating the correct
            // script
                         ...
                         const string& appVersionString)
    {
        ...
        if (!success) {
            cerr << "Error: failed to backup original database; "
                 << "continuing with the upgrade anyway …" << endl;
        }
    ...
    {
        cout << "Debug: backing up file " << originalFilepath << " as "
             << newFilepath << " …" << endl;
