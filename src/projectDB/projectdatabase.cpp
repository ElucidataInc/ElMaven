                                       ...
                                       const string& tableName)
{
    ...
    groupsQuery->bind(":peakML_probability", group->predictionProbability());
...
pair<float, float> ProjectDatabase::_getRtMinMaxForSamples(
...
void ProjectDatabase::saveGroupSettings(const PeakGroup* group, int groupId)
{
    ...
    settingsQuery->bind(":badGroupUpperLimit",
                        BDOUBLE(settingsMap.at("badGroupUpperLimit")));
    settingsQuery->bind(":goodGroupLowerLimit",
                        BDOUBLE(settingsMap.at("goodGroupLowerLimit")));
...
{
    auto tableQuery = _connection->prepare(
        "SELECT *              \
    ...
    const MavenParameters* globalParams)
{
    ...
    while (groupsQuery->next()) {
        ...
        group->peakMLOutputValue =
            groupsQuery->doubleValue("peakML_output_value");
        ...
        string correlatedGroups =
            groupsQuery->stringValue("peakML_correlated_groups");

        // removing json brackets.
        if (correlatedGroups.size() > 2) {
            correlatedGroups =
                correlatedGroups.substr(1, correlatedGroups.size() - 2);
            ...
            for (auto groupValue : eachGroup) {
                ...
                group->addCorrelatedGroup(correlatedGroupId, correlationFactor);
            }
        }
        ...
        if (!predictedLabel.empty()) {
            ...
            group->setPredictedLabel(
                PeakGroup::classificationLabelForValue(value), probability);
                        ...
                        map<string, variant>& settingsMap)
{
    ...
    settingsMap["peakMlModel"] =
        variant(settingsQuery->stringValue("peakMlModel"));
    settingsMap["badGroupUpperLimit"] =
        variant(settingsQuery->doubleValue("badGroupUpperLimit"));
    settingsMap["goodGroupLowerLimit"] =
        variant(settingsQuery->doubleValue("goodGroupLowerLimit"));
    ...
    const shared_ptr<MavenParameters> mp)
{
    ...
    settingsMap["badGroupUpperLimit"] =
        static_cast<double>(mp->badGroupUpperLimit);
    settingsMap["goodGroupLowerLimit"] =
        static_cast<double>(mp->goodGroupLowerLimit);
