...
class PeakGroup
{
    ...
    enum class ClassifiedLabel {
        ...
        Signal,  // the group shows a clear signal but may not be an interesting
                 // metabolite
        Correlation,  // the group is a signal and is correlated to one or more
                      // signals
        Pattern,  // the group is a signal and shows interesting inter-cohort
                  // intensity pattern
        CorrelationAndPattern,  // the group is a signal which shows correlation
                                // as well as intensity pattern
        MaybeGood  // the group wich lies between limits for good and bad
                   // peakgroup by the user.
    };
    ...
    vector<mzSample*> samples;  // this varibale will hold only those sample
                                // which has been used for peak detection
    string srmId;
    string tagString;
     ...
     * @method getName
     * @detail returns compound name, tagString, srmID, meanMz@meanRt or groupId
    ...
    {
        return _integrationType == IntegrationType::Ghost;
    }
    ...
    inline unsigned int childIsotopeCount() const
    {
        return _childIsotopes.size();
    }
    inline unsigned int childAdductsCount() const
    {
        return _childAdducts.size();
    }
    inline unsigned int childIsotopeCountBarPlot() const
    {
        return _childIsotopesBarPlot.size();
    }
    ...
    vector<Scan*> getRepresentativeFullScans();  // TODO: Sahil - Kiran, Added
    ...
    // static bool compInfoScore(const PeakGroup& a, const PeakGroup& b ) {
    // return(a.informationScore > b.informationScore); }
    ...
    static bool compIsotopeLabel(const PeakGroup* a, const PeakGroup* b)
    {
