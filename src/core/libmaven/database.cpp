...
void Database::removeDatabase(string dbName)
{
    ...
    while (iter < end(_compoundsDB)) {
        ...
        if (compound->db() == dbName) {
            _compoundIdenticalCount.erase(compound->id() + compound->name()
                                          + dbName);
            _compoundIdNameDbMap.erase(compound->id() + compound->name()
                                       + dbName);
...
bool Database::addCompound(Compound* newCompound)
{
    ...
    if (_compoundIdenticalCount.count(newCompound->id()
        ...
        for (int i = 0; i < loadOrder; ++i) {
            ...
            Compound* possibleCopy =
                _compoundIdNameDbMap[newCompound->id() + nameWithSuffix
                                     + newCompound->db()];
            ...
            if (possibleCopy) {
                ...
                possibleCopy->setMz(
                    possibleCopy->adjustedMass(newCompound->charge()));
                possibleCopy->setCharge(newCompound->charge());
                newCompound->setMz(
                    newCompound->adjustedMass(newCompound->charge()));
        ...
        _compoundIdenticalCount[newCompound->id() + originalName
                                + newCompound->db()] = ++loadOrder;
    } else {
        _compoundIdenticalCount[newCompound->id() + newCompound->name()
                                + newCompound->db()] = 1;
    }

    _compoundIdNameDbMap[newCompound->id() + newCompound->name()
...
vector<Compound*> Database::findSpeciesByName(string name, string dbname)
{
    ...
    for (unsigned int i = 0; i < _compoundsDB.size(); i++) {
        if (_compoundsDB[i]->name() == name
            && _compoundsDB[i]->db() == dbname) {
                                ...
                                bsignal::signal<void(string, int, int)>* signal)
{
    ...
    for (auto specIter = begin(mgfFile); specIter != end(mgfFile); ++specIter) {
        ...
        Compound* compound = new Compound(
            specIter->getTITLE(), specIter->getTITLE(), "", charge);
                              ...
                              bsignal::signal<void(string, int, int)>* signal)
{
    ...
    unsigned lineCount = std::count(
        istream_iterator<char>(file), istream_iterator<char>(), '\n');
    ...
    while (!file.eof()) {
        ...
        if (_startsWith(line, "MW:")) {
            ...
            currentCompound->setCollisionEnergy(
                string2float(line.substr(3, line.length())));
            ...
            currentCompound->setLogP(
                string2float(line.substr(5, line.length())));

        } else if (_startsWith(line, "RT:")) {
            currentCompound->setExpectedRt(
                string2float(line.substr(3, line.length())));
            ...
            currentCompound->setPrecursorMz(
                string2float(line.substr(13, line.length())));

        } else if (_startsWith(line, "EXACTMASS:")) {
            currentCompound->setMz(
                string2float(line.substr(10, line.length())));
            ...
            if (_contain(line, "N"))
                currentCompound->ionizationMode =
                    Compound::IonizationMode::Negative;
            if (_contain(line, "P"))
                currentCompound->ionizationMode =
                    Compound::IonizationMode::Positive;
            ...
            if (regex_search(comment, match, formulaMatch)
                && match.size() > 1) {
                currentCompound->setFormula(match.str(1));
            }
            if (regex_search(comment, match, retentionTimeMatch)
                && match.size() > 1) {
            ...
            auto comment_begin = sregex_iterator(
                comment.begin(), comment.end(), keyValuePattern);

            auto comment_end = sregex_iterator();

            for (sregex_iterator k = comment_begin; k != comment_end; ++k) {
                smatch match = *k;
                ...
                if (_contain(key, "compound class")
                    ...
                    currentCompound->setCategory(
                        mzUtils::split(categories, "; "));
                }
            ...
            if (mzIntensityPair.size() >= 2) {
                ...
                if (mz >= 0.0 && in >= 0.0) {
                    ...
                    int fragIdx =
                        currentCompound->fragmentMzValues().size() - 1;
                    if (mzIntensityPair.size() >= 3) {
                        auto ionTypes = currentCompound->fragmentIonTypes();
                        ionTypes[fragIdx] = mzIntensityPair.at(2);
                                  ...
                                  string sep)
{
    ...
    auto processHeaderOrLine =
        [this, &lineCount, &headers, &header, &loadCount, &dbName](string& line,
                                                                   string sep) {
            if (lineCount == 1) {
                ...
                mzUtils::removeSpecialCharFromStartEnd(fields);
                Compound* compound = extractCompoundfromEachLine(
                    fields, header, loadCount, dbName);
...
Compound* Database::extractCompoundfromEachLine(vector<string>& fields,
                                                map<string, int>& header,
                                                int loadCount,
                                                string filename)
{
    ...
    string note;
    float rt = 0, mz = 0, charge = 0, collisionenergy = 0, precursormz = 0,
          productmz = 0, mass = 0;
    ...
    if (header.count("collisionenergy")
        && header["collisionenergy"] < NumOfFields)
...
float Database::getChargeFromDB(vector<string>& fields,
                                map<string, int>& header)
{
    float charge = 0;
    int NumOfFields = fields.size();
...
vector<string> Database::getCategoryFromDB(vector<string>& fields,
                                           map<string, int>& header)
{
    vector<string> categorylist;
    int NumOfFields = fields.size();
    ...
    if (header.count("category") && header["category"] < NumOfFields) {
        ...
        if (!catstring.empty()) {
            ...
            if (categorylist.size() == 0)
                categorylist.push_back(catstring);
        }
    }
    return categorylist;
}
...
TEST_CASE("Testing database class")
{
    SUBCASE("Testing CSV Loading File")
    {
        ...
        Compound c9("C00630", "butyryl-CoA", "C25H42N7O17P3S", 0);
        c9.setExpectedRt(15.72);
        c9.setDb("test_loadCSV");
        compoundInput.push_back(c9);

        Compound c10("C00083", "malonyl-CoA", "C24H38N7O19P3S", 0);
        ...
        compoundId = db.findSpeciesById("C00083", "test_loadCSV");
        for (size_t i = 0; i < compoundId.size(); i++) {
            REQUIRE(compoundId[i]->id() == "C00083");
        }

        vector<Compound*> compoundName;
        compoundName = db.findSpeciesByName("malonyl-CoA", "test_loadCSV");
    ...
    SUBCASE("Testing MSP File")
    {
        ...
        Compound* c1 = new Compound(
            "HMDB00902", "NAD-20.0,50.0,100.0", "C21H27N7O14P2", 0);
        c1->setMz(663.109121804);
        vector<string> category;
        category.push_back("None");
        c1->setCategory(category);
        c1->setSmileString(
            "NC(=O)c1ccc[n+](C2OC(COP(=O)([O-])OP(=O)(O)OCC3OC(n4cnc5c(N)"
            "ncnc54)C(O)C3O)C(O)C2O)c1");
        c1->ionizationMode = Compound::IonizationMode::Negative;
        c1->setCollisionEnergy(200);
        c1->setLogP(-3.6479);
        c1->setDb("test_NISTLibrary");
        vector<float> mz1 = {
            62.964214325,  78.9593391418, 92.0254847209, 96.9695861816,
            107.036354065, 134.047064209, 138.980453491, 146.98580424,
            150.980529785, 158.925656128, 174.980461121, 176.936406453,
            192.991004944, 211.001739502, 254.946289062, 272.956855774,
            328.045959473, 346.056945801, 408.012435913, 426.022750854,
        ...
        c1 = new Compound(
            "HMDB00641", "L-GLUTAMINE-20.0,50.0,100.0", "C5H10N2O3", 0);
        ...
        c1->setDb("test_NISTLibrary");
        vector<float> mz2 = {
            52.0191116333, 58.0298690796, 66.0349807739, 67.0301628113,
            70.0298519135, 71.0140228271, 71.0251178741, 72.0091705322,
            72.0455932617, 72.9931793213, 74.0247955322, 81.0460205078,
            82.0252304077, 82.0298728943, 84.0404510498, 84.0455093384,
            86.0247936249, 97.0407562256, 98.0247980754, 99.0564365387,
            101.071949005, 102.056114197, 107.02511851,  109.040693283,
            125.035812378, 125.390640259, 127.042938232, 127.051460266,
            128.035438538, 138.631530762, 142.038706462, 145.062143962};

        c1->setFragmentMzValues(mz2);

        vector<float> ion2 = {51,  2961, 57,   225,  140,  34,  134, 254,
                              28,  42,   1813, 33,   50,   925, 61,  1891,
                              253, 110,  117,  185,  548,  17,  108, 768,
                              397, 26,   42,   2527, 1147, 23,  34,  3244};
        ...
        c1 = new Compound(
            "HMDB00965", "HYPOTAURINE-20.0,50.0,100.0", "C2H7NO2S", 0);
        ...
        c1 = new Compound("HMDB00175",
                          "INOSINE 5'-PHOSPHATE-20.0,50.0,100.0",
                          "C10H13N4O8P",
                          0);
        ...
        vector<float> ion4 = {
            61, 355, 67, 7527, 1374, 871, 230, 113, 69, 523, 6304};
        ...
        c1 = new Compound("HMDB00094", "CITRATE-20.0,50.0,100.0", "C6H8O7", 0);
        c1->setDb("test_NISTLibrary");
        ...
        c1->setLogP(-1.2485);
        vector<float> mz5 = {
            57.0346002579, 58.9589700699, 59.0139074326, 67.0189723969,
            72.9933039347, 73.029571533,  83.0139490763, 85.0295581818,
            87.0087833405, 87.9253025055, 102.948770523, 103.040133158,
            103.920129776, 105.93572998,  111.008729935, 123.94648234,
            129.019475301, 130.998474121, 146.939005534, 147.030131022,
            154.998906453, 173.009328206, 191.020187378};

        c1->setFragmentMzValues(mz5);
        vector<float> ion5 = {2598, 96,  337, 971, 27, 30,  33,   1730,
                              2943, 377, 540, 26,  48, 41,  4779, 29,
                              384,  40,  291, 33,  36, 198, 1634};
        ...
        c1->setLogP(-1.2485);
        vector<float> mz6 = {
            57.0345865885, 58.9589468638, 59.0138645172, 67.0189628601,
            83.0137527466, 85.0295346578, 87.0087547302, 87.9252548218,
            102.948786418, 102.997827148, 103.919984182, 105.935714722,
            106.943557739, 111.008686066, 123.94655482,  129.019424438,
            130.998524984, 146.938732147, 147.029987335, 154.998645782,
            173.008839925, 191.020000458};

        c1->setFragmentMzValues(mz6);
        vector<float> ion6 = {2419, 265, 295, 509, 26,  1641, 2912, 1013,
                              703,  45,  218, 175, 48,  4798, 615,  362,
        ...
        c1->setLogP(-1.2485);
        vector<float> mz7 = {
            57.0345850405, 59.0138343464, 59.9852591621, 67.0189613674,
            67.8221041361, 68.8053665161, 69.5959014893, 83.0137751653,
            85.0295320594, 87.0087479301, 92.4431991577, 101.024281979,
            102.948895264, 103.040128371, 108.914337158, 111.00138855,
            111.008674622, 126.157676697, 129.019415463, 130.998455048,
            147.030051736, 154.998764038, 173.009250641, 190.186141968,
            191.020112879, 209.673370361};

        c1->setFragmentMzValues(mz7);
        vector<float> ion7 = {3297, 372, 14, 1177, 12,  37, 39,   47,   1748,
                              3150, 41,  20, 9,    29,  38, 77,   5432, 37,
                              450,  46,  42, 46,   173, 36, 1360, 36};
        ...
        c1 = new Compound(
            "HMDB00167", "L-THREONINE-20.0,50.0,100.0", "C4H9NO3", 0);
        c1->setDb("test_NISTLibrary");
        ...
        c1 = new Compound("HMDB01366", "PURINE-20.0,50.0,100.0", "C5H4N4", 0);
        ...
        c1->setLogP(0.3529);
        vector<float> mz9 = {64.0066299438,
                             65.0145670573,
                             67.0302454631,
                             68.0254770915,
                             90.0099182129,
        ...
        vector<float> ion9 = {62, 1786, 515, 204, 257, 1913, 9786};
        ...
        Compound* comp = db.findSpeciesByIdAndName(
            "HMDB01366", "PURINE-20.0,50.0,100.0", "test_NISTLibrary");
        REQUIRE(comp);
    }
    ...
    SUBCASE("Testing Mascot Library")
    {
        ...
        Compound* c1 =
            new Compound("HMDB:HMDB04095-2361 5-Methoxytryptamine M-H",
        ...
        vector<float> mz1 = {143.0, 144.0, 173.0, 175.0, 188.0, 190.0};
        c1->setFragmentMzValues(mz1);

        vector<float> ion1 = {7.605, 2.369, 100.0, 3.178, 87.625, 2.304};
        ...
        c1->setDb("test_Mascot");
        vector<float> mz2 = {56.261, 88.097, 133.997, 177.012, 222.983};

        c1->setFragmentMzValues(mz2);
        vector<float> ion2 = {5.478, 9.877, 100.0, 3.318, 91.975};
        ...
        c1->setFragmentMzValues(mz3);
        vector<float> ion3 = {
            5.266, 47.917, 28.935, 11.285, 21.296, 14.525, 100.0, 8.912, 11.69};
        ...
        c1->ionizationMode = Compound::IonizationMode::Positive;
        c1->setSmileString(
            "[H][C@@]12CCC(=O)[C@@]1(C)CC[C@]1([H])C3=C(CC[C@@]21[H])C=C(O)C="
            "C3");
        ...
        c1->ionizationMode = Compound::IonizationMode::Positive;
        c1->setSmileString(
            "OC[C@H]1O[C@@](CO)(O[C@H]2O[C@H](CO)[C@@H](O)[C@H](O)[C@H]2O)[C@@"
            "H](O)[C@@H]1O");
        c1->setPrecursorMz(343.124);
        c1->setDb("test_Mascot");
        vector<float> mz5 = {55.239,
                             69.118,
                             73.123,
                             85.059,
                             97.026,
        ...
        c1->ionizationMode = Compound::IonizationMode::Positive;
        c1->setSmileString(
            "[H][C@@]12CC[C@H]([C@H](C)CCCC(C)C)[C@@]1(C)CC[C@@]1([H])C2=CC="
            "C2C[C@@H](O)CC[C@]12C");
        c1->setPrecursorMz(385.347);
        c1->setDb("test_Mascot");
        vector<float> mz6 = {
            43.315,  55.297,  57.193,  69.167,  71.228,  81.111,  83.101,
            85.037,  93.281,  95.201,  97.106,  99.081,  105.068, 107.083,
            109.136, 110.869, 119.285, 121.033, 122.954, 131.261, 132.947,
            135.008, 142.691, 145.095, 146.953, 148.851, 157.174, 158.173,
            159.094, 160.211, 161.179, 163.264, 169.01,  170.939, 172.821,
            175.124, 176.756, 182.901, 185.072, 187.18,  188.937, 195.309,
            196.41,  197.409, 198.768, 199.65,  201.102, 203.109, 209.27,
            211.246, 212.027, 212.855, 213.628, 214.932, 217.048, 218.774,
            223.077, 225.154, 226.021, 226.934, 228.957, 231.354, 237.11,
            239.015, 241.1,   254.22,  255.048, 325.076, 339.315, 349.47,
            367.233, 367.999, 385.161, 386.021};

        c1->setFragmentMzValues(mz6);

        vector<float> ion6 = {
            18.02,  21.827, 45.685, 70.558, 62.437, 83.756, 63.959, 38.579,
            26.904, 70.558, 38.579, 17.893, 27.284, 40.102, 56.853, 25.127,
            25.761, 43.655, 32.995, 19.543, 35.533, 31.091, 21.32,  36.041,
            42.132, 26.904, 40.102, 22.589, 58.376, 23.223, 54.315, 25.888,
            22.843, 38.071, 47.716, 30.838, 22.589, 22.462, 35.025, 36.041,
            24.492, 23.604, 20.685, 26.777, 57.868, 28.68,  32.995, 20.051,
            21.32,  34.518, 18.02,  56.853, 20.685, 32.995, 17.64,  19.67,
            19.67,  17.513, 17.893, 51.777, 24.619, 22.97,  34.518, 22.97,
            28.299, 22.462, 45.178, 19.797, 19.162, 22.081, 62.944, 17.893,
            100.0,  32.995};
        ...
        c1->ionizationMode = Compound::IonizationMode::Positive;
        c1->setSmileString(
            "O[C@@H]1[C@@H](COP(O)(O)=O)O[C@H]([C@@H]1O)N1C=NC2=C1N=CN=C2NC(CC("
            "O)=O)C(O)=O");
        c1->setPrecursorMz(464.082);
        c1->setDb("test_Mascot");
        vector<float> mz7 = {
            251.0, 252.0, 253.0, 386.0, 387.0, 431.0, 463.0, 464.0, 465.0};

        c1->setFragmentMzValues(mz7);
        vector<float> ion7 = {
            65.439, 36.631, 4.831, 3.04, 3.697, 2.281, 83.455, 100.0, 38.559};
        ...
        c1->setDb("test_Mascot");
        vector<float> mz10 = {
            60.437, 85.247, 99.242, 144.142, 201.043, 260.119};

        c1->setFragmentMzValues(mz10);
        vector<float> ion10 = {17.549, 74.118, 5.221, 1.863, 37.647, 100.0};
        ...
        for (size_t i = 0; i < adductInput.size(); i++) {
            for (size_t j = 0; j < adductDb.size(); j++) {
                if (adductInput[i].getName() == adductDb[j]->getName()) {
                    ...
                    REQUIRE(adductInput[i].getCharge()
                            == adductDb[j]->getCharge());
