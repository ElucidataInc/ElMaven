                   ...
                   MsType msType)
{
    ...
    vector<pair<float, float>> mzarray =
        scan->getTopPeaks(minFractionalIntensity, minSigNoiseRatio, 5);
...
int Fragment::findClosestHighestIntensityPos(float mz, float tolr)
{
    ...
    vector<float>::iterator itr =
        lower_bound(mzValues.begin(), mzValues.end(), mzmin - 1);
    ...
    for (unsigned int k = lb; k < mzValues.size(); k++) {
        ...
        if (mzValues[k] > mzmax)
            break;
        if (intensityValues[k] > highestIntensity)
            highestIntensity = intensityValues[k];
    }
    ...
    for (unsigned int k = lb; k < mzValues.size(); k++) {
        ...
        float alignScore =
            sqrt(intensityValues[k] / highestIntensity) - (deltaMz * deltaMz);
        //	cerr << mz << "\t" << k << "\t" << deltaMz << " " << alignScore
        //<< endl;
...
vector<int> Fragment::compareRanks(Fragment* a,
                                   Fragment* b,
                                   float productPpmTolr)
{
    ...
    for (unsigned int i = 0; i < a->mzValues.size(); i++) {
        for (unsigned int j = 0; j < b->mzValues.size(); j++) {
            if (mzUtils::ppmDist(a->mzValues[i], b->mzValues[j])
                < productPpmTolr) {
                ranks[i] = j;
                break;
            }
...
void Fragment::addBrotherFragment(Fragment* b)
{
...
void Fragment::buildConsensus(float productPpmTolr)
{
    ...
    for (auto brother : brothers) {
        vector<int> ranks =
            compareRanks(brother, consensusFrag, productPpmTolr);
        ...
        for (unsigned int j = 0; j < ranks.size(); j++) {
            ...
            pair<float, float> rtRegionsB = {0.0f, 0.0f};
            if (!rtRegions.empty())
                rtRegionsB = rtRegions.at(j);
            ...
            if (posA >= 0) {
                ...
                thisRegion.first = min(thisRegion.first, rtRegionsB.first);
                thisRegion.second = max(thisRegion.second, rtRegionsB.second);
            } else if (posA == -1) {
                // new entry if m/z does not fall within ppm tolerance of
                // existing m/z
                consensusFrag->insertFragment(mzB, intB, rtRegionsB);
    ...
    if (!consensusFrag->intensityValues.size()
        || !consensusFrag->mzValues.size() || !consensusFrag->obscount.size()) {
        return;
    }
    ...
    for (unsigned int i = 0; i < consensusFrag->intensityValues.size(); i++) {
        consensusFrag->intensityValues[i] =
            consensusFrag->intensityValues[i] / maxValue * 10000;
    }
    this->consensus = consensusFrag;
}
...
void Fragment::sortByMz()
{
    ...
    for (unsigned int i = 0; i < order.size(); i++) {
        ...
        if (annotations.count(order[i]) > 0)
            tempAnnotations[i] = annotations[order[i]];
    };
...
void Fragment::buildConsensusAvg()
{
    ...
    for (unsigned int i = 0; i < fragmentList.size(); i++) {
        ...
        for (unsigned int j = 0; j < brother->mzValues.size(); j++) {
            ...
            mz_bin_map[bin] += ((double)(brother->intensityValues[j])
                                * (brother->mzValues[j]));
            mz_count[bin]++;
        }
    }
    ...
    for (itr = mz_intensity_map.begin(); itr != mz_intensity_map.end(); ++itr) {
        ...
        cerr << "\t" << setprecision(3) << avgMz << " "
             << totalIntensity / mz_count[bin] << endl;
    }
}
...
double Fragment::MVH(const vector<int>& X, Fragment* other)
{
    ...
    double A = logNchooseK(Am, Ak) + 0.1 * logNchooseK(Bm, Bk)
               + 0.001 * logNchooseK(Cm, Ck);
...
double Fragment::mzWeightedDotProduct(const vector<int>& X, Fragment* other)
{
    ...
    for (unsigned int i = 0; i < X.size(); i++) {
        int j = X[i];
        if (j != -1)
            dotP += mzValues[i] * intensityValues[i] * other->mzValues[j]
...
float Fragment::weightedDotProduct(Fragment* other, float fragmentTolerance)
{
    auto weightedDenseVector = [](Fragment* fragmentationProfile,
        ...
        double maxIntensity =
            *(max_element(begin(intensities), end(intensities)));
        ...
        double sumIntensity =
            accumulate(begin(intensities), end(intensities), 0.0);
        for (size_t i = 0; i < intensities.size(); ++i) {
            double weight =
                1.0 / (1.0 + (intensities[i] / (sumIntensity - 0.5)));
    ...
    float dotProduct =
        mzUtils::correlation(denseVectorForThis, denseVectorForOther);
    ...
    float reverseDotProduct =
        mzUtils::correlation(denseVectorForOther, denseVectorForThis);
    if ((ratioOfMatches + dotProduct + reverseDotProduct) / 3.0f > 1.0f) {
        ...
        cerr << "num matches: " << uniqueIndexes.size() << " ("
             << ranksForThis.size() << " vs. " << ranksForOther.size() << ")\n";
...
FragmentationMatchScore Fragment::scoreMatch(Fragment* other,
                                             float productPpmTolr)
{
    FragmentationMatchScore s;
    if (mzValues.size() < 2 or other->mzValues.size() < 2)
        return s;
    ...
    s.hypergeomScore =
        hyperGeometricScore(s.numMatches, a->nobs(), b->nobs(), 100000)
