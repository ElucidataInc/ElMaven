...
int Scan::findClosestHighestIntensityPos(float _mz, MassCutoff* massCutoff)
{
    ...
    vector<float>::iterator itr =
        lower_bound(mz.begin(), mz.end(), mzmin - 0.1);
    int lb = itr - mz.begin();
    float highestIntensity = 0;
    ...
    for (unsigned int k = lb; k < mz.size(); k++) {
        ...
        float alignScore =
            sqrt(intensity[k] / highestIntensity) - (deltaMz * deltaMz);
        //	cerr << _mz << "\t" << k << "\t" << deltaMz << " " << alignScore
        //<< endl;
...
vector<int> Scan::findMatchingMzs(float mzmin, float mzmax)
{
    ...
    //	cerr << "matches:" << mzmin << " " << mzmax << " " << matches.size() <<
    // endl;
    return matches;
}
...
void Scan::findLocalMaximaInIntensitySpace(int vsize,
                                           vector<float>* cMz,
                                           vector<float>* cIntensity,
                                           vector<float>* spline)
{
    for (int i = 1; i < vsize - 2; i++) {
        if ((*spline)[i] > (*spline)[i - 1]
            && (*spline)[i]
                   > (*spline)[i + 1]) {  // local maxima in spline space
...
bool Scan::hasMz(float _mz, MassCutoff* massCutoff)
{
    ...
    // cerr << _mz  << " k=" << lb << "/" << mz.size() << " mzk=" << mz[lb] <<
    // endl;
...
void Scan::summary()
{
    cerr << "Polarity=" << getPolarity() << " msLevel=" << mslevel
         << " rt=" << rt << " m/z size=" << mz.size()
         << " ints size=" << intensity.size() << " precursorMz=" << precursorMz
...
vector<float> Scan::chargeSeries(float Mx, unsigned int Zx)
{
    ...
    double M = (Mx * Zx) - Zx;
    for (unsigned int z = 1; z < Zx + 20; z++)
                             ...
                             float minSigNoiseRatio)
{
    ...
    if (mzfocus_pos < 0) {
        cout << "ERROR: Can't find parent " << mzfocus << endl;
        flag = false;
        return flag;
    }
...
{
    brotherdata->expectedMass = (mzfocus * z) - z;  // predict what M ought to
                                                    // be
...
void Scan::updateBrotherDataIfPeakFound(int loopdirection,
                                        int ii,
                                        bool* flag,
                                        bool* lastMatched,
                                        float* lastIntensity,
                                        ...
                                        MassCutoff* massCutoffMerge)
{
    ...
    if (brotherIntensity < 1.1 * (*lastIntensity) && snRatio > 2
        && withinXMassCutoff(this->mz[pos] * ii - ii,
                             brotherdata->expectedMass,
                             massCutoffMerge)) {
        if (loopdirection == 1) {
            brotherdata->maxZ = ii;
            brotherdata->upCount++;
        ...
        *lastIntensity = brotherIntensity;
        // cout << "up.." << ii << " pos=" << pos << " snRa=" << snRatio << "\t"
        // << " T=" << totalIntensity <<  endl;
    } else if (*lastMatched
               == true) {  // last charge matched ..but this one didn't..
        *flag = false;
        return;
    }
}
                            ...
                            int minChargedStates)
{
    ...
    for (int z = minDeconvolutionCharge; z <= maxDeconvolutionCharge; z++) {
        ...
        for (int ii = z; ii < z + 50 && ii < maxDeconvolutionCharge; ii++) {
            updateBrotherDataIfPeakFound(loopdirection,
                                         ii,
                                         &flag,
                                         &lastMatched,
                                         &lastIntensity,
        ...
        updateChargedSpeciesDataAndFindQScore(
            x, z, mzfocus, noiseLevel, massCutoffMerge, minChargedStates);
    }
    // done..
}

void Scan::updateChargedSpeciesDataAndFindQScore(ChargedSpecies* x,
                                                 int z,
                                                 float mzfocus,
                                                 float noiseLevel,
                                                 MassCutoff* massCutoffMerge,
                                                 int minChargedStates)
{
    if (x->totalIntensity < brotherdata->totalIntensity
        ...
        && brotherdata->upCount >= 2 && brotherdata->downCount >= 2) {
        ...
        for (int ii = brotherdata->minZ; ii <= brotherdata->maxZ; ii++) {
            int pos = this->findHighestIntensityPos(
                (brotherdata->expectedMass + ii) / ii, massCutoffMerge);
            if (pos > 0) {
        ...
        x->qscore = -20 * qscore;
        // cout << " upC=" << x->upCount << " downC=" << x->downCount << "
        // qscore=" << -qscore <<  " M=" << x->deconvolutedMass << endl;
    }
}

ChargedSpecies* Scan::deconvolute(float mzfocus,
                                  float noiseLevel,
                                  MassCutoff* massCutoffMerge,
                                  float minSigNoiseRatio,
                                  int minDeconvolutionCharge,
                                  ...
                                  int minChargedStates)
{
    bool flag = setParentPeakData(
        mzfocus, noiseLevel, massCutoffMerge, minSigNoiseRatio);

    if (flag == false)
        return NULL;
    // cout << "Deconvolution of " << mzfocus << " pSN=" << parentPeakSN <<
    // endl;
...
void Scan::findError(ChargedSpecies* x)
{
    ...
    for (unsigned int i = 0; i < x->observedCharges.size(); i++) {
        float My =
            (x->observedMzs[i] * x->observedCharges[i]) - x->observedCharges[i];
        float deltaM = abs(x->deconvolutedMass - My);
        totalError += deltaM * deltaM;
        brotherdata->totalIntensity += x->observedIntensities[i];
    }
    // cout << "\t" << mzfocus << " matches=" << x->countMatches << "
    // totalInts=" << x->totalIntensity << " Score=" << x->qscore << endl;
    x->error = sqrt(totalError / x->countMatches);
    // cout << "-------- total Error= " << sqrt(totalError/x->countMatches) << "
    // total Intensity=" << totalIntensity << endl;
}
                                             ...
                                             int dropTopX = 40)
{
    ...
    if (mid < N)
        baseline = intensity[positions[mid]];
...
string Scan::toMGF()
{
    ...
    if (sample) {
        buffer << "TITLE=" << sample->sampleName << "." << scannum << "."
               << scannum << "." << precursorCharge << endl;
    }
    buffer << "PEPMASS=" << setprecision(8) << precursorMz << " "
           << setprecision(3) << precursorIntensity << endl;
    buffer << "RTINSECONDS=" << setprecision(9) << rt * 60 << "\n";
    buffer << "CHARGE=" << precursorCharge;
    if (polarity < 0)
        buffer << "-";
    else
        buffer << "+";
    ...
    for (unsigned int i = 0; i < mz.size(); i++) {
        buffer << setprecision(8) << mz[i] << " " << setprecision(3)
               << intensity[i] << endl;
    }
...
vector<int> Scan::assignCharges(MassCutoff* massCutoffTolr)
{
    ...
    // a little silly, required number of peaks in a series in already to call a
    // charge z=0,   z=1,    z=2,   z=3,    z=4,   z=5,    z=6,     z=7,   z=8,
    ...
    for (int i = 0; i < N; i++) {
        ...
        // if ( i < 50) cerr << centerMz << " " << bestZ << " " <<
        // bestSeries.size() << " " << minSeriesSize[bestZ] << endl;
        ...
        if (bestZ > 0 and bestSeries.size() >= minSeriesSize[bestZ]) {
            ...
            for (unsigned int j = 0; j < bestSeries.size(); j++) {
                ...
                if (bestZ > 1 and mz[brother_pos] < mz[parentPeakPos]
                    ...
                    and intensity[brother_pos]
                            > intensity[parentPeakPos] * 0.25)
            ...
            // cerr << "parentPeak=" << mz[parentPeakPos] << " " << bestZ <<
            // endl;
        }
    }
    return parentPeaks;
...
void Scan::recalculatePrecursorMz(float ppm)
{
    ...
    for (int i : {1, 2, 3, 4, 5}) {
        ...
        int pos =
            fullScan->findHighestIntensityPos(this->precursorMz, massCutoff);
...
vector<mzPoint> Scan::getIsolatedRegion(float isolationWindowAmu)
{
    ...
    for (int i = 0; i < lastFullScan->nobs(); i++) {
        if (lastFullScan->mz[i] < minMz)
            continue;
        if (lastFullScan->mz[i] > maxMz)
            break;
        isolatedSegment.push_back(mzPoint(
            lastFullScan->rt, lastFullScan->intensity[i], lastFullScan->mz[i]));
    }
    return isolatedSegment;
}
...
double Scan::getPrecursorPurity(float ppm)
{
    ...
    massCutoff->setMassCutoffAndType(ppm, "ppm");
    int pos =
        lastFullScan->findHighestIntensityPos(this->precursorMz, massCutoff);
    if (pos < 0)
        return 0;
