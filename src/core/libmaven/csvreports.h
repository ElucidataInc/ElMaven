...
class CSVReports {
    ...
    * @details   -   CSVReports will do all stuf to export group info in 
    * csv format
...
public:
     ...
     * 2. Add all groups one by one to be exported by calling addItem method of
     *  this class
     ...
     */ 
    /**
     * Note: don't use same instance of this class to export more that one csv 
     * file.
...
private:
    ...
    /**@brief-  error message, TODO- QString should not be in libmaven folder,
     * only standard C++ statement should be here
     */
    string errorReport;

    /**@brief-  pointers to all mz samples uploaded
     */
    vector<mzSample*> samples;
    /**@brief-  user quant type, represents intensity of peaks
     */
    PeakGroup::QType qtype;
    MavenParameters * mavenparameters;
    ofstream outFileStream;
    string _fileName;
    /**@brief- specify either you want to export group info or peak info, 
     * check enum ExportType
     */
    ExportType _exportType;
    vector<PeakGroup*> groups;
    /**@brief- samples can belong to many sets, specify here you want to 
     * export set names or not
     */
    bool _includeSetNamesLine;
    /**@brief- groups can be labelled as good groups or bad groups, 
     * default assign it -1
     */
    int _selectionFlag;
};
