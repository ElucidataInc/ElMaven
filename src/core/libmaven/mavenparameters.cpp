...
MavenParameters::MavenParameters(string settingsPath)
    : lastUsedSettingsPath(settingsPath)
{
    clsf = NULL;
    ...
    /*
     * Whenever we create an instance of this class, massCutoffType must be set
     * for massCutoffMerge
     */
    ...
    alignMaxIterations =
        10;  // TODO: Sahil - Kiran, Added while merging mainwindow
    alignPolynomialDegree =
        5;  // TODO: Sahil - Kiran, Added while merging mainwindow
    ...
    /* default_settings_xml is a  character array generated using
     * Default_Settings.xml file It's present in settings.h header file
...
void MavenParameters::copyFrom(const MavenParameters& mp)
{
    ...
    isotopicMinSignalBaselineDifference =
...
void MavenParameters::setIsotopeDialogSettings(const char* key,
                                               const char* value)
{
...
void MavenParameters::setPeakDetectionSettings(const char* key,
                                               const char* value)
{
    if (key[0] == '\0' || value[0] == '\0')
        return;
    ...
    vector<string> fragMatchingAlgo = {"Weighted Dot-product",
...
void MavenParameters::setOptionsDialogSettings(const char* key,
                                               const char* value)
{
    if (key[0] == '\0' || value[0] == '\0')
        return;
...
bool MavenParameters::saveSettings(const char* path)
{
    ...
    if (!xmlDoc.save_file(
            path, "\t", format_default, pugi::xml_encoding::encoding_utf8))
        return false;
...
{
    if (compound != nullptr && !this->formulaFlag && compound->charge() != 0) {
...
void MavenParameters::printSettings()
{
    ...
    cerr << "#compoundMassCutoffWindow="
         << compoundMassCutoffWindow->getMassCutoff() << endl;
