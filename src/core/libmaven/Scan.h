...
particular Retention time. There are multiple m/z and intenstities in one scan
*/
...
class Scan
{
    ...
    {
        if (nobs() > 0)
            return *(std::min_element(begin(mz), end(mz)));
        return 0.0f;
    }
    ...
    {
        if (nobs() > 0)
            return *(std::max_element(begin(mz), end(mz)));
        return 0.0f;
    }
    ...
    vector<int> findMatchingMzs(float mzmin,
                                float mzmax);  // TODO why int vector?

    /**
     *@brief In a given m/z range (mz + ppm) return the position of Highest
     *intensity present in the scan
    ...
    int findClosestHighestIntensityPos(
        float mz,
        MassCutoff* massCutoff);  // TODO: Sahil, Added while merging point
    ...
    /**
     * @brief polarity of the scan defines whether the metabolites were
     ...
     * @param  x is the polarity to be set (+1 for positive, -1 for negative and
     * 0 for neutral)
     */
     ...
     * @details intensities are normalized to a maximum intensity in a scan *
     * 100]
    ...
    vector<float> chargeSeries(
        float Mx,
        unsigned int Zx);  // TODO what does this do chargeSeries?
    ...
    /**
     *@brief  return position of intensities in descending order(highest to
     *lowest)
     */
     ...
     * @details if the parent full scan has multiple readings within a precursor
     * m/z window the fragmentation scan would be a mixture of fragments from
     * all those species
     */
    ...
    /** originalRt will hold original retention time when rt is modified in case
     * of alignment
     */
    float originalRt;
    int scannum;
    ...
    int polarity; /**< +1 for positively charged, -1 for negatively charged, 0
                     for neutral*/
     ...
     */
    float scanWindowLowerBound() const
    {
        return _scanWindowLowerBound;
    }
     ...
     * @return A floating point value.
     */
    float scanWindowUpperBound() const
    {
        return _scanWindowUpperBound;
    }
     ...
     */
    float swathWindowMin()
    {
        return _swathWindowMin;
    }
     ...
     * dimension, if this is an MS/MS scan. Otherwise, 0.0 is returned.
     */
    float swathWindowMax()
    {
        return _swathWindowMax;
    }
     ...
     * @return return True if Scan a has lower retention time than Scan b, else
     * false
     */
     ...
     * @return return True if Scan a has lower precursor m/z than Scan b, else
     * false
     */
    ...
    void updateBrotherDataIfPeakFound(int loopdirection,
                                      ...
                                      float noiseLevel,
                                      MassCutoff* massCutoffMerge);
    void updateChargedSpeciesDataAndFindQScore(ChargedSpecies* x,
                                               int z,
                                               float mzfocus,
                                               float noiseLevel,
                                               MassCutoff* massCutoffMerge,
                                               int minChargedStates);
    void findBrotherPeaks(ChargedSpecies* x,
                          float mzfocus,
                          float noiseLevel,
                          MassCutoff* massCutoffMerge,
                          int minDeconvolutionCharge,
                          ...
                          int maxDeconvolutionMass,
                          int minChargedStates);
    bool setParentPeakData(float mzfocus,
                           float noiseLevel,
                           MassCutoff* massCutoffMerge,
                           float minSigNoiseRatio);
    ...
    /**
     * @brief gets the full-scan m/z-int readings that fall within the isolation
     * window of the precursor
     */
    vector<mzPoint> getIsolatedRegion(float isolationWindowAmu = 1.0);

    vector<float> smoothenIntensitites();
    /**
     *@brief compare the current intensity with the left and the right intensity
     *and store the largest amongest them
     */
    void findLocalMaximaInIntensitySpace(int vsize,
                                         vector<float>* cMz,
