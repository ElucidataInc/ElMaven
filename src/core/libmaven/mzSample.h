...
{
    public:
    enum class MsMsType { DDA, PRM, None, DIA };
    ...
    /**
     * @brief Load sample (supported formats: .mzxml, .mzml, .mzdata, .mzcsv and
     * .cdf)
     ...
     * @return Returns a map of name and corresponding value of an xml_node
     * object
     */
     ...
     * @details Update map srmScans where key is the filterline and value is int
     * vector. int vector contains scan numbers
     ...
     * @param eicType Type of EIC (max or sum)
     * @param filterline selected filterline
     * @param precursorMz When creating MS2 EIC, only DIA scans whose
     * isolation window contains precursor's m/z will be used for constructing
     * the EIC.
    ...
    void applyPolynomialTransform();  // TODO: Sahil, Added while merging
                                      // projectdockwidget
     ...
     */
    inline const unsigned int ms1ScanCount()
    {
        return _numMS1Scans;
    }
     ...
     */
    inline const unsigned int ms2ScanCount()
    {
        return _numMS2Scans;
    }
     ...
     */
    inline const unsigned int diaScanCount()
    {
        return _numDIAScans;
    }
    ...
    {
        return mzUtils::compareStringsWithNumbers(a->sampleName, b->sampleName);
    }
    ...
    inline void setMsMsType(MsMsType msMsType)
    {
        _msMsType = msMsType;
    }
    inline MsMsType msMsType() const
    {
        return _msMsType;
    }
...
class ChargedSpecies
{
     ...
     * @return []
     */
    static bool compIntensity(ChargedSpecies* a, ChargedSpecies* b)
    {
     ...
     * @return []
     */
    static bool compMass(ChargedSpecies* a, ChargedSpecies* b)
    {
        return a->deconvolutedMass < b->deconvolutedMass;
    }
     ...
     * @return []
     */
    static bool compMatches(ChargedSpecies* a, ChargedSpecies* b)
    {
        return a->countMatches > b->countMatches;
    }
     ...
     * @return []
     */
    static bool compRt(ChargedSpecies* a, ChargedSpecies* b)
    {
    ...
    {
        return (a->isotopicClusterId * 100000 + a->deconvolutedMass
                < b->isotopicClusterId * 100000 + b->deconvolutedMass);
    }
