...
QPalette namedColorSchemePalette(ThemeType x)
{
    ...
    auto themeColorsToPalette = [](const ThemeColors& x) -> QPalette {
        ...
        if (x.disabledButton.isValid())
            pal.setColor(
                QPalette::Disabled, QPalette::Button, x.disabledButton);
...
MainWindow::MainWindow(Controller* controller, QWidget* parent)
    : _controller(controller), QMainWindow(parent)
{
    ...
    styleSheet +=
        "QTabWidget > QTabBar::tab:selected { background-color: %7; }";
    styleSheet +=
        "QTabWidget > QTabBar::tab:selected { border-bottom-color: %8; }";
...
#ifdef Q_OS_MAC
        methodsFolder =
            QCoreApplication::applicationDirPath() + "/../../../methods/";
#endif
    }
    ...
    appDir =
        QDir::cleanPath(QApplication::applicationDirPath()) + QDir::separator();
#endif

#if defined(Q_OS_MAC)
    appDir = qApp->applicationDirPath() + QDir::separator() + ".."
    ...
    connect(fileLoader,
            SIGNAL(createPeakTableSignal(QString)),
            this,
            SLOT(createPeakTable(QString)));
    ...
    addDockWidget(Qt::BottomDockWidgetArea, _correlationTable, Qt::Horizontal);
    ...
    fragSpectraDockWidget =
        createDockWidget("Fragmentation spectra", fragSpectraWidget);
    ...
    alignmentVizAllGroupsDockWidget =
        createDockWidget("All groups deviation", alignmentVizAllGroupsPlot);
    alignmentVizAllGroupsWidget =
        new AlignmentVizAllGroupsWidget(this, alignmentVizAllGroupsDockWidget);
    ...
    connect(peakDetectionDialog,
            SIGNAL(findPeaksClicked()),
            this,
            SLOT(sendPeaksGA()));
    ...
    addDockWidget(
        Qt::BottomDockWidgetArea, fragSpectraDockWidget, Qt::Horizontal);
    ...
    addDockWidget(Qt::BottomDockWidgetArea,
                  ...
                  Qt::Horizontal);
    addDockWidget(
        Qt::BottomDockWidgetArea, isotopePlotDockWidget, Qt::Horizontal);
    ...
    connect(fileLoader,
            SIGNAL(sampleLoaded()),
            this,
            SLOT(setInjectionOrderFromTimeStamp()));
    connect(fileLoader,
            SIGNAL(sampleLoaded()),
            projectDockWidget,
    ...
    connect(fileLoader,
            SIGNAL(projectLoaded()),
            this,
            SLOT(deleteCrashFileTables()));
    connect(fileLoader,
            SIGNAL(projectLoaded()),
            this,
            SLOT(setInjectionOrderFromTimeStamp()));
    connect(projectDockWidget,
            &ProjectDockWidget::samplesDeleted,
            spectraWidget,
            &SpectraWidget::clearScans);
    connect(fileLoader,
    ...
    connect(fileLoader,
            SIGNAL(unsupportedFileFormat(QStringList)),
            this,
            SLOT(unsupportedFormat(QStringList)));
    // EMDB singals and slots
    connect(fileLoader,
            SIGNAL(sqliteDBLoadStarted(QString)),
            SLOT(_showEMDBProgressBar(QString)));
    connect(fileLoader, &mzFileIO::settingsLoaded, this, [=] {
        _updateEMDBProgressBar(1, 5);
    });
    connect(fileLoader, &mzFileIO::sqliteDBSamplesLoaded, this, [=] {
        _updateEMDBProgressBar(2, 5);
    });
    connect(fileLoader, &mzFileIO::sqliteDBPeakTablesCreated, this, [=] {
        _updateEMDBProgressBar(3, 5);
    });
    connect(fileLoader, &mzFileIO::sqliteDBAlignmentDone, this, [=] {
    ...
    connect(fileLoader,
            ...
            [=](QList<QString> sampleFiles, bool wasMemoryError) {
                ...
                if (wasMemoryError) {
                    ...
                    actionButton =
                        msgBox.addButton("Learn more", QMessageBox::ActionRole);
    ...
    if (ligandWidget) {
         ...
         * inorder to revert the value back to the original, its value was saved
         * and set back to original Note: the default value present in .conf
         * file is KNOWNS
         */

        QString lastDatabaseFile =
    ...
    for (unsigned int i = 0; i < size; ++i) {
        ...
        // TODO:If the file does not exist it has to be removed from the entry
        // of the Qsettings The above part of the code has to be implemented
    ...
    if (CrashFileList.size() > 0) {
        ...
        QPushButton* restoreNO =
            msgBox->addButton(tr("Don't Restore"), QMessageBox::ActionRole);
        msgBox->setIcon(QMessageBox::Information);
        msgBox->setText(
            tr("Unfortunately, ElMaven crashed when you were using it "
               "earlier.\nHowever, we can restore the application without "
               "losing information."));
...
{
    qDebug() << "New release" << QString("v%1").arg(version) << "available.";
...
{
    if (isotopeDialog->isotopeDetectionEnabled()
        && peakDetectionDialog->databaseSearchEnabled()) {
...
void MainWindow::threadSave(const QString filename,
...
void MainWindow::showWarning(QString message)
{
    ...
    auto yesButton = warning->addButton(tr("Ok"), QMessageBox::AcceptRole);
    warning->exec();
...
void MainWindow::unsupportedFormat(QStringList unsupportedFileList)
{
    ...
    auto htmlText = QString(
                        "<p>El-MAVEN does not support the file format"
                        " of the following files: </p>"
                        "<ul>%1</ul>")
                        .arg(fileList);
    htmlText += "<p>Some files were not uploaded.</p>";
    auto reply = QMessageBox::critical(this, tr(""), htmlText);
    ...
    if (settings->value("closeEvent").toInt() == 1) {
        ...
        if (getLatestUserProject().isEmpty()) {
            ...
            confirmation.setText(
                "Would you like to save your data for this "
                "session as a project, before you quit?");
            QPushButton* cancelButton =
                confirmation.addButton(tr("Cancel"), QMessageBox::ActionRole);
            QPushButton* noButton =
                confirmation.addButton(tr("No"), QMessageBox::RejectRole);
            QPushButton* yesButton =
                confirmation.addButton(tr("Yes"), QMessageBox::AcceptRole);
            ...
            QMessageBox confirmation;
            confirmation.setText(
                "Would you like to save changes made to this "
            ...
            QMessageBox msgBox;
            msgBox.setText(
                "Please choose the project file to save your "
                "progress for this session.");
            QPushButton* saveButton = msgBox.addButton(tr("Save in current"),
                                                       QMessageBox::AcceptRole);
            QPushButton* newButton =
                msgBox.addButton(tr("Save in new"), QMessageBox::AcceptRole);
...
void MainWindow::showAlignmentErrorDialog(QString errorMessage)
{
    ...
    auto yesButton =
        alignmentError->addButton(tr("Ok"), QMessageBox::AcceptRole);
    alignmentError->exec();
    QCoreApplication::processEvents();
}
...
{
    TableDockWidget* panel =
        new PeakTableDockWidget(this, tableTitle, hasClassifiedGroups);
    analytics->hitEvent("New Table", "Peak Table");

    addDockWidget(Qt::BottomDockWidgetArea, panel, Qt::Horizontal);

    QToolButton* btnTable = addDockWidgetButton(
        sideBar, panel, QIcon(rsrcPath + "/Peak Table.png"), "");
...
void MainWindow::setInjectionOrderFromTimeStamp()
{
    ...
    for (unsigned int i = 1; i <= samples.size(); i++) {
        ...
        if (sample->injectionTime > 0)
            sample->setInjectionOrder(i);
    }
}
...
void MainWindow::setTotalCharge()
{
    ...
    bool emDBIsLoading =
        (_loadProgressDialog != nullptr && _loadProgressDialog->isVisible());
...
void MainWindow::setFormulaFocus(QString formula)
{
    ...
    if (eicWidget->isVisible()) {
        ...
        Compound* c = new Compound(
            compoundId, compoundName, formula.toStdString(), charge);
        setCompoundFocus(c);
    }
}
                                  ...
                                  float fragmentMz)
{
    ...
    if (eicWidget->isVisible() && samples.size() > 0) {
        if (fragmentMz > 0.0f) {
            eicWidget->showMsMsEic(
                compound->adjustedMass(mavenParameters->charge), fragmentMz);
        } else {
...
void MainWindow::open()
{
    ...
    if (!_versionRecordExists()) {
        ...
        msgBox.setText(
            "El-MAVEN is now capable of reading files containing "
    ...
    if ((cumulativeSize * 1.5) + (numCpus * firstFileSize) >= totalMemory) {
        ...
        msgBox.setText(
            "It seems you are trying to load data that might be too "
        ...
        QPushButton* learnButton =
            msgBox.addButton("Learn more", QMessageBox::AcceptRole);
        ...
        if (msgBox.clickedButton() == learnButton) {
            QDesktopServices::openUrl(
                QUrl("https://docs.elucidata.io/Apps/"
    ...
    if (!sampleList.isEmpty() && projectList.isEmpty()) {
        ...
        QMessageBox msgBox;
        msgBox.setText(
            "It seems you are trying to load sample files and "
            ...
            "be loaded.");
        QPushButton* sampleChosen =
            msgBox.addButton("Sample files", QMessageBox::ActionRole);
        QPushButton* projectChosen =
            msgBox.addButton("Project file", QMessageBox::ActionRole);
    ...
    Q_FOREACH (QString filename, filelist) {
        if (fileLoader->isEmdbProject(filename)) {
            ...
            emdbProjectBeingLoaded =
                fileLoader->swapFilenameExtension(filename, "emDB");
            ...
            if (xml.isStartElement()) {
                if (xml.name() == "sample") {
                    ...
                    while (!sampleFile.exists()) {
                        QString message =
                            "Select the directory having samples corresponding "
                            "to "
                            + filename;
...
{
    QStringList filelist = QFileDialog::getOpenFileNames(
        this, "Select Model To Load", ".", "All Files(*.model)");
    if (filelist.size() > 0)
    ...
    if ((compoundCount > 0 || reloading) && ligandWidget) {
        ...
        if (!loadedCompounds.empty()) {
            ...
            if (loadedCompounds[0]->precursorMz() > 0
                ...
                eventLabel =
                    DB.isSpectralLibrary(dbName) ? "MS2 (PRM)" : "MS2 (MRM)";
            }
            analytics->hitEvent(
                "Load Compound DB", "Successful Load", eventLabel);
        ...
        setStatusText(tr("Failed to load compound database").arg(filename));
                                         ...
                                         bool failedToLoadCompletely)
{
    if (failedToLoadCompletely) {
        analytics->hitEvent(
            "Load Compound DB", "Column Error", "Complete Failure");
        ...
        if (!DB.notFoundColumns().empty()) {
            analytics->hitEvent(
                "Load Compound DB", "Column Error", "Partial Failure");
            ...
            string msgString =
                "Following are the unknown column name(s) "
                "found:";
            for (const auto& column : DB.notFoundColumns()) {
                msgString += "\n - " + column;
            }
        ...
        if (!DB.notFoundColumns().empty()) {
            ...
            string msgString =
                "The following compounds had insufficient "
...
void MainWindow::_warnIfNISTPolarityMismatch()
{
    ...
    Compound::IonizationMode polarity =
        DB.getCompoundsSubset(dbName)[0]->ionizationMode;
    int dbPolarity = 0;
    ...
    if (samplePolarity != dbPolarity) {
        ...
        QString msg =
            "The polarity of loaded samples and spectral library do "
        ...
        if (msgBox.clickedButton() == upload) {
            analytics->hitEvent(
                "PRM", "Loaded Spectral Library", "Polarity Mismatch prompt");
            loadCompoundsFile();
        } else {
...
bool MainWindow::_versionRecordExists()
{
    ...
    QString versionLogPath = QString(
        QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation)
        + QDir::separator() + "ElMaven" + QDir::separator() + "version.log");
    ...
    QRegularExpression ver(
        "v((\\d+\\.\\d+\\.\\d+)(-(beta|alpha))?(\\.(\\d)*)?(-(\\d+))?)");
    ...
    while (!stream.atEnd()) {
        ...
        if (match.hasMatch() && sameVersionMatch(currentVersionMatch, match)) {
            return true;
        }
    }
...
void MainWindow::checkCorruptedSampleInjectionOrder()
{
    ...
    // store samples with same injection order in a map with injection order as
    // their key
    QMap<int, QList<mzSample*>> sameInjectionOrder;
    auto it = samples.begin();
    while (it != samples.end()) {
        if ((*it)->getInjectionOrder() != 0
            && ((*it)->getInjectionOrder() != -1)) {
            if (sameInjectionOrder.find((*it)->getInjectionOrder())
                == sameInjectionOrder.end())
                sameInjectionOrder.insert((*it)->getInjectionOrder(),
                                          QList<mzSample*>() << (*it));

            else {
                QMap<int, QList<mzSample*>>::iterator i =
                    sameInjectionOrder.find((*it)->getInjectionOrder());
    ...
    // present a warning to user only if we found some samples with corrupted
    // injection order
...
void MainWindow::warningForInjectionOrders(
    QMap<int, QList<mzSample*>> sameOrders,
    QList<mzSample*> negativeOrders)
{
    ...
    {
        // iterarte on negative orders and add corrupted samples to warning
        // message
...
bool MainWindow::loadMetaInformation(QString filename)
{
    ...
    if (sampleCount > 0) {
        setStatusText(tr("loadMetaInfo: done after loading %1 meta information")
                          .arg(QString::number(sampleCount)));
        Q_EMIT(metaCsvFileLoaded());

        // user should be presented a warning if injection order of two samples
        // is same or is negative
        checkCorruptedSampleInjectionOrder();
        return true;
    } else {
        setStatusText(tr("loadMetaInfo: not able to load %1 meta information")
                          .arg(filename));
        return false;
    }
}
...
void MainWindow::loadMetaInformation()
{
    ...
    if (!loadMetaInformation(filelist[0])) {
        ...
        msgBox.setText(tr("Trouble in loading set information %1")
                           .arg(QString::fromStdString(dbname)));
...
int MainWindow::loadMetaCsvFile(string filename)
{
    ...
    static const string allHeadersarr[] = {
        "sample", "set", "cohort", "scaling", "injection order", "color"};
    vector<string> allHeaders(
        allHeadersarr,
        allHeadersarr + sizeof(allHeadersarr) / sizeof(allHeadersarr[0]));

    // assume that files are tab delimited, unless matched ".csv", then comma
    // delimited
    string sep = "\t";
    if (filename.find(".csv") != -1 || filename.find(".CSV") != -1)
        sep = ",";
    ...
    while (getline(myfile, line)) {
        ...
        if (header.count("sample") && header["sample"] < N)
            sampleName = QString::fromUtf8(fields[header["sample"]].c_str());
        ...
        if (header.count("injection order") && header["injection order"] < N) {
            ...
            if (!io.empty()) {
                try {
                    // injection order with a value between 1-9 is considered
                    // valid otherwise it's given a value of -1
                    ...
                    // if any of the character is anything other than a  digit
                    // that means injection order is invalid
                    else
                        injectionOrder = -1;
                } catch (std::exception&) {
                    // we could not convert injection order string to int since
                    // it was something invalid
                    injectionOrder = -1;
                }
            }
        }
        ...
        if (header.count("color") && header["color"] < N) {
            ...
            if (!colorName.empty()) {
                if (colorName[0] == '#') {
                    ...
                    transform(colorName.begin(),
                              colorName.end(),
                              colorName.begin(),
                              ::tolower);
                    auto colorNameQstring = QString::fromStdString(colorName);
                    string hexValue = rootxkcdColors.value(colorNameQstring)
                                          .toString()
                                          .toStdString();
        ...
        if (redF != -1 && greenF != -1 && blueF != -1 && alpha != -1) {
            sample->color[0] = redF;
...
void MainWindow::exportPDF()
{
    ...
    const QString fileName = QFileDialog::getSaveFileName(
        this, "Export File Name", QString(), "PDF Documents (*.pdf)");
...
void MainWindow::writeSettings()
{
    ...
    settings->setValue(
        "mzslice", QRectF(slice.mzmin, slice.mzmax, slice.rtmin, slice.rtmax));
...
void MainWindow::createMenus()
{
    ...
    connect(saveProject, SIGNAL(triggered()), this, SLOT(explicitSave()));
    fileMenu->addAction(saveProject);

    // add option to save as a database
    QAction* saveProjectAsSQLite = new QAction(tr("Save project as…"), this);
    ...
    QAction* saveProjectWithRaw =
        new QAction(tr("Save project with raw data…"), this);
                ...
                map<int, QUrl> URL{
                    ...
                    {2,
                     QUrl("https://www.youtube.com/channel/"
                          "UCZYVM0I1zqRgkGTdIlQZ9Yw/videos")},
...
{
    QString fileName =
        QFileDialog::getSaveFileName(Q_NULLPTR, "Save Settings", QString());
...
void MainWindow::_showEMDBProgressBar(QString projectFilename)
{
    ...
    QLabel* label = new QLabel(
        QString("<b>Loading Project \"%1\"</b>").arg(projectFilename));
...
void MainWindow::loadSettings()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(
        ...
        "Load Settings",
        dir,
        tr("All Known Formats(*.xml *.XML);; ") + tr("All Files(*.*)"));
    ...
    QFile file(fileName);
    if (fileName.size()
        && !(fileName.contains(".xml") || fileName.contains(".XML"))) {
        ...
        auto htmlText =
            QString(
                "<p>El-MAVEN settings does not support the file format"
...
void MainWindow::createToolBars()
{
    ...
    btnSettings->setToolTip(
        tr("1. File import: scan filter settings\n"
    ...
    btnFeatureDetect->setToolTip(
        tr("Peak detection and group filtering settings"));
    ...
    btnPollyBridge->setToolTip(
        tr("Send peaks to Polly™ to store, collaborate, "
    ...
    btnInfo->setToolTip(
        tr("Documentation, information and technical support "
           "for El-MAVEN."));

    connect(btnOpen, &QToolButton::clicked, this, &MainWindow::open);
    connect(btnAlign, SIGNAL(clicked()), alignmentDialog, SLOT(show()));
    connect(
        btnIsotope, &QToolButton::clicked, isotopeDialog, &IsotopeDialog::show);
    connect(
        btnAdducts, &QToolButton::clicked, adductWidget, &AdductWidget::show);
    connect(
        btnFeatureDetect, SIGNAL(clicked()), SLOT(showPeakdetectionDialog()));
    connect(btnPollyBridge,
            SIGNAL(clicked()),
            SLOT(showPollyElmavenInterfaceDialog()));
    connect(btnSettings, SIGNAL(clicked()), SLOT(showsettingsForm()));
    // connect(btnSpectraMatching, SIGNAL(clicked()),
    // SLOT(showspectraMatchingForm()));
    ...
    connect(
        ionChargeBox, SIGNAL(valueChanged(int)), this, SLOT(setTotalCharge()));
    ...
    QShortcut* ctrlK =
        new QShortcut(QKeySequence(tr("Ctrl+K", "Do search")), this);
    QShortcut* ctrlF =
        new QShortcut(QKeySequence(tr("Ctrl+F", "Do search")), this);
    ...
    connect(suggestPopup,
            SIGNAL(compoundSelected(Compound*)),
            this,
    ...
    connect(suggestPopup,
            ...
            ligandWidget,
            SLOT(setCompoundFocus(Compound*)));
    connect(ligandWidget,
            SIGNAL(databaseChanged(QString)),
            suggestPopup,
    ...
    connect(perGroupAlignment,
            &QAction::triggered,
            this,
            &MainWindow::togglePerGroupAlignmentWidget);
    connect(allGroupAlignment,
            &QAction::triggered,
            this,
            &MainWindow::toggleAllGroupAlignmentWidget);
    connect(sampleRtDeviation,
            &QAction::triggered,
            this,
            &MainWindow::toggleSampleRtWidget);
...
void MainWindow::_postProjectLoadActions()
{
    ...
    if (!tableName.isEmpty()) {
        auto foundAt =
            find_if(begin(groupTables),
                    end(groupTables),
                    [tableName](TableDockWidget* table) {
                        return TableDockWidget::getTitleForId(table->tableId)
                               == tableName;
                    });
...
{
    QString message(
        "The project \"%1\" seems to have been created in a later "
...
void MainWindow::showSRMList()
{
    if (srmDockWidget->isVisible()) {
        ...
        if (getIonizationMode())
            userPolarity = getIonizationMode();
        ...
        vector<mzSlice*> slices = srmList->getSrmSlices(
            amuQ1, amuQ3, userPolarity, associateCompoundNames);
...
void MainWindow::setPeakGroup(shared_ptr<PeakGroup> group)
{
    ...
    if (group->hasCompoundLink() && isotopeWidget != nullptr
        && (isotopeWidget->isVisible() || isotopePlot->isVisible())) {
        isotopeWidget->setPeakGroupAndMore(group);
    }

    if (group->hasCompoundLink()) {
        if (group->ms2EventCount)
            fragSpectraDockWidget->setVisible(true);
...
void MainWindow::Align()
{
    ...
    if (alignmentDialog->alignAlgo->currentIndex() == 0) {
        ...
        connect(
            workerThread, SIGNAL(finished()), eicWidget, SLOT(replotForced()));
        connect(
            workerThread, SIGNAL(finished()), alignmentDialog, SLOT(close()));
...
void MainWindow::showPeakInfo(Peak* _peak)
{
    ...
    // if (getIonizationMode())
    ionizationMode =
        mavenParameters->ionizationMode;  // user specified ionization mode
...
QString MainWindow::groupTextExport(PeakGroup* group)
{
    ...
    if (group->hasCompoundLink()) {
        compoundName =
            "\"" + QString(group->getCompound()->name().c_str()) + "\"";
    ...
    groupInfo << "sample\tgroupId\tcompoundName\texpectedRt\tpeakMz\tmedianMz\t"
                 "rt\trtmin\trtmax\tquality\tpeakIntensity\tpeakArea\tpeakSplin"
                 "eArea\tpeakAreaTop\tpeakAreaCorrected\tnoNoiseObs\tsignalBase"
                 "LineRatio\tfromBlankSample";
...
void MainWindow::showFragmentationScans(float pmz)
{
    ...
    for (auto sample : samples) {
        ...
        for (auto scan : sample->scans) {
            if (scan->mslevel != 2)
                continue;
            if (massCutoffDist(scan->precursorMz, pmz, massCutoff)
                > massCutoff->getMassCutoff()) {
                continue;
            }
...
bool MainWindow::checkCompoundExistance(Compound* c)
{
    ...
    for (unsigned int i = 0; i < samples.size(); i++) {
        ...
        for (unsigned int j = 0; j < samples[i]->scans.size(); j++) {
            vector<int> matches =
                samples[i]->scans[j]->findMatchingMzs(mzmin, mzmax);
            if (matches.size() > 0) {
                consectveMatches++;
            } else
                consectveMatches = 0;
...
QWidget* MainWindow::eicWidgetController()
{
    ...
    QWidgetAction* btnZoom =
        new MainWindowWidgetAction(toolBar, this, "btnZoom");
    QWidgetAction* btnCopyCSV =
        new MainWindowWidgetAction(toolBar, this, "btnCopyCSV");
    QWidgetAction* btnIntegrateArea =
        new MainWindowWidgetAction(toolBar, this, "btnIntegrateArea");
    QWidgetAction* btnLast =
        new MainWindowWidgetAction(toolBar, this, "btnLast");
    QWidgetAction* btnNext =
        new MainWindowWidgetAction(toolBar, this, "btnNext");
    QWidgetAction* btnExport =
        new MainWindowWidgetAction(toolBar, this, "btnExport");
    QWidgetAction* btnAutoZoom =
        new MainWindowWidgetAction(toolBar, this, "btnAutoZoom");
    QWidgetAction* btnShowTic =
        new MainWindowWidgetAction(toolBar, this, "btnShowTic");
    QWidgetAction* btnShowBarplot =
        new MainWindowWidgetAction(toolBar, this, "btnShowBarplot");
    QWidgetAction* btnShowIsotopeplot =
        new MainWindowWidgetAction(toolBar, this, "btnShowIsotopeplot");
    QWidgetAction* btnShowBoxplot =
        new MainWindowWidgetAction(toolBar, this, "btnShowBoxplot");
    QWidgetAction* spacer = new MainWindowWidgetAction(toolBar, this, "spacer");
    QWidgetAction* toleranceSyncSwitch =
        new MainWindowWidgetAction(toolBar, this, "toleranceSyncSwitch");
...
QWidget* MainWindowWidgetAction::createWidget(QWidget* parent)
{
    if (btnName == "btnZoom") {
        ...
        connect(
            btnZoom, SIGNAL(clicked()), mw->getEicWidget(), SLOT(resetZoom()));
        return btnZoom;

    } else if (btnName == "btnCopyCSV") {
        QToolButton* btnCopyCSV = new QToolButton(parent);
        btnCopyCSV->setIcon(QIcon(rsrcPath + "/copyCSV.png"));
        btnCopyCSV->setToolTip(
            tr("Copy group information to clipboard (Ctrl + C)"));
        ...
        btnIntegrateArea->setToolTip(
            tr("Manual integration (Shift + mouse-drag)"));
        connect(btnIntegrateArea,
                SIGNAL(clicked()),
                mw->getEicWidget(),
        ...
        QAction* pdfAction = btnExportMenu->addAction(
            QIcon(rsrcPath + "/exportpdf.png"), "Export EIC as PDF file");
        ...
        QAction* printAction = btnExportMenu->addAction(
            QIcon(rsrcPath + "/fileprint.png"), "Print EIC (Ctrl+P)");
        ...
        btnAutoZoom->setToolTip(
            tr("Auto Zoom. Always center chromatogram on expected retention "
               "time!"));
        ...
        connect(btnShowTic,
                &QToolButton::toggled,
                mw->getEicWidget(),
                &EicWidget::showTicLine);
        connect(btnShowTic,
                SIGNAL(toggled(bool)),
                mw->getEicWidget(),
                SLOT(replot()));
        connect(mw->getEicWidget(),
                &EicWidget::optionTicChecked,
                btnShowTic,
        ...
        connect(btnShowBarplot,
                &QToolButton::toggled,
                mw->getEicWidget(),
                &EicWidget::showBarPlot);
        connect(btnShowBarplot,
                SIGNAL(toggled(bool)),
                mw->getEicWidget(),
                SLOT(replot()));
        connect(mw->getEicWidget(),
                &EicWidget::optionBarPlotChecked,
                btnShowBarplot,
                &QToolButton::setChecked);
        ...
        connect(btnShowIsotopeplot,
                SIGNAL(clicked(bool)),
                mw,
                SLOT(toggleIsotopicBarPlot(bool)));
        connect(btnShowIsotopeplot,
                SIGNAL(clicked(bool)),
                mw->isotopeWidget,
                SLOT(updateIsotopicBarplot()));
        btnShowIsotopeplot->setChecked(mw->isotopePlotDockWidget->isVisible());
        connect(mw->isotopePlotDockWidget,
                SIGNAL(visibilityChanged(bool)),
                btnShowIsotopeplot,
                SLOT(setChecked(bool)));
        ...
        connect(btnShowBoxplot,
                &QToolButton::toggled,
                mw->getEicWidget(),
                &EicWidget::showBoxPlot);
        connect(btnShowBoxplot,
                SIGNAL(toggled(bool)),
                mw,
                SLOT(analyticsBoxPlot()));
        connect(btnShowBoxplot,
                SIGNAL(toggled(bool)),
                mw->getEicWidget(),
                SLOT(replot()));
        connect(mw->getEicWidget(),
                &EicWidget::optionBoxPlotChecked,
                btnShowBoxplot,
                &QToolButton::setChecked);
        ...
        toleranceSyncSwitch->setIcon(
            QIcon(rsrcPath + "/toleranceSyncUnlock.png"));
        toleranceSyncSwitch->setToolTip(
            tr("Sync EIC with current global mass "
               "tolerance"));
...
void MainWindow::getLinks(Peak* peak)
{
    ...
    vector<mzLink> linksX = SpectraWidget::findLinks(
        peak->peakMz, scan, massCutoff, ionizationMode);
    ...
    for (int i = 0; i < links.size(); i++) {
        links[i].correlation = sample->correlation(links[i].mz1,
                                                   links[i].mz2,
                                                   massCutoff,
                                                   rtmin,
                                                   rtmax,
...
MatrixXf MainWindow::getIsotopicMatrix(PeakGroup* group, bool barplot)
{
    ...
    for (int i = 0; i < isotopes.size(); i++) {
        ...
        vector<float> values =
            isotopes[i]->getOrderedIntensityVector(vsamples, qtype);
...
MatrixXf MainWindow::getIsotopicMatrixIsoWidget(PeakGroup* group)
{
    ...
    for (int i = 0; i < group->childIsotopes().size(); i++) {
        if (group->childIsotopes()[i]->isIsotope()) {
            ...
            if (isotope->tagString.find(delimIsotopic) != string::npos
                || isotope->tagString.find(delimParent) != string::npos) {
                if (isotope->tagString.find(delimParent) != string::npos) {
                    carbonIsotopeSpecies.insert(
                        pair<unsigned int, string>(0, isotope->tagString));
                } else if (isotope->tagString.find(delimIsotopic)
                           != string::npos) {
                    unsigned int carbonLabel =
                        atoi(isotope->tagString
                                 .substr(delimIsotopic.size()
                                         - (isotope->tagString.size()
                                            - delimIsotopic.size() - 1))
                                 .c_str());
                    carbonIsotopeSpecies.insert(pair<unsigned int, string>(
                        carbonLabel, isotope->tagString));
                }
            }
        }
    }
    ...
    MatrixXf MM((int)vsamples.size(),
                (int)2 * isotopes.size());  // rows=samples, cols=isotopes
    ...
    for (int i = 0; i < isotopes.size(); i++) {
        if (!isotopes[i])
            continue;
        vector<float> values = isotopes[i]->getOrderedIntensityVector(
            vsamples,
    ...
    if (group->getCompound() && !group->getCompound()->formula().empty()) {
        map<string, int> composition =
            MassCalculator::getComposition(group->getCompound()->formula());
...
void MainWindow::isotopeC13Correct(
    MatrixXf& MM,
    int numberofCarbons,
    map<unsigned int, string> carbonIsotopeSpecies)
{
...
mzSample* MainWindow::getSampleByName(QString name)
{
    ...
    for (int i = 0; i < samples.size(); i++) {
        if (samples[i] == NULL)
            continue;
        if (samples[i]->sampleName
            == mzUtils::cleanFilename(name.toStdString())) {
            return samples[i];
        }
    }
    return NULL;
