...
EicWidget::EicWidget(QWidget* p)
{
    ...
    scene()->setItemIndexMethod(
        QGraphicsScene::NoIndex);  // TODO: Sahil, uncommed this while merging
                                   // eicwidget
    ...
    _mouseEndPos = _mouseStartPos =
        QPointF(0, 0);  // TODO: Sahil, added while merging eicwidget
...
void EicWidget::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    } else if (abs(deltaX) > 10
               && event->button()
                      == Qt::LeftButton) {  // user released button and no items
                                            // are selected
        if (deltaX > 0) {                   // zoom in
...
void EicWidget::mouseDoubleClickEvent(QMouseEvent* event)
{
    ...
    if (selScan != NULL) {
        ...
        Q_EMIT(scanChanged(
            selScan));  // TODO: Sahil, added while merging eicwidget
    }
}
...
void EicWidget::setScan(Scan* scan)
{
    ...
    if (scan == NULL)
        return;
    getMainWindow()->spectraWidget->setScan(
        scan, eicParameters->_slice.rtmin - 5, eicParameters->_slice.rtmax + 5);
}
...
void EicWidget::computeEICs()
{
    ...
    if (_groupPeaks)
        groupPeaks();  // TODO: Sahil, added while merging eicwidget
...
void EicWidget::findPlotBounds()
{
    ...
    for (int i = 0; i < eicParameters->peakgroups.size(); i++) {
        if (mzUtils::checkOverlap(eicParameters->peakgroups[i].minRt,
                                  eicParameters->peakgroups[i].maxRt,
                                  eicParameters->_slice.rtmin,
                                  eicParameters->_slice.rtmax)
            > 0) {
    ...
    if (_minX > _maxX)
        swap(_minX, _maxX);
    // qDebug() << "EicWidget::findPlotBounds()" << _slice.rtmin << " " <<
    // _slice.rtmax << " " << _minY << " " << _maxY;
}
                            ...
                            float rtMax)
{
    ...
    auto setLineAttributes = [&](EicLine* line,
        ...
        QColor eicColor = QColor::fromRgbF(
            eic->color[0], eic->color[1], eic->color[2], alpha);
        QPen pen(
            eicColor.darker(), 1, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
    ...
    for (unsigned int i = 0; i < eicParameters->eics.size(); i++) {
        ...
        if (_showBaseline) {
            ...
            if (_areaIntegration || overlayingIntegratedArea) {
                setLineAttributes(
                    lineBelowBaseline, eic, fadedMultiplier, zValue);
            } else {
                setLineAttributes(
                    lineBelowBaseline, eic, alphaMultiplier, zValue);
            }
            ...
            if (overlayingIntegratedArea) {
                ...
                setLineAttributes(
                    lineBelowBaselineLeft, eic, fadedMultiplier, zValue);
                setLineAttributes(
                    lineBelowBaselineRight, eic, fadedMultiplier, zValue);
            }
        }
    }
}
    ...
    for (unsigned int i = 0; i < eicParameters->eics.size(); i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (eic->rt[j] < eicParameters->_slice.rtmin
                || eic->rt[j] > eicParameters->_slice.rtmax)
                continue;
            x[j] = eic->rt[j];
            f[j] = eic->intensity[j];
        ...
        QColor pcolor =
            QColor::fromRgbF(eic->color[0], eic->color[1], eic->color[2], 0.3);
        QPen pen(pcolor, 1);
        QColor bcolor =
            QColor::fromRgbF(eic->color[0], eic->color[1], eic->color[2], 0.3);
        QBrush brush(bcolor);
...
void EicWidget::addTicLine()
{
    ...
    if (eicParameters->tics.size() == 0
        ...
        for (int i = 0; i < samples.size(); i++) {
            int mslevel = 1;
            // attempt at automatically detecting correct scan type for
            // construstion of TIC
            ...
            if (chrom != NULL)
                eicParameters->tics.push_back(chrom);
        }
    }
...
EicLine* EicWidget::addBaseLine(EIC* eic, double zValue)
{
    ...
    QColor color =
        QColor::fromRgbF(eic->color[0], eic->color[1], eic->color[2], 1);
    line->setColor(color);
    line->setZValue(zValue);
...
{
    // Peak table is being deleted. Making sure the selected group is not
    // holding any garbage value;
...
void EicWidget::replot(shared_ptr<PeakGroup> group)
{
    ...
    if (group != nullptr && !group->tableName().empty()) {
        ...
        if (_plottingMs2) {
            ...
            auto pos =
                fragPattern.findClosestHighestIntensityPos(fragMz, fragTolr);
        ...
        } else if (group && group->getCompound() != nullptr
        ...
        if (_showMS2Events && eicParameters->_slice.mz > 0)
            addMS2Events(eicParameters->_slice.mzmin,
    ...
    void EicWidget::setTitle()
    {
        ...
        if (_plottingMs2) {
            tagString = "[F]";
        } else if (eicParameters->displayedGroup() != nullptr) {
            tagString =
                QString(eicParameters->displayedGroup()->getName().c_str());
        ...
        if (eicParameters->_slice.srmId.empty()) {
            ...
            if (eicParameters->_slice.compound != nullptr
                ...
                m1 = QString::number(
                    eicParameters->_slice.compound->precursorMz(), 'f', 3);
                m1 = "prec=" + m1;
                m2 = QString::number(
                    eicParameters->_slice.compound->productMz(), 'f', 3);
                m2 = "prod=" + m2;
            }
        ...
        bool hasData = false;
        for (int i = 0; i < eicParameters->eics.size(); i++) {
            if (eicParameters->eics[i]->size() > 0) {
                hasData = true;
                break;
            }
        }
        if (hasData == false) {
            ...
            text->setPos(scene()->width() / 2 - textWith / 2,
                         scene()->height() / 2);
    ...
    void EicWidget::_drawNoPeaksMessage()
    {
        ...
        QGraphicsPathItem* boundingRect =
            scene()->addPath(path, QPen(Qt::darkGray), QBrush(Qt::darkGray));
    ...
    void EicWidget::wheelEvent(QWheelEvent * event)
    {
        ...
        if (eicParameters->_slice.rtmax > bounds.rtmax)
            eicParameters->_slice.rtmax = bounds.rtmax;
        // qDebug() << "EicWidget::wheelEvent() " << _slice.rtmin << " " <<
        // _slice.rtmax << endl;
        replot(eicParameters->displayedGroup());
        if (eicParameters->displayedGroup())
            addPeakPositions(eicParameters->displayedGroup());
    ...
    void EicWidget::addFocusLine(PeakGroup * group)
    {
        ...
        if (group->getCompound() != NULL
            and group->getCompound()->expectedRt() > 0)
            _focusLineRt = group->getCompound()->expectedRt();
        else
            _focusLineRt = 0;
        ...
        if (group->peaks.size() > 0) {
            ...
            if (getMainWindow()->spectraWidget
                ...
                getMainWindow()->spectraWidget->setScan(
                    scan, selPeak.peakMz - 5, selPeak.peakMz + 5);
            }
        }
        return;
    }
    ...
    void EicWidget::setBarplotPosition(PeakGroup * group)
    {
        ...
        for (int i = 0; i < group->peaks.size(); i++) {
            ...
            if (x >= xpos_right - legendShift - 5
                && x <= xpos_right + bwidth + 5 && y <= ypos + bheight)
                count_right++;
            if (x >= xpos_left - legendShift - 5 && x <= xpos_left + bwidth + 5
                && y <= ypos + bheight)
                count_left++;
        }
    ...
    void EicWidget::addPeakPositions(shared_ptr<PeakGroup> group)
    {
        ...
        if (eicParameters->displayedGroup()
            && group == eicParameters->displayedGroup()) {
            setZValue = true;
        }
        sort(group->peaks.begin(), group->peaks.end(), Peak::compIntensity);
        ...
        for (unsigned int i = 0; i < group->peaks.size(); i++) {
            ...
            if (peak.getSample() != NULL
                && peak.getSample()->isSelected == false)
                continue;
            if (eicParameters->_slice.rtmin != 0
            ...
            EicPoint* p = new EicPoint(
                toX(peak.rt), toY(peak.peakIntensity), &peak, getMainWindow());
            if (setZValue)
                p->setZValue(i);
            p->setColor(color);
            p->setBrush(brush);
    ...
    void EicWidget::resetZoom()
    {
        ...
        eicParameters->_slice.rtmin = bounds.rtmin;
        eicParameters->_slice.rtmax = bounds.rtmax;
        // qDebug() << "EicWidget::resetZoom() " << _slice.rtmin << " " <<
        // _slice.rtmax << endl;
        replot(NULL);
    }
    ...
    void EicWidget::setCompound(
        Compound * compound, Isotope isotope, Adduct * adduct)
    {
        if (compound == nullptr)
            return;
    ...
    void EicWidget::setPeakGroup(shared_ptr<PeakGroup> group)
    {
        ...
        if (_autoZoom) {
            if (group->parent != nullptr) {
                ...
                if (parentGroup->isGhost() || parentGroup->isEmpty()) {
                    // zoom to the combined (common) region of all child
                    // siblings
                ...
                if (group->isGhost() || group->isEmpty()) {
                    ...
                    eicParameters->_slice.rtmin =
                        group->minRt - 2.0f * _zoomFactor;
                    eicParameters->_slice.rtmax =
                        group->maxRt + 2.0f * _zoomFactor;
                }
            }
        } else {
            resetZoom();
        ...
        if (eicParameters->_slice.srmId.length())
            for (int i = 0; i < eicParameters->peakgroups.size(); i++)
                eicParameters->peakgroups[i].srmId =
                    eicParameters->_slice.srmId;
    ...
    void EicWidget::setSensitiveToTolerance(bool sensitive)
    {
        ...
        if (sensitive) {
            ...
            // a new `PeakGroup` object has to be created because the
            // `setPeakGroup` method initiates a `cleanup` that erases all
            // peakgroups in `eicParameters` object;
            // `eicParameters->selectedGroup()` might also be deleted which
            // leads to corruption
            if (eicParameters->selectedGroup() != nullptr) {
                auto sharedGroup = make_shared<PeakGroup>(
                    *(eicParameters->selectedGroup().get()));
    ...
    void EicWidget::showMsMsEic(float precursorMz, float fragmentMz)
    {
        ...
        eicParameters->_slice.mzmin =
            fragmentMz - massCutoff.massCutoffValue(fragmentMz);
        eicParameters->_slice.mzmax =
            fragmentMz + massCutoff.massCutoffValue(fragmentMz);
    ...
    void EicWidget::contextMenuEvent(QContextMenuEvent * event)
    {
        ...
        connect(d, &QAction::triggered, settingsForm, &SettingsForm::exec);
    ...
    */
    ...
    void EicWidget::eicToClipboard()
    {
        if (eicParameters->eics.size() == 0)
            return;
        getMainWindow()->getAnalytics()->hitEvent(
            "Exports", "Clipboard", "From EIC Menu");
    ...
    void EicWidget::selectGroupNearRt(float rt)
    {
        ...
        selGroup = eicParameters->selectGroupNearRt(
            rt,
            selGroup,
    ...
    void EicWidget::keyReleaseEvent(QKeyEvent * e)
    {
        ...
        switch (e->key()) {
        case Qt::Key_Shift:
            // if area was being integrated and shift was released, we ignore
            // the next drag-finish event fired, otherwise it will trigger
            // zoom-in
            if (_areaIntegration)
    ...
    {
        getMainWindow()->getAnalytics()->hitEvent(
            "Exports", "Clipboard", "From Dropdown");
        getMainWindow()->setClipboardToGroup(
            eicParameters->displayedGroup().get());
    }
            ...
            for (auto const& scan : sample->scans) {
                if (scan->mslevel == 2 && scan->precursorMz >= mzmin
                    ...
                    QColor color = QColor::fromRgbF(sample->color[0],
                                                    ...
                                                    1);
                    EicPoint* p = new EicPoint(
                        toX(scan->rt), toY(10), NULL, getMainWindow());
    ...
    void EicWidget::renderPdf(shared_ptr<PeakGroup> group, QPainter * painter)
    {
        ...
        auto setLineAttributes =
            [&](EicLine* line, EIC* eic, float alpha, int zValue) {
                if (line == nullptr)
                    return;
        ...
        // lambda that when given an EicLine and a pair of RT and intensity
        // values, adds their co-ordinates to the line
        auto addPoint = [this](EicLine* line,
                               float rt,
                               float intensity,
                               float width,
                               float height,
                               ...
                               float _maxX,
                               float _maxY) {
            line->addPoint(QPointF(
                ((rt - _minX) / (_maxX - _minX) * width),
        ...
        eicparameters->getEIC(bounds, samples, group->parameters().get());
            ...
            for (int j = 0; j < eic->size(); j++) {
                if (peakRtMin > 0.0f && eic->rt[j] < peakRtMin) {
                    ...
                    addPoint(lineEicLeft,
                             ...
                             _maxY);
                    setLineAttributes(
                        lineEicLeft, eic, fadedMultiplier, zValue);
                ...
                if (peakRtMax > 0.0f && eic->rt[j] > peakRtMax) {
                    ...
                    if (prevIdx > 0 && eic->rt[prevIdx] <= peakRtMax) {
                        ...
                        addPoint(lineEicRight,
                                 ...
                                 _maxY);
                        setLineAttributes(
                            lineEicRight, eic, fadedMultiplier, zValue);
                    }
                    if (eic->intensity[j] > _maxY)
                        eic->intensity[j] = _maxY;
                    addPoint(lineEicRight,
        ...
        selectionLine->setLine(
            ((rtMin - _minX) / (_maxX - _minX) * scene.width()),
            scene.height(),
            (rtMax - _minX) / (_maxX - _minX) * scene.width(),
            scene.height());
        selectionLine->update();

        // Add Focus Line
        float rt;
        if (group && group->getCompound() != nullptr
            && group->getCompound()->expectedRt() > 0)
            ...
            for (auto const& scan : sample->scans) {
                if (scan->mslevel == 2 && scan->precursorMz >= mzmin
                    ...
                    QColor color = QColor::fromRgbF(sample->color[0],
                                                    ...
                                                    1);
                    EicPoint* p = new EicPoint(
                        (scan->rt - _minX) / (_maxX - _minX) * scene.width(),
                        scene.height()
                            - ((10 - _minY) / (_maxY - _minY) * scene.height()),
                        NULL,
                        getMainWindow());
                    p->setPointShape(EicPoint::TRIANGLE_UP);
                    p->forceFillColor(true);
