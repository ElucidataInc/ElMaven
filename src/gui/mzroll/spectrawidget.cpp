...
void SpectraWidget::setCurrentScan(Scan* scan)
{
    ...
    if (scan) {
        ...
        if (scan->mslevel == 1) {
            chargeStates =
                _currentScan->assignCharges(mainwindow->getUserMassCutoff());
        }
    }
}
...
void SpectraWidget::_placeLabels()
{
    ...
    int lowerLabelYPos = _showAsMirrorPlot
                             ? scene()->height() - lowerLabelHeight
                             : scene()->height() / 2;
    _lowerLabel->setPos(scene()->width() - lowerLabelWidth, lowerLabelYPos);
}
...
void SpectraWidget::setTitle(QString titleText)
{
    ...
    if (!_title)
        _title = scene()->addText(titleText, font);
    ...
    if (_currentScan && _currentScan->nobs() == 0) {
        ...
        _title->setPos(scene()->width() / 2 - textWidth / 2,
                       scene()->height() / 2);
...
void SpectraWidget::setScan(Scan* scan, float mzmin, float mzmax)
{
    ...
    // mainwindow->getEicWidget()->setFocusLine(scan->rt); //TODO: Sahil,
    // Removed while merging spectrawidget
    drawGraph();
    repaint();
}
...
void SpectraWidget::setScan(mzSample* sample, int scanNum = -1)
{
    ...
    if (sample->scans.size() == 0)
        return;
    if (_currentScan && scanNum < 0)
        scanNum = _currentScan->scannum;
    if (scanNum > sample->scans.size())
        scanNum = sample->scans.size() - 1;
...
void SpectraWidget::overlayPeptideFragmentation(QString peptideSeq,
                                                MassCutoff* productMassCutoff)
{
    if (!_currentScan)
        return;
    if (peptideSeq.isEmpty())
        return;
    ...
    for (unsigned int i = 0; i < ions.size(); i++) {
        ...
        int pos = _currentScan->findClosestHighestIntensityPos(
            ion->m_mz, productMassCutoff);
...
void SpectraWidget::overlayPeakGroup(shared_ptr<PeakGroup> group)
{
    ...
    if (group->getCompound()) {
        ...
        // if (!group->compound->smileString.empty())
        // overlayTheoreticalSpectra(group->compound);
    }
    delete (avgScan);
}
...
{
    clearOverlay();
    if (!_currentScan || !c || c->fragmentMzValues().size() == 0)
        return;
    ...
    if (!c->formula().empty() || c->neutralMass() != 0.0f)
        c->setPrecursorMz(
            c->adjustedMass(mainwindow->mavenParameters->getCharge(c)));
...
void SpectraWidget::overlaySpectralHit(SpectralHit& hit)
{
    ...
    int pos = _currentScan->findHighestIntensityPos(hit.precursorMz,
                                                    productMassCutoff);
    if (pos >= 0) {
...
void SpectraWidget::setScanTitle()
{
    ...
    if (_currentScan->sample)
        sampleName = QString(_currentScan->sample->sampleName.c_str());

    if (_currentScan->scannum)
        _titleText += tr("<b>Scan #</b> %1  ")
                          .arg(QString::number(_currentScan->scannum));

    if (_currentScan->rt)
        _titleText += tr("<b>RT:</b> %1  ")
                          .arg(QString::number(_currentScan->rt, 'f', 2));

    if (_currentScan->mslevel)
        _titleText += tr("<b>MS level:</b> %1  ")
                          .arg(QString::number(_currentScan->mslevel));

    if (_currentScan->precursorMz) {
        _titleText +=
            tr("<b>Pre m/z:</b> %1  ")
                .arg(QString::number(_currentScan->precursorMz, 'f', 4));
    }

    if (_currentScan->collisionEnergy) {
        _titleText +=
            tr("<b>CE:</b> %1  ")
                .arg(QString::number(_currentScan->collisionEnergy, 'f', 0));
    }

    if (_currentScan->precursorMz > 0 && _currentScan->productMz) {
        _titleText +=
            tr("<b>Prod m/z:</b> %1  ")
                .arg(QString::number(_currentScan->productMz, 'f', 3));
    }
    ...
    if (_currentScan->precursorMz > 0
        ...
        _titleText +=
            tr("<b>Purity:</b> %1  ").arg(QString::number(purity, 'f', 1));
        _titleText +=
            tr("<b>Isolation Window:</b> %1  ")
                .arg(QString::number(_currentScan->isolationWindow(), 'f', 1));
    }
...
void SpectraWidget::setGroupTitle()
{
    ...
    if (_currentGroup.getCompound())
        compoundName = QString(_currentGroup.getCompound()->name().c_str());
    ...
    _titleText +=
        tr("<b>Pre m/z:</b> %1  ").arg(QString::number(meanMz, 'f', 4));

    if (_currentScan != nullptr
        && _currentScan->msType() == Scan::MsType::DDA) {
        _titleText +=
            tr("<b>Purity:</b> %1  ").arg(QString::number(purity, 'f', 2));
    }

    setTitle(_titleText);
    mainwindow->fragSpectraDockWidget->setWindowTitle("Fragmentation spectra: "
                                                      + compoundName);
}
...
void SpectraWidget::drawScan(Scan* scan, QColor sampleColor)
{
    ...
    for (int j = 0; j < scan->nobs(); j++) {
        if (scan->mz[j] < _minX || scan->mz[j] > _maxX)
            continue;
        int x = toX(scan->mz[j]);
        ...
        if (_profileMode) {
            ...
            QGraphicsLineItem* line =
                new QGraphicsLineItem(x, y, x, yzero, nullptr);
            scene()->addItem(line);
            line->setPen(pen);
            _items.push_back(line);
        }
...
void SpectraWidget::drawScanSet(vector<Scan*>& scanset)
{
    ...
    for (unsigned int i = 0; i < scanset.size(); i++) {
        ...
        if (scanset[i]->sample) {
            ...
            sampleColor = QColor::fromRgbF(
                sample->color[0], sample->color[1], sample->color[2], 0.6);
        }
...
void SpectraWidget::drawMzLabels(Scan* scan)
{
    ...
    for (auto pos : positions) {
        if ((scan->mz[pos] < _minX) || (scan->mz[pos] > _maxX))
            continue;
        if ((scan->intensity[pos] / _maxY) < 0.005)
            continue;
...
void SpectraWidget::drawAnnotations()
{
    ...
    for (int i = 0; i < links.size(); i++) {
        if (links[i].mz2 < _minX || links[i].mz2 > _maxX)
            continue;
...
void SpectraWidget::drawGraph()
{
    ...
    if (_spectralHit.mzList.size() > 0) {
        ...
        if (fabs(_spectralHit.precursorMz - _currentScan->precursorMz) < 0.1f) {
            ...
            // TODO: either remove the check or inform user on the UI in case of
            // failure
            qDebug() << "spectra overlay skipped: " << _spectralHit.precursorMz
...
void SpectraWidget::showNextScan()
{
    incrementScan(+1, 0);
}
void SpectraWidget::showLastScan()
{
    incrementScan(-1, 0);
}
void SpectraWidget::showNextFullScan()
{
    incrementScan(+1, 1);
}
void SpectraWidget::showLastFullScan()
{
    incrementScan(-1, 1);
}
...
void SpectraWidget::incrementScan(int increment, int msLevel = 0)
{
    ...
    if (_currentScan == NULL || _currentScan->sample == NULL
    ...
    if (msLevel != 0 && newScan && newScan->mslevel != msLevel) {
        for (unsigned int i = newScan->scannum;
             i >= 0 && i < sample->scans.size();
             i += increment) {
            newScan = sample->getScan(i);
            if (newScan && newScan->mslevel == msLevel)
                break;
        }
    }
    if (newScan == NULL)
...
void SpectraWidget::setMzFocus(float mz)
{
    ...
    if (_currentScan->mslevel == 1) {
        ...
        mzSlice eicSlice =
            mainwindow->getEicWidget()->getParameters()->getMzSlice();
...
void SpectraWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (pos.y() < 5 || pos.y() > height() - 5 || pos.x() < 5
        || pos.y() > width() - 5) {
        _vnote->hide();
        _note->hide();
        _varrow->hide();
    ...
    if (nearestPos >= 0) {
        _nearestCoord = QPointF(_currentScan->mz[nearestPos],
...
int SpectraWidget::findNearestPeakPos(QPointF pos)
{
    ...
    for (const auto& elem : _scanPeaks) {
        ...
        float dist =
            hypotf(coordinates.first - pos.x(), coordinates.second - pos.y());
        if (dist < leastDist) {
...
void SpectraWidget::drawArrow(float mz1,
                              float intensity1,
                              float mz2,
                              float intensity2)
{
    ...
    if (massCutoffDist(mz1, mz2, mainwindow->getUserMassCutoff()) < 0.1)
        return;
    ...
    QString note = tr("m/z: %1 &Delta;%2")
                       .arg(QString::number(mz1, 'f', 6),
...
void SpectraWidget::annotateScan() {
    ...
    for(int i=0; i < links.size(); i++ ) {
                vector<int> matches =
_currentScan->findMatchingMzs(links[i].mz2-0.01, links[i].mz2+0.01);
        //qDebug() << "annotateScan() " << links[i].note.c_str() << " " <<
links[i].mz2 << " " << matches.size(); links[i].value2=0;

        if (matches.size() > 0) {
                        links[i].value2 = _currentScan->intensity[matches[0]];

            for(int i=1; i < matches.size(); i++ ) {
                                if(_currentScan->intensity[ matches[i] ] >
links[i].value2 ) { links[i].value2==_currentScan->intensity[ matches[i] ];
                }
            }
        }
    }
...
void SpectraWidget::annotateScan()
{
    ...
    if (mzfocus == 0 || _currentScan == NULL || _currentScan->nobs() < 2)
        return;
    float noiseLevel = 1;
    ...
    if (x and x->observedMzs.size() >= 5) {
        for (unsigned int i = 0; i < x->observedMzs.size(); i++) {
            QString noteText = tr("z=%1 M=%2")
                                   .arg(x->observedCharges[i])
                                   .arg(x->deconvolutedMass);
            mzLink link =
                mzLink(mzfocus, x->observedMzs[i], noteText.toStdString());
...
void SpectraWidget::zoomIn()
{
    ...
    if (_focusCoord.x() != 0 && _focusCoord.x() > _minX
        && _focusCoord.x() < _maxX)
        _centerX = _focusCoord.x();
...
void SpectraWidget::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    void SpectraWidget::spectraToClipboard()
    {
        ...
        for (int i = 0; i < _currentScan->nobs(); i++) {
            clipboardText << tr("%1\t%2")
                                 .arg(QString::number(
                                     _currentScan->mz[i], 'f', 6))
                                 .arg(QString::number(
                                     _currentScan->intensity[i], 'f', 6));
        }
    ...
    void SpectraWidget::spectraToClipboardTop()
    {
        ...
        vector<pair<float, float>> mzarray =
            _currentScan->getTopPeaks(0.05, 3.0, 40);
    ...
    {
        if (_currentScan == NULL or _currentScan->sample == NULL)
            return;
    ...
    vector<mzLink> SpectraWidget::findLinks(
        float centerMz, Scan* scan, MassCutoff* massCutoff, int ionizationMode)
    {
        vector<mzLink> links;
        ...
        // 	if(frag->charge != 0 && SIGN(frag->charge) !=
        // SIGN(ionizationMode) ) continue;
        //     float mzMinus=centerMz-frag->mass;
        //     float mzPlus =centerMz+frag->mass;
        //     if( scan->hasMz(mzPlus,massCutoff)) {
        //         QString noteText = tr("%1
        //         Fragment").arg(QString(DB.fragmentsDB[i]->name.c_str()));
        ...
        // parent check
        // TODO Shubhra: Figure out the purpose of this function and uncomment
        // as required
        ...
        for (int i = 0; i < adductsDB.size(); i++) {
            ...
            //     if( abs(parentMass-centerMz)>0.1 &&
            //     scan->hasMz(parentMass,massCutoff)) {
            //         cerr << DB.adductsDB[i]->name << " " <<
            //         DB.adductsDB[i]->charge << " " << parentMass << endl;
            //         QString noteText = tr("Possible Parent
            //         %1").arg(QString(DB.adductsDB[i]->name.c_str()));
        ...
        for (int i = 0; i < adductsDB.size(); i++) {
            if (SIGN(adductsDB[i]->getCharge()) != SIGN(ionizationMode))
                continue;
            //     float parentMass = centerMz-ionizationMode * H_MASS;
            //     //adjusted mass float
            //     adductMass=DB.adductsDB[i]->computeAdductMass(parentMass);
            //     if( abs(adductMass-centerMz)>0.1 &&
            //     scan->hasMz(adductMass,massCutoff)) {
            //         QString noteText = tr("Adduct
            //         %1").arg(QString(DB.adductsDB[i]->name.c_str()));
    ...
    void SpectraWidget::constructAverageScan(float rtmin, float rtmax)
    {
        ...
        if (_currentScan && _currentScan->getSample()) {
            Scan* avgScan = _currentScan->getSample()->getAverageScan(
                rtmin,
                rtmax,
                _currentScan->mslevel,
                _currentScan->getPolarity(),
                (float)100.0);
