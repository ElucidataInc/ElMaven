...
void SpectraWidget::zoomIn()
{
    ...
    // cerr << "zoomIn center=" << _centerX << " D=" << D <<  " focus=" <<
    // _focusCoord.x() << endl;

    _minX = _centerX - D / 2;
    _maxX = _centerX + D / 2;
    // cerr << _centerX << " " << _minX << " " << _maxX << " " << _minZ << " "
    // << _maxZ << endl;
    findBounds(false, true);
    replot();
}
...
vector<mzLink> SpectraWidget::findLinks(float centerMz,
                                        Scan* scan,
                                        ...
                                        int ionizationMode)
{
    ...
    // 	if(frag->charge != 0 && SIGN(frag->charge) != SIGN(ionizationMode) )
    // continue;
    ...
    // parent check
    // TODO Shubhra: Figure out the purpose of this function and uncomment as
    // required
    ...
    for (int i = 0; i < adductsDB.size(); i++) {
        ...
        //         DB.adductsDB[i]->charge << " " << parentMass << endl; QString
        //         noteText = tr("Possible Parent
    ...
    for (int i = 0; i < adductsDB.size(); i++) {
        if (SIGN(adductsDB[i]->getCharge()) != SIGN(ionizationMode))
            continue;
        //     float parentMass = centerMz-ionizationMode * H_MASS;   //adjusted
        //     mass float
        //     adductMass=DB.adductsDB[i]->computeAdductMass(parentMass); if(
        //     abs(adductMass-centerMz)>0.1 &&
