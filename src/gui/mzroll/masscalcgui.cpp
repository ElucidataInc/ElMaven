...
void MassCalcWidget::setMassCutoff(MassCutoff* massCutoff)
{
    ...
    label_3->setText(
        QApplication::translate("MassCalcWidget", &massCutoffType[0], 0));
    showTable();
}
...
void MassCalcWidget::showTable()
{
    ...
    for (unsigned int i = 0; i < matches.size(); i++) {
        ...
        QStringList rowItems = QStringList() << item1 << item2 << item3 << item4
                                             << item5 << item6;
...
void MassCalcWidget::setupSortedCompoundsDB()
{
    ...
    copy(
        compoundsDB.begin(), compoundsDB.end(), back_inserter(sortedcompounds));
    sort(sortedcompounds.begin(), sortedcompounds.end(), Compound::compMass);
}

QSet<Compound*> MassCalcWidget::findMathchingCompounds(float mz,
                                                       ...
                                                       float charge)
{
    ...
    vector<Compound*>::iterator itr = lower_bound(
        sortedcompounds.begin(), sortedcompounds.end(), &x, Compound::compMass);
    for (; itr != sortedcompounds.end(); itr++) {
        Compound* c = *itr;
        if (!c)
            continue;
        double cmass = MassCalculator::computeMass(c->formula(), charge);
        if (mzUtils::massCutoffDist((double)cmass, (double)mz, massCutoff)
...
void MassCalcWidget::getMatches()
{
    ...
    QSet<Compound*> compounds =
        findMathchingCompounds(_mz, _massCutoff, charge);
    delete_all(matches);
    Q_FOREACH (Compound* c, compounds) {
        MassCalculator::Match* m = new MassCalculator::Match();
        m->name = c->formula();
        m->mass = MassCalculator::computeMass(
            c->formula(), _mw->mavenParameters->getCharge(c));
        m->diff =
            mzUtils::massCutoffDist((double)m->mass, (double)_mz, _massCutoff);
        m->compoundLink = c;
        matches.push_back(m);
    }
}
...
void MassCalcWidget::pubChemLink(QString formula)
{
    ...
    QString requestStr(
        tr("http://pubchem.ncbi.nlm.nih.gov/search/"
           "search.cgi?cmd=search&q_type=mf&q_data=%1&simp_schtp=mf")
            .arg(formula));
...
void MassCalcWidget::keggLink(QString formula)
{
    ...
    QString requestStr(tr("http://www.genome.jp/ligand-bin/"
                          "txtsearch?column=formula&query=%1&DATABASE=compound")
                           .arg(formula));
    _mw->setUrl(requestStr);
}
