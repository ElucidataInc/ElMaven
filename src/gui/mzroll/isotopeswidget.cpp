...
void IsotopeWidget::_insertLinkForPeakGroup(PeakGroup* group)
{
    auto linkForSamplePeak = [this, group]() {
        ...
        auto quantity =
            group->getOrderedIntensityVector(samples, _mw->getUserQuantType())
                .at(0);
...
void IsotopeWidget::_pullIsotopesForFormula(string formula, bool updateBarplot)
{
    ...
    connect(workerThread, &BackgroundOpsThread::finished, this, [this]() {
        setEnabled(true);
    });
...
void IsotopeWidget::setClipboard(QList<shared_ptr<PeakGroup>>& groups)
{
    ...
    Q_FOREACH (shared_ptr<PeakGroup> group, groups) {
        ...
        QString infoText =
            groupIsotopeMatrixExport(group.get(), includeSampleHeader);
                                                ...
                                                bool includeSampleHeader)
{
    ...
    if (tag.isEmpty() && group->meanMz > 0)
        tag = QString::number(group->meanMz, 'f', 6) + "@"
              + QString::number(group->meanRt, 'f', 2);
    if (tag.isEmpty())
    ...
    if (includeSampleHeader) {
        ...
        if (includeSetNamesLine) {
            for (int i = 0; i < vsamples.size(); i++) {
                isotopeInfo +=
                    "\t" + QString(vsamples[i]->getSetName().c_str());
            }
            isotopeInfo += "\n";
        }
    }
...
void IsotopeWidget::showTable()
{
    ...
    p->clear();
    p->setColumnCount(6);
    p->setHeaderLabels(QStringList()
                       << "Isotope Name"
                       << "m/z" << _mw->quantType->currentText() << "%Labeling"
                       << "%Expected"
                       << "%Relative");
    ...
    for (unsigned int i = 0; i < isotopeParameters->links.size(); i++) {
        float frac = 0;
        if (isotopeParameters->links[i].value2 == 0
            && floor(isotopeParameters->links[i].value1 * 1e+6) == 0)
            continue;
        if (isotopeIntensitySum > 0)
            frac =
                isotopeParameters->links[i].value2 / isotopeIntensitySum * 100;

        NumericTreeWidgetItem* item =
            new NumericTreeWidgetItem(treeWidget, mzSliceType);
        QString item1 = QString(isotopeParameters->links[i].note.c_str());
        QString item2 =
            QString::number(isotopeParameters->links[i].mz2, 'f', 5);
        QString item3 =
            QString::number(isotopeParameters->links[i].value2, 'f', 4);
        //%labeling
        QString item4 = QString::number(frac, 'f', 4);
        //%Expected abundance
        QString item5 =
            QString::number(isotopeParameters->links[i].value1 * 100, 'f', 4);
        //%Relative abundance
        QString item6 = QString::number(
            isotopeParameters->links[i].value1 / maxAbundance * 100, 'f', 4);
