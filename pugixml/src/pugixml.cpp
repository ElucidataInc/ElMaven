...
{	
	...
	struct xml_parser
	{
		...
		xml_parser(xml_allocator& alloc): alloc(alloc)
		{
                ...
                int parse(char* s, xml_node_struct* xmldoc, unsigned int optmsk = parse_default)
		{
                        if (!s || !xmldoc) return 2;
			...
			while (*s != 0)
			{
									...
									if (ch == '=') // '<... #=...'
									{
										...
										if (*s == '\'' || *s == '"') // '<... #="...'
										{
											...
											// everything else will be detected
                                                                                        if (is_chartype(*s, ct_start_symbol)) return 13;
										}
                                                                                else return 14;
									}
                                                                        else return 15;
				...
				else
				{
					...
					goto LOC_TAG;
                        ...
                        if (cursor != xmldoc) {
                            fprintf(stderr, "xml parser failed: XML structure problem");
                            return 1;
...
namespace pugi
{
	...
	bool xml_document::parse(char* xmlstr, unsigned int options)
	{
		...
		xml_parser parser(alloc);
                ...
                int errcode = parser.parse(xmlstr, _root, options); // Parse the input string.
                if (errcode == 1) {
                    return true;
                } else {
                    fprintf(stderr, "parser failed: errcode %d", errcode);
                    return false;
                }
	}
